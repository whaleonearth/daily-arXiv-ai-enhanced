...
TL;DR: ViLLa is a neuro-symbolic framework that combines vision, language, and logic to monitor animals in images by interpreting queries and providing transparent, modular results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create an interpretable system for monitoring animal populations that can handle both visual data and natural language queries, addressing the limitations of black-box models.

Method: ViLLa uses a visual detection module, a language parser, and a symbolic reasoning layer with predefined rules to ground visual data into facts and answer queries about animal count, presence, and location.

Result: The system was evaluated on various animal imagery tasks and showed success in connecting visual content with structured, human-readable queries.

Conclusion: ViLLa offers a modular and transparent approach to animal monitoring, improving interpretability and reliability compared to end-to-end black-box models.
...
TL;DR: A high-performance, low-latency video surveillance system using entropy-based adaptive frame buffering and MobileNetV2 for resource-constrained devices.


<details>
  <summary>Details</summary>
Motivation: To create an efficient video surveillance system that works well on devices with limited resources like Raspberry Pi, Amazon, and Jetson Nano, while maintaining high accuracy and privacy.

Method: Proposed an entropy-based adaptive frame buffering algorithm combined with MobileNetV2 to optimize throughput and reduce latency.

Result: Achieved sub-50ms end-to-end latency, over 92% detection accuracy, and robustness under different conditions.

Conclusion: The system is scalable, cost-effective, and more privacy-compliant, suitable for smart city and embedded security applications.
...
TL;DR: This paper introduces LRCN and Transformer models using GelSight sensors to improve compliance prediction accuracy, showing better performance than baselines and highlighting challenges with harder objects.
...
Method: Proposed two models based on LRCNs and Transformers using RGB tactile images and GelSight sensor data for compliance prediction.

Result: The models showed significant performance improvement over baselines and effectively estimated compliance, but harder objects than the sensor are more challenging.

Conclusion: The LRCN and Transformer models using GelSight sensors enhance compliance prediction accuracy, addressing limitations of traditional methods and existing neural networks, though challenges remain for harder objects.
...
<details>
  <summary>Details</summary>
Motivation: Existing vector quantization methods in autoregressive models suffer from training instability and codebook collapse, which hinder their effectiveness.

Method: The paper proposes using Wasserstein distance to align the distributions of features and code vectors, preventing codebook collapse and reducing gradient discrepancies.

Result: The approach achieves near 100% codebook utilization and significantly reduces quantization error, validated by empirical and theoretical analyses.

Conclusion: By addressing the distribution mismatch through Wasserstein alignment, the paper improves the performance and stability of vector quantization in autoregressive models.
...
TL;DR: SynPo improves medical image segmentation using better negative prompts with confidence maps and K-means clustering.


<details>
  <summary>Details</summary>
Motivation: Existing training-free methods using LVMs like SAM don't effectively use negative prompts, causing poor performance on low-contrast medical images.

Method: SynPo uses a Confidence Map Synergy Module combining DINOv2 and SAM to select positive and negative points via top-k pixels and Gaussian distribution with K-means clustering for better prompts.

Result: SynPo performs as well as top training-based few-shot methods in medical image segmentation without needing training.

Conclusion: SynPo effectively enhances segmentation by improving negative prompts, showing that training-free approaches can rival training-based ones in medical imaging.
...
TL;DR: ViMAT is an AI system for real-time visual monitoring of industrial assembly tasks without needing rigid setups or visual markers.


<details>
  <summary>Details</summary>
Motivation: To prevent equipment damage and ensure worker safety in industrial assembly tasks, while avoiding the need for rigid workspace setups or visual markers.

Method: ViMAT uses a perception module to extract visual data from multi-view videos and a reasoning module to determine the likely action using task knowledge.

Result: ViMAT was validated on LEGO component replacement and hydraulic press mold reconfiguration tasks, showing effectiveness in real-world scenarios with partial/uncertain observations.

Conclusion: ViMAT provides a flexible solution for real-time visual monitoring in industrial assembly without relying on rigid setups or markers.
...
TL;DR: This paper proposes MapFM, an enhanced end-to-end model for online vectorized HD map generation in autonomous driving, using a foundation model for better feature encoding and auxiliary heads for improved semantic segmentation and prediction accuracy.
...
Method: MapFM uses a foundation model to encode camera images, enhancing feature representation. It also integrates auxiliary prediction heads for semantic segmentation in the BEV representation, enabling multi-task learning with contextual supervision.

Result: The multi-task learning approach with auxiliary heads leads to better environmental understanding and higher accuracy in predicted vectorized HD maps.

Conclusion: By leveraging foundation models and multi-task learning with auxiliary heads, MapFM improves the quality of HD maps, which is essential for accurate autonomous driving systems.
...
TL;DR: This paper introduces MRG-LLM, a multimodal model combining a frozen LLM with a learnable visual encoder and dynamic prompt customization for better medical report generation.


<details>
  <summary>Details</summary>
Motivation: Medical report generation from imaging data is challenging, and while LLMs show promise, their integration with medical images needs more exploration.

Method: MRG-LLM combines a frozen LLM with a learnable visual encoder and uses dynamic prompt customization via conditional affine transformations from visual features.

Result: MRG-LLM achieves state-of-the-art performance on IU X-ray and MIMIC-CXR datasets for medical report generation.

Conclusion: The paper concludes that MRG-LLM effectively enhances medical report generation through its multimodal approach and dynamic prompting, with code to be released publicly.
...
TL;DR: The NTIRE 2025 Shadow Removal Challenge had 306 participants, with 17 teams submitting solutions. It had two tracks: reconstruction fidelity and visual perception via user study, evaluated on the WSRD+ dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance shadow removal techniques by evaluating them on a diverse dataset with both reconstruction fidelity and visual perception metrics.

Method: The method involved organizing a challenge with two tracks: one for reconstruction fidelity and another for visual perception through a user study.

Result: 17 teams successfully submitted solutions in the final evaluation phase out of 306 registered participants.

Conclusion: The challenge successfully engaged a large number of participants and evaluated solutions on a diverse dataset, promoting advancements in shadow removal techniques.
...
TL;DR: This paper introduces Show-o2, an improved native unified multimodal model that uses autoregressive modeling and flow matching, built on a 3D causal variational autoencoder space with dual-path spatial-temporal fusion, enabling scalability across image and video modalities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a more versatile and scalable multimodal model that can effectively handle tasks involving text, images, and videos.

Method: The method involves using a 3D causal variational autoencoder space, dual-path spatial(-temporal) fusion for unified visual representations, autoregressive modeling for the language head, and flow matching for the flow head, along with a two-stage training recipe.

Result: The Show-o2 models show versatility in handling various multimodal tasks across different modalities, including text, images, and videos.

Conclusion: The paper concludes that Show-o2 is an effective and scalable solution for multimodal understanding and generation, with code and models publicly available.
...
TL;DR: The paper introduces MEAL, a new benchmark for continual multi-agent reinforcement learning (CMARL) that uses JAX for GPU acceleration, allowing for efficient learning across long task sequences. It highlights the limitations of existing methods and identifies key features for effective CMARL.


<details>
  <summary>Details</summary>
Motivation: Benchmarks are essential for developing and analyzing RL algorithms, but existing ones have limitations in handling continual learning in cooperative multi-agent settings, which is underexplored.

Method: The authors developed MEAL, a benchmark for CMARL using JAX for GPU acceleration, and conducted experiments and ablation studies to evaluate and identify critical features for CMARL.

Result: MEAL enables efficient continual learning across 100 tasks on a standard desktop PC. Combining existing CL and MARL methods works well on simple environments but fails in complex ones requiring sustained coordination.

Conclusion: The paper concludes that MEAL is a valuable benchmark for CMARL research, emphasizing the need for architectural and algorithmic features that support sustained coordination and adaptation in complex settings.
...
TL;DR: This paper introduces CAN, a causality-aware navigation framework for Embodied AI agents, which improves performance by modeling causal relationships in sequential data.


<details>
  <summary>Details</summary>
Motivation: Existing visual navigation methods process all historical observations at once, missing internal associations that could limit performance improvements.

Method: Examines navigation tasks through causality, introduces Causal Understanding Module in CAN to enhance environmental understanding.

Result: Empirical evaluations show CAN outperforms baselines across tasks and environments, with ablation studies confirming effectiveness of the Causal Understanding Module.

Conclusion: CAN improves visual navigation by leveraging causal structures, offering better performance without extra computation.
...
TL;DR: This paper introduces an LLM-based Planning Agent (LPA) to automate tasks in Failure Analysis labs, such as detecting non-conformities, retrieving cases, and generating reports, by orchestrating AI components into efficient workflows.


<details>
  <summary>Details</summary>
Motivation: The motivation is to automate intricate tasks in FA labs using AI, but the challenge is integrating multiple AI models into cohesive workflows that align with FA processes.

Method: The method involves designing and implementing an LLM-based Planning Agent that combines LLMs with planning capabilities and external tools for autonomous processing of complex queries and data retrieval.

Result: The evaluation shows the LPA is effective and reliable in supporting FA tasks.

Conclusion: The LPA can enhance FA processes by efficiently orchestrating AI components, leading to improved automation and support for engineers.
...
TL;DR: The paper demonstrates that LLMs can generate reversible sentence embeddings using a special memory token, enabling exact text reconstruction without changing model weights.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore whether LLMs can create reversible embeddings for exact text reconstruction, which could have applications in memory retrieval, compression, and controlled generation.

Method: The method involves introducing a special memory token whose embedding is optimized during training on a fixed sequence, allowing the model to reconstruct the original text when prompted with this embedding.

Result: The results show that Llama 3.1 8B can reconstruct all tested sequences, and the approach works across English and Spanish datasets with sequences up to 240 tokens and varying model sizes.

Conclusion: The conclusion is that LLMs have this capability, suggesting potential uses in memory-based retrieval, compression, and controlled text generation.
...
TL;DR: TopClustRAG is a RAG system using hybrid retrieval and K-Means clustering to improve answer quality in large-scale web question answering.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenges of end-to-end question answering over large web corpora by enhancing answer diversity, relevance, and faithfulness through clustering-based methods.

Method: TopClustRAG uses a hybrid retrieval strategy (sparse and dense indices) followed by K-Means clustering to group similar passages. Cluster-specific prompts are generated for an LLM to create intermediate answers, which are then filtered, reranked, and synthesized.

Result: TopClustRAG ranked 2nd in faithfulness and 7th in correctness on the FineWeb Sample-10BT dataset in the LiveRAG Challenge.

Conclusion: The study demonstrates that clustering-based context filtering and prompt aggregation can effectively improve the performance of large-scale RAG systems.
...
TL;DR: The paper surveys LLM-based approaches for clinical trial recruitment, highlighting their potential and challenges.


<details>
  <summary>Details</summary>
Motivation: To address the limited adoption of LLMs in critical domains like clinical trial recruitment, where trial descriptions and patient data are in natural language, requiring knowledge aggregation and reasoning.

Method: The paper conducts a survey of existing benchmarks, approaches, and evaluation frameworks for trial-patient matching using LLMs.

Result: The survey identifies challenges in adopting LLMs for clinical research and outlines future directions for improvement.

Conclusion: LLMs have potential for a more general solution in clinical trial recruitment, but issues with proprietary models and weak benchmarks remain.
...
TL;DR: This paper introduces a fine-grained evaluation framework and a training-free technique called Context-Aware Cropping to address hallucinations in multimodal large language models (MLLMs) for GUI agents, enhancing their reliability and interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of hallucinations in MLLMs used for GUI agents, which are systematic localization errors that reduce reliability, and to provide better quantification of model uncertainty beyond traditional accuracy metrics.

Method: The method involves a fine-grained evaluation framework that categorizes predictions into four types and introduces the Peak Sharpness Score (PSS) to evaluate semantic continuity and logits distribution alignment. Additionally, a training-free technique named Context-Aware Cropping is proposed to improve performance by adaptively refining input context.

Result: The experiments show that the proposed framework and methods offer actionable insights, improve interpretability, and enhance the robustness of GUI agent behavior.

Conclusion: The conclusion is that the framework and techniques effectively mitigate hallucinations in MLLMs for GUI agents, leading to more reliable and interpretable system interactions.
...
TL;DR: This paper addresses the challenge of approximating training data from language model weights, introducing a gradient-based method that effectively recovers relevant data from public corpora, achieving significant performance improvements in tasks like AG News classification and SFT on MSMARCO.


<details>
  <summary>Details</summary>
Motivation: The motivation is to formalize the problem of data approximation from model weights, as modern language models often have open weights but closed training data, making it difficult to understand or replicate their training process.

Method: The method involves a gradient-based approach to select the highest-matching data from a large public text corpus, enabling the recovery of useful training data without prior knowledge of the original data.

Result: The method successfully recovers a small subset of public Web documents that can train a model to near-original performance, improving AG News classification accuracy from 65% to 80% and reducing perplexity in SFT tasks.

Conclusion: The conclusion states that even without access to true training data, the proposed method can effectively approximate training data for language models, achieving performance close to expert models and offering a practical solution for data recovery.
...
TL;DR: This paper introduces GIFI, a new metric for evaluating gender fairness in LLMs, and finds significant variations in inclusivity across 22 models.


<details>
  <summary>Details</summary>
Motivation: Previous studies on gender fairness in LLMs have focused mainly on binary gender distinctions, but there is a need to address non-binary gender inclusivity as well.

Method: The paper introduces GIFI, a comprehensive metric that evaluates LLMs at multiple levels, including probing with gender pronouns and testing model generation and cognitive behaviors under different gender assumptions.

Result: The evaluation of 22 LLMs using GIFI reveals significant variations in their gender inclusivity.

Conclusion: The study emphasizes the importance of improving LLMs' gender inclusivity and provides a benchmark for future research in gender fairness for generative models.
...
TL;DR: SciVer is a new benchmark for testing how well foundation models can check claims in scientific papers that have images and text. It has 3,000 examples from 1,113 papers and shows that these models aren't as good as humans.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the need to create a specialized benchmark for evaluating multimodal foundation models in scientific claim verification, which is crucial for ensuring the reliability of AI in scientific research.

Method: The method involves creating the SciVer benchmark with 3,000 expert-annotated examples from 1,113 scientific papers, organized into four subsets based on common reasoning types. They test 21 top multimodal models and analyze their performance through RAG and human error evaluations.

Result: The results show that current multimodal foundation models significantly underperform compared to human experts on the SciVer benchmark, highlighting major limitations in their ability to handle multimodal scientific reasoning tasks.

Conclusion: The conclusion is that SciVer provides a valuable tool for assessing and improving AI models' capabilities in verifying scientific claims, emphasizing the need for better comprehension and reasoning in multimodal scientific literature tasks.
...
TL;DR: This study evaluates six machine learning models for Romanian authorship attribution using character n-grams, finding ANN achieves high accuracy, even perfect in some cases, showing simple features can rival complex methods in low-resource languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address authorship attribution for Romanian texts using a standard benchmark (ROST corpus) and explore if lightweight, interpretable methods can achieve state-of-the-art results in resource-constrained or under-studied languages.

Method: The method involves systematically evaluating six machine learning techniques (SVM, LR, k-NN, DT, RF, ANN) with character n-gram features for classification.

Result: The ANN model achieved the highest performance, with perfect classification in four out of fifteen runs using 5-gram features. Other models also showed good results, but ANN was superior.

Conclusion: The conclusion is that lightweight, interpretable character n-gram approaches can deliver state-of-the-art accuracy for Romanian authorship attribution, rivaling more complex methods, and highlighting the potential of simple features in low-resource language settings.
...
TL;DR: This paper explores privacy leaks in the internal reasoning processes of large models used as personal agents, showing that increased reasoning steps can lead to more data leakage despite improved output safety.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the overlooked privacy risks in model reasoning traces, which are often considered internal and safe but can actually expose sensitive user data.

Method: The method involves probing and agentic evaluations to assess how test-time compute approaches, especially increased reasoning steps, affect privacy leakage.

Result: The result shows that increasing the reasoning steps in test-time compute approaches leads to more verbose reasoning and greater data leakage, even though the final outputs become more cautious.

Conclusion: The conclusion emphasizes the need for safety measures to focus on both model outputs and internal reasoning processes to mitigate privacy risks.
...
TL;DR: The paper evaluates 21 MT systems for their ability to handle gender neutrality in translations, finding mostly a lack of gender-neutral outputs but some systems using specific strategies for certain languages.


<details>
  <summary>Details</summary>
Motivation: To address the issue of gender-inclusive machine translation and prevent misgendering and representational harms by preserving source gender ambiguity.

Method: Assessing the sensitivity of MT systems to gender neutrality in three translation directions, categorizing observed strategies, and examining the effect of binary gender stereotypes.
...
Conclusion: The study highlights the need for better gender-neutral translation practices in MT systems, emphasizing that while current performance is inadequate, some systems show potential through targeted strategies.
...
### [131] [Comparative Analysis of QNN Architectures for Wind Power Prediction: Feature Maps and Ansatz Configurations](https://arxiv.org/abs/2506.14795)
...
### [132] [Advanced Prediction of Hypersonic Missile Trajectories with CNN-LSTM-GRU Architectures](https://arxiv.org/abs/2506.15043)
*Amir Hossein Baradaran*

Main category: cs.CR

TL;DR: This paper presents a hybrid deep learning model using CNN, LSTM, and GRU for accurate hypersonic missile trajectory prediction.


<details>
  <summary>Details</summary>
Motivation: Hypersonic missiles are a critical threat due to their high speed and maneuverability, requiring accurate trajectory prediction for effective defense.

Method: The authors developed a novel hybrid deep learning approach combining CNNs, LSTMs, and GRUs to predict complex missile trajectories.

Result: The proposed method achieves high accuracy in predicting hypersonic missile trajectories, contributing to better defense strategies and interception technologies.

Conclusion: The research highlights the potential of advanced machine learning techniques in improving defense system predictive capabilities.
...
### [133] [RAS-Eval: A Comprehensive Benchmark for Security Evaluation of LLM Agents in Real-World Environments](https://arxiv.org/abs/2506.15253)
...
### [134] [Evaluation Pipeline for systematically searching for Anomaly Detection Systems](https://arxiv.org/abs/2506.15388)
*Florian Rokohl,Alexander Lehnert,Marc Reichenbach*

Main category: cs.CR

TL;DR: This paper proposes an anomaly detection system on hardware using FPGAs to secure the medical world against network intruders while meeting real-time and power restrictions.


<details>
  <summary>Details</summary>
Motivation: Digitalization in the medical world brings significant benefits but also makes it a target for attackers, making security a critical challenge.

Method: The paper presents a hardware-based anomaly detection system utilizing FPGAs to address real-time and power constraints.

Result: The system achieves overall performance through a holistic system evaluation, effectively detecting malicious clients in real-time.

Conclusion: By leveraging FPGAs, the proposed system offers a viable solution for real-time anomaly detection in the medical field, enhancing security without compromising performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing motion capture methods using IMUs assume tight attachment, which is not always feasible in real-world applications. This paper aims to address the challenges of using loosely attached sensors for pose estimation.

Method: The authors simulate IMU data from a garment-aware dataset and use transformer-based diffusion models to synthesize data and estimate poses. They also incorporate garment-related parameters during training.

Result: The proposed diffusion methods trained on simulated data outperformed state-of-the-art approaches in both quantitative and qualitative experiments.

Conclusion: The study demonstrates that using diffusion models with garment parameters can effectively handle loose IMU data, providing a promising direction for future research in motion capture.
...
### [138] [Forecasting the spatiotemporal evolution of fluid-induced microearthquakes with deep learning](https://arxiv.org/abs/2506.14923)
...
### [139] [Efficient Serving of LLM Applications with Probabilistic Demand Modeling](https://arxiv.org/abs/2506.14851)
...
### [140] [Cost-Efficient Serving of LLM Agents via Test-Time Plan Caching](https://arxiv.org/abs/2506.14852)
*Qizheng Zhang,Michael Wornow,Kunle Olukotun*

Main category: cs.DC

TL;DR: This paper introduces agentic plan caching to reduce costs in LLM-based agentic applications by reusing structured plan templates across similar tasks.


<details>
  <summary>Details</summary>
Motivation: LLM-based agentic applications are costly due to extensive planning and reasoning, and existing caching methods aren't suitable for tasks dependent on external data or environments.

Method: Extracting, storing, adapting, and reusing structured plan templates from previous agent executions using keyword extraction and lightweight models for task-specific adaptation.

Result: The system reduces costs by 46.62% on average across multiple real-world applications while maintaining performance.

Conclusion: Agentic plan caching provides an efficient complementary solution to existing LLM serving infrastructures for reducing serving costs in agentic applications.
...
### [141] [Data Verbalisation: What is Text Doing in a Data Visualisation?](https://arxiv.org/abs/2506.15129)
*Paul Murrell*
...
### [142] [See What I Mean? CUE: A Cognitive Model of Understanding Explanations](https://arxiv.org/abs/2506.14775)
...
### [143] [WebXAII: an open-source web framework to study human-XAI interaction](https://arxiv.org/abs/2506.14777)
...
### [144] [Analyzing Character Representation in Media Content using Multimodal Foundation Model: Effectiveness and Trust](https://arxiv.org/abs/2506.14799)
...
### [145] [The Hardness of Achieving Impact in AI for Social Impact Research: A Ground-Level View of Challenges & Opportunities](https://arxiv.org/abs/2506.14829)
...
### [146] [Insights Informed Generative AI for Design: Incorporating Real-world Data for Text-to-Image Output](https://arxiv.org/abs/2506.15008)
...
### [147] [Mapping Caregiver Needs to AI Chatbot Design: Strengths and Gaps in Mental Health Support for Alzheimer's and Dementia Caregivers](https://arxiv.org/abs/2506.15047)
...
### [148] [Accessible Gesture-Driven Augmented Reality Interaction System](https://arxiv.org/abs/2506.15189)
*Yikan Wang*
...
### [149] [Co-Creative Learning via Metropolis-Hastings Interaction between Humans and AI](https://arxiv.org/abs/2506.15468)
...
### [150] [Optimizing Web-Based AI Query Retrieval with GPT Integration in LangChain A CoT-Enhanced Prompt Engineering Approach](https://arxiv.org/abs/2506.15512)
*Wenqi Guan,Yang Fang*

Main category: cs.HC

TL;DR: This paper proposes using GPT models in LangChain to improve remote learning resource retrieval with CoT reasoning and prompt engineering, showing better user satisfaction and learning outcomes.
...
Method: Integrating GPT-based models into the LangChain framework with CoT reasoning and prompt engineering.

Result: Improved precision, relevance, user satisfaction, and learning outcomes compared to paradigmatic LLMs.

Conclusion: The proposed framework effectively provides comprehensive, contextually enriched explanations and resources tailored to student needs.
...
### [151] [Preparing for the Intelligence Explosion](https://arxiv.org/abs/2506.14863)
*William MacAskill,Fin Moorhouse*

Main category: cs.CY

TL;DR: The paper discusses the potential of AI to accelerate technological progress and the need for AGI preparedness to address future grand challenges.


<details>
  <summary>Details</summary>
Motivation: The motivation is to highlight the urgent need to address the consequential and hard-to-reverse decisions that may arise from rapid technological and political developments driven by AI.

Method: The method involves analyzing the potential impacts of AI and identifying the grand challenges that may emerge from an intelligence explosion.

Result: The result is a call to action for immediate preparation for the challenges posed by advanced AI systems, beyond just ensuring alignment.

Conclusion: The conclusion emphasizes that AGI preparedness should include proactive measures to handle the disorienting range of developments from an intelligence explosion.
...
### [152] [Hypothesis Testing for Quantifying LLM-Human Misalignment in Multiple Choice Settings](https://arxiv.org/abs/2506.14997)
*Harbin Hong,Sebastian Caldas,Liu Leqi*

Main category: cs.CY

TL;DR: This paper introduces a framework using hypothesis testing to evaluate how well LLMs simulate human behaviors in surveys, finding that a popular model fails for certain sub-populations.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used in social science research, it's important to check if they accurately replicate human behavior.

Method: The paper uses hypothesis testing to create a quantitative framework for assessing the misalignment between LLM-simulated and actual human behaviors in multiple-choice surveys.

Result: The framework showed that a popular language model isn't suitable for simulating opinions of different sub-populations (like various races, ages, incomes) on contentious issues.

Conclusion: The study questions the model's alignment with these populations and suggests that new practices are needed for using LLMs in social science beyond simple simulations.
...
### [153] [Transit for All: Mapping Equitable Bike2Subway Connection using Region Representation Learning](https://arxiv.org/abs/2506.15113)
...
### [154] [On the Effectiveness of Classical Regression Methods for Optimal Switching Problems](https://arxiv.org/abs/2506.15436)
*Martin Andersson,Benny Avelin,Marcus Olofsson*

Main category: math.OC

TL;DR: Simple regression methods like k-NN are effective for optimal switching problems, even in high dimensions, due to their robustness against approximation bias and Monte Carlo noise.


<details>
  <summary>Details</summary>
Motivation: To find robust and near-optimal solutions for optimal switching problems in various dimensions without extensive hyperparameter tuning.

Method: Tested eight regression approaches on four benchmark problems, analyzed the effectiveness of k-NN under jump-diffusion dynamics, and used PCA to enable k-NN to scale to high dimensions.

Result: Simple regression methods maintained stable performance across diverse problem characteristics, even after neural network optimization. Contaminated training targets favored robust methods over complex ones like neural networks.

Conclusion: Simple, minimally-tuned regression methods provide reliable performance for computationally demanding switching problems, making them a practical choice for practitioners.
...
TL;DR: The paper proposes an extension of multiple contrast testing procedures (MCTPs) within a semiparametric MANCOVA framework to evaluate intervention effects on multiple outcomes, accounting for confounding and allowing applicability beyond traditional assumptions like multivariate normality.


<details>
  <summary>Details</summary>
Motivation: Researchers often need to compare interventions across multiple correlated outcomes, requiring methods that handle multivariate data and confounding variables, which traditional MCTPs may not adequately address due to strict assumptions.

Method: The authors extend MCTPs in a semiparametric MANCOVA framework, which is more flexible than traditional methods by not requiring multivariate normality, homoscedasticity, or non-singular covariance structures.

Result: The approach was illustrated with multivariate psychological intervention data, focusing on constructs like heart rate variability that combine physiological and psychological factors.

Conclusion: The proposed semiparametric MCTP extension offers a more robust method for evaluating intervention effects across multiple outcomes while accounting for confounding, applicable in broader scenarios than traditional approaches.
...
### [159] [An Empirical Study of Bugs in Data Visualization Libraries](https://arxiv.org/abs/2506.15084)
...
### [160] [Uncovering Intention through LLM-Driven Code Snippet Description Generation](https://arxiv.org/abs/2506.15453)
...
### [161] [Explain First, Trust Later: LLM-Augmented Explanations for Graph-Based Crypto Anomaly Detection](https://arxiv.org/abs/2506.14933)
*Adriana Watson*

Main category: cs.CE

TL;DR: The paper discusses the need for automated detection tools in DeFi due to rising financial crime linked to cryptocurrency's growth.


<details>
  <summary>Details</summary>
Motivation: The DeFi community's rapid growth and cryptocurrency's popularity have led to increased financial crime, but the technology's novelty complicates enforcement.

Method: The paper suggests developing automated tools aligned with regulatory policies to detect and prosecute offenders in cryptocurrency markets.

Result: The implementation of these tools could effectively address the challenges posed by financial crime in DeFi.

Conclusion: Automated detection systems are crucial for managing criminal activities in the evolving DeFi landscape.
...
### [162] [Deploying and Evaluating Multiple Deep Learning Models on Edge Devices for Diabetic Retinopathy Detection](https://arxiv.org/abs/2506.14834)
...
### [163] [Foundation Artificial Intelligence Models for Health Recognition Using Face Photographs (FAHR-Face)](https://arxiv.org/abs/2506.14909)
...
### [164] [Empirical Studies of Large Scale Environment Scanning by Consumer Electronics](https://arxiv.org/abs/2506.14771)
...
### [169] [Privacy-Preserving Chest X-ray Classification in Latent Space with Homomorphically Encrypted Neural Inference](https://arxiv.org/abs/2506.15258)
...
### [170] [FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation](https://arxiv.org/abs/2506.15365)
...
### [171] [A Real-time Endoscopic Image Denoising System](https://arxiv.org/abs/2506.15395)
...
### [172] [Advanced cervical cancer classification: enhancing pap smear images with hybrid PMD Filter-CLAHE](https://arxiv.org/abs/2506.15489)
...
### [173] [Automated MRI Tumor Segmentation using hybrid U-Net with Transformer and Efficient Attention](https://arxiv.org/abs/2506.15562)
...
### [174] [Improved Image Reconstruction and Diffusion Parameter Estimation Using a Temporal Convolutional Network Model of Gradient Trajectory Errors](https://arxiv.org/abs/2506.14995)
*Jonathan B. Martin,Hannah E. Alderson,John C. Gore,Mark D. Does,Kevin D. Harkins*

Main category: physics.med-ph

TL;DR: The paper proposes using a temporal convolutional network to model and correct gradient distortions in non-Cartesian MRI, improving image quality and diffusion mapping.


<details>
  <summary>Details</summary>
Motivation: Errors in gradient trajectories cause artifacts in MRI, especially in non-Cartesian sequences. Current methods are insufficient for nonlinear distortions.

Method: Trained a temporal convolutional network on measured gradient waveforms from a small animal imaging system to predict distortions.

Result: The network accurately predicted nonlinear distortions and enhanced image quality and diffusion parameter mapping when integrated into reconstruction.

Conclusion: TCNs outperform linear methods in modeling gradient systems and enable retrospective correction of gradient errors.
...
### [176] [Advancing Loss Functions in Recommender Systems: A Comparative Study with a Rényi Divergence-Based Solution](https://arxiv.org/abs/2506.15120)
...
### [177] [Scaling Intelligence: Designing Data Centers for Next-Gen Language Models](https://arxiv.org/abs/2506.15006)
...
### [178] [J3DAI: A tiny DNN-Based Edge AI Accelerator for 3D-Stacked CMOS Image Sensor](https://arxiv.org/abs/2506.15316)
...
### [179] [An accurate and revised version of optical character recognition-based speech synthesis using LabVIEW](https://arxiv.org/abs/2506.15029)
...
<details>
  <summary>Details</summary>
Motivation: Visually impaired individuals face limitations in accessing books due to reliance on Braille and audio recordings, which are not always available or convenient.

Method: The paper describes the development of an OCR-based speech synthesis system using LabVIEW.

Result: The system is accurate, reliable, cost-effective, and user-friendly.

Conclusion: The OCR-based speech synthesis system using LabVIEW can improve access to books for blind and visually impaired persons.
...
### [180] [SonicVerse: Multi-Task Learning for Music Feature-Informed Captioning](https://arxiv.org/abs/2506.15154)
...
### [181] [pycnet-audio: A Python package to support bioacoustics data processing](https://arxiv.org/abs/2506.14864)
...
### [182] [MODS: Multi-source Observations Conditional Diffusion Model for Meteorological State Downscaling](https://arxiv.org/abs/2506.14798)
...
### [183] [Feedback-MPPI: Fast Sampling-Based MPC via Rollout Differentiation -- Adios low-level controllers](https://arxiv.org/abs/2506.14855)
...
### [184] [FEAST: A Flexible Mealtime-Assistance System Towards In-the-Wild Personalization](https://arxiv.org/abs/2506.14968)
...
### [185] [Towards Perception-based Collision Avoidance for UAVs when Guiding the Visually Impaired](https://arxiv.org/abs/2506.14857)
*Suman Raj,Swapnil Padhi,Ruchi Bhoot,Prince Modi,Yogesh Simmhan*

Main category: cs.RO

TL;DR: This paper presents a perception-based path planning system for drones assisting visually impaired people, combining local and global planning with multi DNN for obstacle avoidance, validated in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: To develop autonomous navigation systems for drones that can assist visually impaired individuals in outdoor urban environments, improving their mobility and safety.

Method: Developed a perception-based local path planning system integrated with a global planner using GPS and maps, and proposed a multi DNN framework for obstacle avoidance for both the UAV and the VIP.

Result: The algorithms were verified feasible in three scenarios: walking on footpaths, near parked vehicles, and in crowded streets.

Conclusion: The study demonstrates the potential of drones equipped with machine learning and computer vision for assisting VIPs in navigating urban areas, highlighting the effectiveness of the integrated path planning and obstacle avoidance system.
...
### [186] [Robust Instant Policy: Leveraging Student's t-Regression Model for Robust In-context Imitation Learning of Robot Manipulation](https://arxiv.org/abs/2506.15157)
...
### [187] [MCOO-SLAM: A Multi-Camera Omnidirectional Object SLAM System](https://arxiv.org/abs/2506.15402)
...
### [188] [Particle-Grid Neural Dynamics for Learning Deformable Object Models from RGB-D Videos](https://arxiv.org/abs/2506.15680)
...
### [189] [BMFM-RNA: An Open Framework for Building and Evaluating Transcriptomic Foundation Models](https://arxiv.org/abs/2506.14861)
...
### [190] [On the Upper Bounds for the Matrix Spectral Norm](https://arxiv.org/abs/2506.15660)
...
### [191] [Intrinsic and Extrinsic Organized Attention: Softmax Invariance and Network Sparsity](https://arxiv.org/abs/2506.15541)
...
TL;DR: This paper introduces directional-SpeechLlama, a method using smart glasses' microphone arrays for directional speech recognition, source localization, and suppressing bystander cross-talk. It uses S-DOT and CDDA techniques to improve understanding of spatial audio.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore and enhance the capabilities of Speech LLMs in handling multi-channel audio with spatial cues, which is an under-researched area despite recent advancements in prompting LLMs with audio encodings for speech recognition.

Method: The method involves directional-SpeechLlama, which utilizes the microphone array of smart glasses. Two techniques are introduced: serialized directional output training (S-DOT) and contrastive direction data augmentation (CDDA) to improve the model's understanding of directivity in spatial audio.

Result: The results demonstrate that directional-SpeechLlama effectively captures the relationship between textual cues and spatial audio, showing strong performance in speech recognition and source localization tasks.

Conclusion: The conclusion states that directional-SpeechLlama is a successful approach that enhances Speech LLMs' ability to process multi-channel audio with spatial cues, particularly in directional speech recognition, source localization, and bystander cross-talk suppression.
...
TL;DR: HAC is a new neural speech codec that separates speech into acoustic, phonetic, and lexical levels using knowledge distillation from HuBERT and LaBSE, achieving better disentanglement and reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Current speech codecs often fail to capture both acoustic details and lexical meanings effectively, leading to suboptimal performance in speech generation and understanding tasks. There is a need for a unified representation that can disentangle these linguistic levels.

Method: HAC factorizes the bottleneck into three linguistic levels (acoustic, phonetic, lexical) using two knowledge distillation objectives: one from HuBERT for phoneme-level structure and another from LaBSE for lexical cues.

Result: HAC's factorized bottleneck produces disentangled token sets, with one aligning to phonemes and another capturing word-level semantics. It outperforms single-level baselines in disentanglement and reconstruction quality while preserving naturalness.

Conclusion: HAC offers a unified discrete speech representation that effectively bridges acoustic detail and lexical meaning, showing promise for downstream speech tasks.
...
### [198] [Identifiability by common backdoor in summary causal graphs of time series](https://arxiv.org/abs/2506.14862)
*Clément Yvernes,Charles K. Assaad,Emilie Devijver,Eric Gaussier*

Main category: math.ST

TL;DR: The paper addresses the identifiability of interventions in time series using summary causal graphs, providing conditions and algorithms for determining if total effects can be computed from observational data.


<details>
  <summary>Details</summary>
Motivation: To assess if total effects of interventions can be determined from observational data without requiring do-calculus, especially in time series with limited causal graph information.

Method: Analyzing identifiability through common backdoor sets in time series, establishing conditions for their existence in both consistent and non-consistent scenarios, and developing efficient algorithms for identifiability checks.

Result: Conditions for the existence of a common backdoor set in time series with and without consistency are established, along with algorithms for identifiability decision-making.

Conclusion: The study contributes to causal inference by enabling identification of intervention effects from observational data in time series settings with summary causal graphs.
...
### [199] [Probabilistic closed-form formulas for pricing nonlinear payoff variance and volatility derivatives under Schwartz model with time-varying log-return volatility](https://arxiv.org/abs/2506.15386)
...
### [200] [Density estimation via periodic scaled Korobov kernel method with exponential decay condition](https://arxiv.org/abs/2506.15419)
...
TL;DR: This paper introduces MedSyn, a hybrid human-AI framework for clinical decision-making through interactive dialogues between physicians and LLMs, showing promise as physician assistants.


<details>
  <summary>Details</summary>
Motivation: Clinical decision-making is complex and affected by biases, incomplete info, and ambiguity; current LLMs may not capture real-world medical practice due to limited interaction.

Method: Propose MedSyn, a framework enabling multi-step interactive dialogues between physicians and LLMs to refine diagnoses and treatments.

Result: Open-source LLMs are promising as physician assistants in real-world scenarios, as demonstrated by simulated interactions.

Conclusion: Future work involves real physician interactions to validate MedSyn's impact on diagnostic accuracy and patient outcomes.
...
TL;DR: The paper introduces TRAINCHECK, a framework for proactively detecting silent training errors in deep learning models by inferring tailored invariants and identifying bugs in training libraries.


<details>
  <summary>Details</summary>
Motivation: Deep learning training is complex and error-prone, with silent errors being hard to detect. Existing methods may not effectively address these issues, necessitating a proactive approach.

Method: TRAINCHECK automatically infers invariants specific to DL training and uses them to detect silent errors during training, offering debugging assistance.

Result: TRAINCHECK detected 18 out of 20 reproduced silent errors in one training iteration and found 6 previously unknown bugs in popular training libraries.

Conclusion: TRAINCHECK provides an effective proactive method for detecting and diagnosing silent training errors in DL models, improving reliability and debugging efficiency.
...
TL;DR: The paper proposes using Group Relative Policy Optimization (GRPO) to train smaller VLMs with external tools, improving performance on VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Vision-language models (VLMs) have limited detailed visual reasoning abilities, especially under constrained compute resources. The goal is to enhance their performance on VQA tasks without requiring large-scale models.

Method: The authors use GRPO to train smaller models, enabling them to leverage external tools like zoom. They also implement a simple reward structure, a simplified tool-calling interface, and a training data mix emphasizing visually challenging examples.

Result: The method outperforms similarly-sized baseline models on certain VQA tasks, demonstrating that smaller models can achieve better performance with GRPO and external tools.

Conclusion: Combining GRPO learning, simplified reward structures, tool interfaces, and focused training data enhances detailed visual reasoning in smaller VLMs, offering a viable alternative to large models under compute constraints.
...
TL;DR: This paper proposes ETS, a framework combining EEG and eye-tracking data for open-vocabulary text generation and sentiment classification, achieving better performance than previous methods.


<details>
  <summary>Details</summary>
Motivation: Decoding natural language from EEG is challenging, especially for open-vocabulary scenarios where traditional methods fail due to noise and variability. Previous studies have only succeeded on small vocabularies.

Method: The study introduces ETS, integrating EEG with synchronized eye-tracking data to tackle open-vocabulary text generation and sentiment analysis. It uses a model that leverages both modalities for improved decoding.

Result: The model shows superior performance on BLEU and Rouge scores for EEG-To-Text decoding and achieves up to 10% higher F1 score on EEG-based ternary sentiment classification compared to supervised baselines.

Conclusion: ETS demonstrates effectiveness in handling diverse subjects and data sources, indicating promising potential for developing high-performance open-vocabulary EEG-to-text systems.
...
TL;DR: This paper proposes using machine learning algorithms to automatically set the separation threshold in Autoencoder-based Anomaly Detection Systems, addressing data imbalance issues.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of Anomaly Detection Systems by automatically determining the separation threshold, which is currently defined manually and affects detection accuracy.

Method: The method involves evaluating three machine learning algorithms: K-Nearest Neighbors, K-Means, and Support Vector Machine, to automatically define the separation threshold for reconstruction error in AEs.

Result: The result shows that using machine learning algorithms can effectively automate the threshold definition, leading to better performance in anomaly detection despite data imbalance.

Conclusion: The conclusion is that automatically defining the threshold with ML algorithms enhances the effectiveness of AEs in detecting anomalies, making them more reliable for digital security.
...
TL;DR: The paper introduces a MA-MAB framework that ensures fairness and efficiency through a probing mechanism, with algorithms for both offline and online settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of decision-making under limited information about arm rewards in multi-agent systems while ensuring fair outcomes and maximizing system performance.

Method: The method involves a novel probing framework that gathers information about selected arms before allocation. For offline settings, a greedy algorithm using submodular properties is designed. For online settings, an algorithm with sublinear regret and fairness is developed.

Result: Experiments on synthetic and real-world datasets demonstrate that the proposed approach outperforms baseline methods in terms of fairness and efficiency.

Conclusion: The conclusion is that the MA-MAB framework with the probing mechanism effectively balances fairness and system performance, showing superior results compared to existing methods.
...
### [219] [LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning](https://arxiv.org/abs/2506.15606)
...
### [220] [AutoRule: Reasoning Chain-of-thought Extracted Rule-based Rewards Improve Preference Learning](https://arxiv.org/abs/2506.15651)
...
### [221] [Stable CDE Autoencoders with Acuity Regularization for Offline Reinforcement Learning in Sepsis Treatment](https://arxiv.org/abs/2506.15019)
*Yue Gao*
...
### [222] [SFT-GO: Supervised Fine-Tuning with Group Optimization for Large Language Models](https://arxiv.org/abs/2506.15021)
...
### [223] [Dense SAE Latents Are Features, Not Bugs](https://arxiv.org/abs/2506.15679)
...
### [224] [CACTUS as a Reliable Tool for Early Classification of Age-related Macular Degeneration](https://arxiv.org/abs/2506.14843)
...
### [227] [Unlocking Post-hoc Dataset Inference with Synthetic Data](https://arxiv.org/abs/2506.15271)
...
### [228] [Pixel-level Certified Explanations via Randomized Smoothing](https://arxiv.org/abs/2506.15499)
*Alaa Anani,Tobias Lorenz,Mario Fritz,Bernt Schiele*

Main category: cs.LG

TL;DR: The paper proposes a certification framework for pixel-level attribution maps using randomized smoothing, enhancing their robustness and reliability.


<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods are non-robust to small input perturbations, which affects their trustworthiness and reliability in practical applications.

Method: The framework uses randomized smoothing to sparsify and smooth attribution maps, reformulating the task as a segmentation problem to certify pixel importance against l2-bounded perturbations.

Result: Extensive evaluation of 12 methods across 5 ImageNet models shows certified attributions are robust, interpretable, and faithful.

Conclusion: The proposed method provides rigorous robustness guarantees for attribution maps, making them reliable for downstream tasks.
...
TL;DR: This paper studies how zero-shot RL methods perform under partial observability and finds that memory-based approaches improve their effectiveness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of zero-shot RL in real-world scenarios where Markov states are only partially observable.

Method: The method involves analyzing the performance degradation of standard zero-shot RL methods under partial observability and evaluating memory-based architectures as a solution.

Result: The results show that memory-based zero-shot RL methods perform better than memory-free baselines in partially observable domains with changing dynamics.

Conclusion: The conclusion is that memory-based architectures are a viable and effective remedy for partial observability in zero-shot RL, similar to their role in single-task RL.
...
TL;DR: The paper introduces a new method to simplify and visualize warping paths in time series comparison, focusing on key transformations like shift, compression, and amplitude differences to improve interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a qualitative comparison of time series beyond traditional elastic distance measures, which lack insights into structural relationships and transformations between subsequences.

Method: The method involves a novel technique to simplify the warping path, highlighting and quantifying key transformations such as shift, compression, and amplitude differences.
...
Conclusion: The conclusion states that their method improves the understanding of time series transformations, making it easier to identify where and how one series shifts, speeds up, or slows down relative to another.
...
TL;DR: This paper extends Gold's inductive inference framework to include computational observations and restricted input sources, showing that learning general recursive functions can be achieved with time-bound or approximate observations, and connects policy trajectory learning to rational function inference, but proves that characteristic sets don't exist for linear-time computable functions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to study the learnability of general recursive functions under more realistic constraints by extending traditional inductive inference frameworks.

Method: The paper introduces Time-Bound Observations and Policy-Trajectory Observations, and develops a formal framework for computational agents, analyzing the learnability of functions with these observations.

Result: Traditional input-output observations are insufficient for learning general recursive functions in the limit, but adding computational complexity constraints or approximate time-bound observations allows overcoming this barrier. Also, policy trajectories relate to rational function inference, but characteristic sets cannot exist for linear-time computable functions.

Conclusion: The study demonstrates that incorporating computational observations and constraints enhances the learnability of recursive functions, reveals connections to finite-state transducers, and identifies limitations in characteristic set existence for linear-time functions.
...
TL;DR: DAILOC is a new domain-incremental learning framework that tackles both device and time-related changes in Wi-Fi fingerprinting for indoor location, using a multi-level variational autoencoder and memory-guided alignment to reduce errors significantly.


<details>
  <summary>Details</summary>
Motivation: Existing methods handle device heterogeneity and temporal variations separately, leading to poor generalization and forgetting issues over time.

Method: DAILOC uses a multi-level variational autoencoder to separate domain shifts from location features and a memory-guided class latent alignment to prevent catastrophic forgetting.

Result: Experiments show DAILOC reduces average error by 2.74x and worst-case error by 4.6x compared to current methods.

Conclusion: DAILOC effectively addresses domain shifts in Wi-Fi fingerprinting, improving localization accuracy across different devices and time periods.
...
### [241] [Identifying economic narratives in large text corpora -- An integrated approach using Large Language Models](https://arxiv.org/abs/2506.15041)
...
### [242] [Omnidirectional Video Super-Resolution using Deep Learning](https://arxiv.org/abs/2506.14803)
