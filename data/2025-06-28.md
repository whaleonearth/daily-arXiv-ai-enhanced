...
TL;DR: The paper introduces MIDAS, a data augmentation technique for dynamic facial expression recognition (DFER) that uses soft labels to improve performance on ambiguous in-the-wild data, showing better results than existing methods on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to accurately recognize ambiguous facial expressions in real-world scenarios, which is crucial for practical DFER applications.

Method: MIDAS is a data augmentation method that convexly combines video frames and their soft labels (probabilities of multiple emotion classes) to create augmented data, extending mixup to video data with soft labels.

Result: Models trained with MIDAS-augmented data outperform state-of-the-art methods trained on original datasets on DFEW and FERV39k-Plus.

Conclusion: MIDAS provides a simple and effective way to handle ambiguity in DFER through data augmentation with soft labels, improving performance on challenging in-the-wild data.
...
TL;DR: ThirdEye is a monocular depth estimation method that uses explicit cues like occlusion boundaries, shading, and perspective through a cue-aware pipeline with a three-stage cortical hierarchy and a key-value working-memory module to weight cues by reliability, requiring only modest fine-tuning due to frozen cue experts.


<details>
  <summary>Details</summary>
Motivation: Traditional monocular depth estimation methods train deep models to infer depth directly from RGB pixels, which may overlook explicit monocular cues that the human visual system uses, such as occlusion boundaries, shading, and perspective.

Method: ThirdEye uses a cue-aware pipeline that supplies explicit monocular cues through specialised, pre-trained, and frozen networks. These cues are fused in a three-stage cortical hierarchy (V1->V2->V3) with a key-value working-memory module that weights them by reliability. An adaptive-bins transformer head then generates a high-resolution disparity map.

Result: The extended version of ThirdEye provides additional architectural details, neuroscientific motivation, and an expanded experimental protocol, but quantitative results will be included in a future revision.

Conclusion: ThirdEye leverages external supervision by using frozen cue experts, allowing for efficient learning with minimal fine-tuning while incorporating explicit monocular cues inspired by the human visual system.
...
TL;DR: This paper explores the geometric basis of stereo vision, emphasizing 3D structure and human-like perception for better depth reconstruction. It introduces new geometric constraints for occlusions and depth changes, examines deep learning's feature matching, and highlights attention's role in 3D surface recovery. The study shows that merging geometric knowledge with learned features improves understanding of stereo systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance depth reconstruction in stereo vision systems by leveraging geometric foundations and human-inspired perception, addressing challenges like occlusions and depth discontinuities.

Method: The method involves revisiting the Cyclopean Eye model, proposing new geometric constraints, analyzing deep learning-based feature matching, and studying attention mechanisms' impact on 3D surface recovery through theoretical and empirical approaches.

Result: The results show that combining strong geometric priors with learned features offers internal abstractions that improve the understanding and accuracy of stereo vision systems.

Conclusion: The conclusion is that integrating geometric knowledge with learned features is crucial for advancing stereo vision systems, providing better depth reconstruction and 3D surface understanding.
...
TL;DR: This paper introduces a new dataset for aviation weather, addresses the lack of publicly available data for atmospheric visibility estimation, and presents benchmarks and a baseline for comparison with ASTM standards.


<details>
  <summary>Details</summary>
Motivation: To develop low-cost alternatives to traditional expensive weather sensors in aviation, specifically addressing the absence of suitable publicly available datasets for atmospheric visibility estimation.

Method: The paper describes the creation of a new dataset through a year-long data collection campaign using FAA weather camera images. It also presents benchmarks using three common approaches and a general-purpose baseline, evaluated on multiple datasets including their own and comparing against an ASTM standard.

Result: The introduction of a new dataset for aviation weather, along with benchmarks and a baseline that can be used for visibility estimation tasks, providing a reference point for future research and comparisons against ASTM standards.

Conclusion: The paper concludes that their dataset and benchmarks contribute to the field of aviation weather by offering a resource for supervised learning tasks in atmospheric visibility estimation, despite the previous lack of publicly available data.
...
TL;DR: This paper introduces a step-by-step video-to-audio generation method that creates individual audio tracks for each sound event in a video, inspired by Foley workflows and concept negation, achieving higher-quality composite audio without specialized datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to comprehensively capture all sound events in a video and improve composite audio synthesis quality by generating semantically distinct audio tracks for each event.

Method: The method involves a sequential generation of audio tracks, each conditioned on a text prompt and previously generated tracks, using a training framework that leverages pre-trained models without needing paired datasets.

Result: The results show that the method can generate multiple semantically distinct audio tracks from a single video, leading to better composite audio quality compared to existing methods.

Conclusion: The conclusion is that their step-by-step approach, inspired by Foley workflows and concept negation, effectively enhances audio synthesis by generating separate tracks for each sound event, without reliance on specialized data.
...
TL;DR: This paper introduces DBMovi-GS, a method for generating novel views from blurry monocular videos by using sparse-controlled Gaussian splatting to restore sharpness and reconstruct 3D geometry.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with dynamic scenes and blurry inputs due to reliance on high-res images and static assumptions.

Method: DBMovi-GS uses sparse-controlled Gaussian splatting to generate dense 3D Gaussians, restoring sharpness and reconstructing 3D geometry from blurry videos.

Result: The model performs robustly in novel view synthesis under dynamic blurry scenes and sets a new benchmark for realistic results.

Conclusion: DBMovi-GS addresses the challenges of dynamic view synthesis from blurry monocular videos, improving robustness and visual fidelity in real-world settings.
...
TL;DR: This paper introduces Inverse STR (ISTR) to detect and localize text removal in images, improving STR and addressing misuse risks.


<details>
  <summary>Details</summary>
Motivation: To address the increasing misuse risks of Scene Text Removal (STR) and enhance its reliability.

Method: Inverse STR (ISTR) analyzes STR-processed images through binary classification and localization of removed text regions, with text recovery using a trained text recognizer.

Result: Experiments show high accuracy in detecting and localizing text removal, enabling misuse detection and STR improvement.

Conclusion: ISTR is effective for analyzing STR-processed images, aiding in detecting misuse and enhancing STR techniques.
...
TL;DR: The paper introduces the Aging Multiverse framework, which generates multiple facial aging trajectories from a single image, considering external factors like environment, health, and lifestyle. It uses a training-free diffusion method with attention mixing and simulated aging regularization for better identity preservation and age accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to move beyond single deterministic aging paths and create diverse, controllable aging trajectories for applications in digital storytelling, health education, and personal visualization.

Method: A training-free diffusion-based method with attention mixing and Simulated Aging Regularization to balance identity preservation, age accuracy, and condition control.

Result: The framework shows state-of-the-art performance in identity preservation, aging realism, and conditional alignment, outperforming existing models that often struggle with these criteria.

Conclusion: The Aging Multiverse transforms aging into a multi-dimensional, controllable process, offering new possibilities for creative and practical uses in various fields.
...
TL;DR: The paper introduces a method for novel view synthesis using locally reconstructed light fields and error visualization, reducing the need for 3D annotations and allowing more flexible scene exploration.


<details>
  <summary>Details</summary>
Motivation: Existing AR approaches for novel view synthesis require 3D annotations and limit scene exploration, making them mentally demanding and restrictive.

Method: The authors propose using locally reconstructed light fields and visualizing errors to be removed by inserting new views, avoiding the need for 3D annotations.

Result: Error-peaking visualization is less invasive, reduces user disappointment, and works well with fewer view samples in a mobile system.

Conclusion: The approach can enhance radiance field reconstruction for larger scenes, like 3D Gaussian splatting, by enabling more flexible data collection.
...
TL;DR: This paper introduces FGS, a method to improve faithfulness in text-guided diffusion models without sacrificing editability.
...
Result: Experiments show FGS improves faithfulness while keeping editability, works with different editing methods, and enables precise image edits.

Conclusion: FGS is effective for high-quality image synthesis and editing, offering a balanced approach between editability and faithfulness.
...
TL;DR: CL-Splats efficiently updates 3D scene representations by detecting changes and optimizing locally, improving quality and enabling temporal analysis.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and accurate updating of 3D scenes in dynamic environments for robotics, mixed reality, and embodied AI applications.

Method: CL-Splats uses a change-detection module to segment dynamic and static parts of a scene, allowing local optimization and preserving previous states.

Result: CL-Splats provides efficient updates with better reconstruction quality than existing methods, supporting temporal segmentation and analysis.

Conclusion: CL-Splats offers a strong foundation for real-time 3D scene reconstruction adaptations.
...
TL;DR: This paper introduces a novel Graph-oriented Inverse Reinforcement Learning (GoIRL) framework for trajectory prediction in autonomous driving, which uses vectorized context representations and a hierarchical parameterized trajectory generator to achieve state-of-the-art performance and better generalization.


<details>
  <summary>Details</summary>
Motivation: Trajectory prediction for surrounding agents is challenging in autonomous driving due to uncertainty and multimodality, and existing data-driven methods primarily use supervised learning which may not capture these aspects effectively.

Method: The paper proposes a GoIRL framework that includes a feature adaptor to aggregate lane-graph features into grid space, integrates with maximum entropy IRL to infer reward distribution and obtain policy, and uses a hierarchical parameterized trajectory generator with refinement module and probability fusion strategy.

Result: The approach achieves state-of-the-art performance on Argoverse & nuScenes benchmarks and shows superior generalization compared to existing supervised models.

Conclusion: The GoIRL framework with vectorized context representations and hierarchical trajectory generation offers improved trajectory prediction in autonomous driving by addressing uncertainty and multimodality more effectively than supervised learning methods.
...
TL;DR: This paper introduces an OCR system for Greek polytonic texts using CNNs and RNNs, improving accuracy and efficiency, with open-source availability.


<details>
  <summary>Details</summary>
Motivation: Traditional OCR methods have limitations in accurately recognizing Greek polytonic scripts, which require a more effective solution for digitization.
...
Result: The proposed OCR system demonstrates significant improvements in accuracy and efficiency compared to traditional methods.

Conclusion: The researchers developed an effective OCR system for Greek polytonic texts, releasing the model as open-source for academic use.
...
TL;DR: The paper introduces HalluSegBench, a new benchmark for evaluating hallucinations in vision-language segmentation by using counterfactual visual reasoning, showing that vision-driven hallucinations are more common than label-driven ones.


<details>
  <summary>Details</summary>
Motivation: To address the issue of hallucinations in vision-language segmentation models, which often produce incorrect masks or labels, the authors aim to create a benchmark that can better diagnose grounding failures by manipulating visual context.

Method: The authors created HalluSegBench, a dataset with 1340 counterfactual instance pairs across 281 object classes, and introduced new metrics to measure hallucination sensitivity under visually coherent scene edits.

Result: Experiments with state-of-the-art models on HalluSegBench showed that vision-driven hallucinations are significantly more prevalent than label-driven ones, and models often fail to correct false segmentations.

Conclusion: The study emphasizes the importance of counterfactual reasoning for evaluating grounding fidelity in vision-language segmentation models, suggesting that current models need improvement in this area.
...
TL;DR: This paper introduces tree-based semantic loss functions for biomedical HSI segmentation, improving performance on sparse datasets with a hierarchical label structure and enabling OOD detection without harming ID performance.


<details>
  <summary>Details</summary>
Motivation: Current learning methods for biomedical segmentation treat all errors equally, ignoring inter-class semantics in the label space, which limits their effectiveness in distinguishing subtle tissue variations.
...
Result: The method achieves state-of-the-art performance on a sparsely annotated HSI dataset with 107 classes organised in a clinical semantic tree. It also effectively detects OOD pixels without degrading ID performance.

Conclusion: The paper concludes that using tree-based semantic losses with sparse annotations improves segmentation accuracy and OOD detection in hyperspectral imaging for surgical applications.
...
TL;DR: This paper proposes a deep-learning pipeline for automated myocardial scar detection and segmentation, using KL loss and data augmentation to handle label noise, data heterogeneity, and class imbalance, showing superior performance and generalizability compared to existing models.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of myocardial scars from cardiac MRI is crucial for clinical assessment and treatment planning, but existing methods face challenges with label noise, data heterogeneity, and class imbalance.

Method: The authors propose a deep-learning pipeline that fine-tunes state-of-the-art models, utilizing Kullback-Leibler loss and extensive data augmentation to address label noise, data heterogeneity, and class imbalance.

Result: The model demonstrates accurate and smooth segmentation of myocardial scars even with noisy labels, outperforming state-of-the-art models like nnU-Net and showing strong generalizability in out-of-distribution test sets.

Conclusion: The results establish a reliable foundation for automated myocardial scar quantification and support the broader clinical adoption of deep learning in cardiac imaging.
...
TL;DR: DiMPLe disentangles invariant and spurious features across vision and language modalities to improve OOD generalization and robustness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of spurious correlations in visual data that negatively impact out-of-distribution performance, by disentangling features across modalities.

Method: DiMPLe combines mutual information minimization between invariant and spurious features, spurious feature regularization, and contrastive learning on invariant features.

Result: Experiments show DiMPLe outperforms CoOp-OOD with 15.27 and 44.31 gains in base and novel class accuracy, respectively.

Conclusion: The conclusion is that DiMPLe enhances generalization to novel classes and robustness against distribution shifts by effectively disentangling features across modalities.
...
TL;DR: This paper proposes a novel Human Motion Segmentation approach called $	ext{TR}^2	ext{C}$ that jointly learns structured representations and affinity for better performance in complex video scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing subspace clustering methods for HMS assume that high-dimensional temporal data follows a Union-of-Subspaces (UoS) distribution, but this may not hold for videos with complex human motions and cluttered backgrounds.

Method: $	ext{TR}^2	ext{C}$, which jointly learns structured representations and affinity to segment frame sequences in video, maintaining temporally consistent representations that align with UoS structure.

Result: The method achieves state-of-the-art performance on five benchmark HMS datasets using different feature extractors.

Conclusion: The paper concludes that $	ext{TR}^2	ext{C}$ provides a more effective approach for HMS by addressing the limitations of traditional subspace clustering methods in complex video environments.
...
TL;DR: The paper introduces CoPa-SG, a synthetic scene graph dataset with precise ground truth and detailed relations, along with parametric and proto-relations to improve scene understanding and applications.


<details>
  <summary>Details</summary>
Motivation: Current 2D scene graph work lacks accurate data, hindering progress. The paper aims to address this by creating a high-quality dataset with exhaustive relations.

Method: The paper creates CoPa-SG, a synthetic dataset with precise ground truth. It introduces parametric relations (adding parameters like angles/distance) and proto-relations (hypothetical relations for new objects).

Result: CoPa-SG provides a dataset with accurate and detailed relations. The paper shows how parametric and proto-relations improve scene graph models and enhance planning/ reasoning in applications.

Conclusion: CoPa-SG and the new relation concepts address data limitations in scene graphs, offering better structural understanding and applicability. The paper suggests future work on integrating these relations into more applications.
...
TL;DR: The paper addresses issues in detection-free image-to-point cloud registration by proposing CAA and GOS modules, improving accuracy and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve registration accuracy by addressing feature channel attention differences and redundant correspondences in cross-modal matching.

Method: The paper proposes Channel Adaptive Adjustment Module (CAA) to enhance intra-modal features and suppress cross-modal sensitivity, and Global Optimal Selection Module (GOS) to replace local selection with global optimization.

Result: Experiments on RGB-D Scenes V2 and 7-Scenes show the method achieves state-of-the-art performance in image-to-point cloud registration.

Conclusion: The proposed CAA and GOS modules effectively address the issues of feature channel attention differences and redundant correspondences, leading to improved registration accuracy.
...
TL;DR: GenFlow is a modular framework that makes generative art accessible to all users by simplifying workflow creation with a node-based editor and NLP-powered assistant.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between generative art's potential and the technical expertise required, making advanced tools accessible to everyone.

Method: GenFlow uses a node-based editor for customization and an intelligent NLP assistant to automate deployment and reduce technical barriers.

Result: A user study showed GenFlow optimizes workflows, reduces task completion times, and improves user understanding with its intuitive interface and adaptive features.
...
TL;DR: The paper introduces XVerse, a new model for fine-grained control in multi-subject text-to-image generation, addressing issues like artifacts and attribute entanglement by using reference images as offsets for text-stream modulation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to achieve fine-grained control over subject identity and semantic attributes (like pose, style, lighting) in multi-subject image generation without causing artifacts or attribute entanglement, which are common issues in existing methods.
...
Result: The result is a high-fidelity, editable multi-subject image synthesis model with robust control over individual subject characteristics and semantic attributes, improving personalized and complex scene generation.

Conclusion: The conclusion states that XVerse advances personalized and complex scene generation capabilities by effectively solving the problems of artifacts and attribute entanglement in multi-subject controlled generation.
...
TL;DR: This paper introduces Radial Cross-Modal Embeddings (RCME) to model transitive entailment in vision-language models, improving hierarchical tasks like species classification and retrieval.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of learning hierarchical structures in vision-language models by explicitly modeling transitive entailment, which previous methods failed to do.

Method: RCME is a framework that optimizes for the partial order of concepts, enabling explicit modeling of transitivity-enforced entailment in vision-language models.

Result: Experiments show improved performance in hierarchical species classification and retrieval tasks compared to existing state-of-the-art models.

Conclusion: The authors conclude that their RCME framework effectively enhances vision-language models' ability to represent hierarchical structures, with open-source code and models available.
...
TL;DR: The paper introduces WAFT, a new optical flow method that replaces cost volume with high-resolution warping, offering better accuracy and lower memory usage, and outperforms existing methods in speed and benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to challenge the conventional approach of using cost volumes in optical flow methods, seeking a more efficient and effective solution.

Method: WAFT uses high-resolution warping instead of cost volume, employing a simple and flexible meta-architecture with minimal inductive biases and custom designs.

Result: WAFT ranks 1st on Spring and KITTI benchmarks, achieves the best zero-shot generalization on KITTI, and is up to 4.1x faster than similar-performing methods.

Conclusion: WAFT provides a novel approach to optical flow by replacing cost volume with high-resolution warping, demonstrating superior performance and efficiency.
...
TL;DR: This paper addresses challenges in cross-modal image-text retrieval by proposing Maximal Pair Assignment Similarity and two loss functions to improve set-based representations.


<details>
  <summary>Details</summary>
Motivation: Traditional methods using single-vector embeddings fail to capture nuanced cross-modal relationships, while set-based approaches, though promising, have issues like sparse supervision and set collapse.

Method: Proposed Maximal Pair Assignment Similarity for one-to-one matching in embedding sets, along with Global Discriminative Loss and Intra-Set Divergence Loss to enhance representations.

Result: Achieved state-of-the-art performance on MS-COCO and Flickr30k datasets without external data.

Conclusion: Set-based representations can be effective if optimized with the proposed methods to preserve semantic diversity and prevent collapse.
...
TL;DR: This paper introduces PEVA, a model that predicts ego-centric video from human actions using a diffusion transformer and pose trajectories, evaluated on the Nymeria dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to model complex real-world environments and embodied agent behaviors through video prediction from a human's first-person perspective.

Method: The method involves training an auto-regressive conditional diffusion transformer on the Nymeria dataset, using kinematic pose trajectories structured by the body's joint hierarchy.

Result: The model successfully simulates how physical human actions shape the environment from a first-person view, with a hierarchical evaluation protocol showing its embodied prediction and control abilities.

Conclusion: The work presents an initial attempt at addressing the challenges of modeling real-world environments and embodied behaviors through ego-centric video prediction.
...
TL;DR: The paper discusses the importance of building a trusted ecosystem for AI, highlighting the need for safety, reliability, and security. It references the 2025 Singapore Conference on AI (SCAI) which aimed to identify research priorities in AI safety, building upon the International AI Safety Report led by Yoshua Bengio and supported by 33 governments. The report categorizes AI safety research into three areas: Development, Assessment, and Control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure AI is safe, trustworthy, reliable, and secure, thereby building a trusted ecosystem that fosters innovation without backlash.

Method: The method involves bringing together AI scientists globally to identify and synthesise research priorities in AI safety, using a defence-in-depth model to categorize research domains into three types: Development, Assessment, and Control.

Result: The resulting report from the SCAI conference builds on the International AI Safety Report by organizing AI safety research into three categories: Development, Assessment, and Control.

Conclusion: The conclusion emphasizes the importance of establishing a trusted ecosystem for AI, which requires research in Development, Assessment, and Control to ensure safe and reliable AI systems while enabling innovation.
...
TL;DR: This paper introduces a dynamic context-aware prompt recommendation system for domain-specific AI applications, combining contextual analysis, retrieval-augmented knowledge grounding, hierarchical skill organization, and adaptive ranking to generate effective prompts.


<details>
  <summary>Details</summary>
Motivation: LLM applications are sensitive to prompt quality, which is challenging to achieve, especially in domain-specific contexts. There's a need for a system that can dynamically recommend relevant prompts.

Method: The system uses behavioral telemetry and a two-stage hierarchical reasoning process to select and rank skills. It generates prompts with predefined and adaptive templates using few-shot learning.
...
Conclusion: The proposed system effectively addresses prompt quality challenges in domain-specific AI applications through dynamic context-aware recommendations.
...
TL;DR: The paper proposes a framework to analyze the macroscopic societal impact of language model-based agents and introduces a dataset of 100 indirect harm scenarios, showing significant improvements in safety benchmarks.


<details>
  <summary>Details</summary>
Motivation: As language models influence high-stakes decisions, ensuring their beneficial impact requires understanding the implications of their suggestions.

Method: A proof-of-concept framework for projecting how model-generated advice propagates through societal systems over time, along with a dataset of 100 indirect harm scenarios.

Result: Over 20% improvement on the new dataset and an average win rate over 70% against strong baselines on existing safety benchmarks.

Conclusion: The approach suggests a promising direction for developing safer language model-based agents.
...
TL;DR: The paper introduces IXAII, an interactive explainable AI system that combines LIME, SHAP, Anchors, and DiCE to improve transparency and user agency.


<details>
  <summary>Details</summary>
Motivation: Most post-hoc explainable AI methods are static and don't consider the user perspective, making them less effective.

Method: Developed IXAII, an interactive system offering explanations from four XAI methods with tailored views for five user groups.

Result: Evaluation via interviews showed IXAII is perceived as helpful for increasing transparency with multiple visualization options.

Conclusion: IXAII bridges gaps between XAI methods, interactivity, and implementation, offering a novel approach to AI explanations and human-AI interaction.
...
TL;DR: This paper studies how LLMs handle morally tricky situations with violence, using a special test called VBVQ. They found that LLMs might say one thing but prefer another, and their violent responses change based on race, age, and place, which is different from what real human studies show.


<details>
  <summary>Details</summary>
Motivation: The motivation is to check if LLMs can handle moral questions about violence, since they're used a lot for finding violent content online, but we don't know how they deal with tricky, real-life situations.

Method: They used a test called VBVQ, which is used for humans, to check LLMs. They also tried different identities like race, age, and where you're from in the US to see if the LLMs have any bias.

Result: The results showed that LLMs sometimes say one thing but act differently. Also, their violent answers change depending on the identity they're using, which isn't what real humans usually do.

Conclusion: The conclusion is that LLMs might not be good enough for real-world violence problems because they don't act like humans in tricky situations, and their answers change based on fake identities, which could be a problem for fairness and accuracy.
...
TL;DR: The paper proposes DALR, a method for improving multimodal sentence representations by addressing cross-modal misalignment and intra-modal semantic divergence through consistency learning and ranking distillation.


<details>
  <summary>Details</summary>
Motivation: Previous methods focus on coarse-level alignment between images and text, leading to cross-modal misalignment bias and intra-modal semantic divergence, which harm sentence representation quality.

Method: DALR uses a consistency learning module to soften negative samples and leverage semantic similarity from an auxiliary task for fine-grained cross-modal alignment, and integrates ranking distillation with global intra-modal alignment learning to capture complex sentence relationships.

Result: Experiments on STS and TR tasks show that DALR outperforms state-of-the-art baselines, demonstrating its effectiveness in improving sentence representation quality.

Conclusion: The paper concludes that DALR effectively addresses the challenges of cross-modal misalignment and intra-modal semantic divergence, leading to better multimodal sentence representations.
...
TL;DR: ComRAG improves real-time industrial CQA by integrating static and dynamic knowledge with a centroid-based memory mechanism, showing significant performance gains and efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to effectively leverage historical interactions and domain knowledge in real-time for CQA platforms, addressing limitations in existing methods such as underutilization of external knowledge, lack of dynamic context incorporation, and unsuitable memory mechanisms for industrial use.

Method: ComRAG uses a centroid-based memory mechanism to integrate static knowledge with dynamic historical QA pairs, enhancing retrieval, generation, and storage efficiency.

Result: ComRAG outperforms baselines in three industrial datasets, with 25.9% improvement in vector similarity, 8.7-23.3% latency reduction, and 20.23% to 2.06% chunk growth reduction over iterations.

Conclusion: ComRAG effectively addresses challenges in real-time industrial CQA by combining static and dynamic knowledge, demonstrating superior performance and efficiency, making it suitable for industrial deployment.
...
TL;DR: Progtuning is a new method for fine-tuning Transformer models that reduces the number of parameters updated by 25% while maintaining performance and adapting well to different scenarios.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency of fine-tuning large Transformer models by optimizing resource allocation, as existing methods update the same number of parameters regardless of their contribution.

Method: Progtuning combines progressive learning with parameter-efficient fine-tuning, progressively reducing the number of updated transformer blocks based on their contribution.

Result: Progtuning reduces the number of updated parameters by about 25% and maintains competitive performance, showing high adaptability across various adaptation scenarios.

Conclusion: Progtuning offers an efficient and adaptable approach to fine-tuning Transformer models, optimizing resource allocation by focusing on the most contributing blocks.
...
TL;DR: This paper discusses the engineering practices that ensure the MTEB benchmark's reproducibility, extensibility, and scalability, including CI pipelines, design choices, and community contribution strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to maintain and improve MTEB's reproducibility and usability as it scales, ensuring it remains relevant and reliable for machine learning evaluation.

Method: The method involves creating robust CI pipelines for validating dataset integrity, automating tests, and assessing generalizability, along with design choices and strategies for community contributions and adding new tasks/datasets.

Result: The result is a more comprehensive and scalable MTEB benchmark that maintains quality and relevance, with proven engineering practices that benefit other benchmark maintainers.

Conclusion: The conclusion is that the engineering practices developed for MTEB provide valuable insights for maintaining reproducibility and usability in machine learning evaluation benchmarks, emphasizing the importance of CI, design, and community involvement.
...
TL;DR: This paper introduces a dynamic turn-taking prediction model controlled by textual prompts, enhancing accuracy and adaptability in conversational systems.


<details>
  <summary>Details</summary>
Motivation: To enable intuitive control over turn-taking in dialogue systems using textual instructions like 'faster' or 'calmer', adapting to conversational contexts.

Method: Building on a transformer-based VAP model, integrating textual prompt embeddings into channel-wise and cross-channel transformers, using LLMs to generate synthetic prompts due to data absence.

Result: Improved prediction accuracy and effective variation of turn-taking timing based on textual prompts.

Conclusion: The proposed model offers a feasible approach for dynamic turn-taking control, enhancing adaptability and user interaction in conversational agents.
...
TL;DR: The paper proposes a retrieval-based prompting strategy for Automatic Term Extraction (ATE) using syntactic similarity instead of semantic similarity, showing improved F1-scores in in-domain and cross-domain settings.


<details>
  <summary>Details</summary>
Motivation: Despite advancements in NLP tasks by large language models (LLMs), their potential for Automatic Term Extraction (ATE) remains underexplored. ATE is vital for tasks like machine translation and information retrieval, so improving it with LLMs is important.

Method: The method involves a retrieval-based prompting strategy in a few-shot setting, where demonstrations are selected based on syntactic similarity rather than semantic similarity. This approach is domain-agnostic and aims to provide more reliable term boundary guidance.

Result: Experiments on three specialized ATE benchmarks demonstrate that syntactic retrieval improves F1-score compared to methods using semantic similarity. The approach shows effectiveness in both in-domain and cross-domain scenarios.

Conclusion: The findings emphasize the significance of syntactic cues in adapting LLMs for terminology-extraction tasks, suggesting that syntactic similarity can lead to better performance in ATE.
...
TL;DR: This paper investigates using a text-only language model to extract referring expressions from visually grounded dialogue, finding it effective despite limitations in capturing visual context.


<details>
  <summary>Details</summary>
Motivation: The motivation is to determine how much linguistic context alone can help detect mentions with visual referents in conversations.

Method: Adapting a pretrained LLM for coarse-grained annotation of mention spans through next-token prediction to demarcate boundaries in text.

Result: A text-only approach with a moderate LLM, small datasets, and efficient fine-tuning was effective, showing the importance of linguistic context.
...
TL;DR: This paper introduces GLASS, a framework using Greimas Semiotic Square for literary analysis, with a dataset and metrics to improve LLMs' ability to critique complex texts.


<details>
  <summary>Details</summary>
Motivation: LLMs lack proficiency in providing deep literary criticism for complex works, prompting the need for a structured analytical framework.

Method: GLASS leverages the Greimas Semiotic Square to dissect narrative structures and meanings, introduces a dataset of 48 works, and uses LLM-as-a-judge for quantitative metrics.

Result: GLASS demonstrates high performance compared to expert criticism and LLMs, and generates original analyses for 39 classic works.

Conclusion: GLASS offers an AI tool for literary research and education, enhancing understanding of literary engagement's cognitive mechanisms.
...
TL;DR: The paper studies how well reinforcement learning works for fine-tuning large language models in different regimes (offline, semi-online, online) and finds that online methods perform better than offline ones, with similar performance between semi-online and online.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the effectiveness of reinforcement learning methods when moving from offline to semi-online and fully online settings for both verifiable and non-verifiable tasks.

Method: The method involves comparing online and semi-online Direct Preference Optimization and Group Reward Policy Optimization objectives through experiments on verifiable math tasks and non-verifiable instruction following tasks.

Result: The results show that online and semi-online methods have similar performance and convergence, both outperforming offline methods. Multi-tasking with both reward types improves performance across the board.

Conclusion: The conclusion is that online and semi-online reinforcement learning methods are effective for fine-tuning large language models, and combining verifiable and non-verifiable rewards enhances performance in both task types.
...
TL;DR: This paper introduces skLEP, a comprehensive benchmark for evaluating Slovak NLU models, including original datasets, translations of English resources, and tools for model evaluation and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a benchmark for Slovak NLU models to facilitate research and improve performance on various tasks.

Method: The method involves creating skLEP by curating original datasets and translating existing English NLU resources, then evaluating different language models on these tasks.

Result: The result is the skLEP benchmark, along with an open-source toolkit and leaderboard for model evaluation and fine-tuning.

Conclusion: The conclusion is that skLEP provides a thorough assessment of model capabilities and aims to foster reproducibility and future research in Slovak NLU.
...
### [165] [Progressive Size-Adaptive Federated Learning: A Comprehensive Framework for Heterogeneous Multi-Modal Data Systems](https://arxiv.org/abs/2506.20685)
...
### [166] [Diffusion Tree Sampling: Scalable inference-time alignment of diffusion models](https://arxiv.org/abs/2506.20701)
...
### [167] [On Convolutions, Intrinsic Dimension, and Diffusion Models](https://arxiv.org/abs/2506.20705)
...
### [168] [Test-time Scaling Techniques in Theoretical Physics -- A Comparison of Methods on the TPBench Dataset](https://arxiv.org/abs/2506.20729)
...
### [169] [Stochastic Parameter Decomposition](https://arxiv.org/abs/2506.20790)
...
### [170] [GPU Kernel Scientist: An LLM-Driven Framework for Iterative Kernel Optimization](https://arxiv.org/abs/2506.20807)
...
### [171] [FINN-GL: Generalized Mixed-Precision Extensions for FPGA-Accelerated LSTMs](https://arxiv.org/abs/2506.20810)
...
### [172] [Leaner Training, Lower Leakage: Revisiting Memorization in LLM Fine-Tuning with LoRA](https://arxiv.org/abs/2506.20856)
*Fei Wang,Baochun Li*

Main category: cs.LG

TL;DR: This paper studies memorization in large language models during fine-tuning, especially with LoRA, and finds that it reduces data extraction risks without harming performance.


<details>
  <summary>Details</summary>
Motivation: To understand how memorization in fine-tuning affects data extraction attacks, particularly for LoRA, a parameter-efficient method.

Method: Re-examining memorization in fine-tuning and comparing LoRA with full fine-tuning using a relaxed similarity-based metric.

Result: LoRA reduces memorization risks compared to full fine-tuning while maintaining good task performance.

Conclusion: LoRA is a safer fine-tuning method for reducing memorization risks without sacrificing performance.
...
### [173] [Omniwise: Predicting GPU Kernels Performance with LLMs](https://arxiv.org/abs/2506.20886)
...
### [174] [SharpZO: Hybrid Sharpness-Aware Vision Language Model Prompt Tuning via Forward-Only Passes](https://arxiv.org/abs/2506.20990)
...
### [175] [Enhancing LLM Tool Use with High-quality Instruction Data from Knowledge Graph](https://arxiv.org/abs/2506.21071)
*Jingwei Wang,Zai Zhang,Hao Qian,Chunjing Gan,Binbin Hu,Ziqi Liu,Zhiqiang Zhang,Jun Zhou,Bin Shi,Bo Dong*

Main category: cs.LG

TL;DR: This paper proposes using knowledge graphs to generate high-quality instruction data for teaching LLMs to use tools, showing that fine-tuning on a small sample of this data improves tool utilization and overall capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve LLMs' problem-solving abilities and expand their applications by effectively teaching them to use tools, addressing the challenge of insufficient quality in previous instruction data generation methods.

Method: The method involves extracting query pathways from a knowledge graph, transforming them into user queries, translating entity relationships into tools, and parsing queries into solution steps to create high-quality instruction data.

Result: Experiments demonstrate that fine-tuning with a small sample of the synthetic data significantly enhances LLMs' tool utilization and overall capabilities.

Conclusion: The conclusion is that using knowledge graphs for instruction data generation is effective, leading to better tool usage and improved performance of LLMs with minimal data.
...
### [176] [Learning to Skip the Middle Layers of Transformers](https://arxiv.org/abs/2506.21103)
...
### [181] [Antibody Design and Optimization with Multi-scale Equivariant Graph Diffusion Models for Accurate Complex Antigen Binding](https://arxiv.org/abs/2506.20957)
...
TL;DR: The paper introduces Latent Prototype Routing (LPR), a new method to address load imbalance in Mixture-of-Experts (MoE) systems by using clustering, which significantly improves expert utilization without affecting performance.


<details>
  <summary>Details</summary>
Motivation: Current MoE systems have severe load imbalance, with only a few experts being activated, leading to underutilization of model capacity and resources.

Method: The authors propose LPR, a routing framework that uses clustering to generalize existing approaches and promote balanced expert utilization.

Result: Experiments show LPR reduces the Gini coefficient of expert load from 0.70 to 0.035 and improves the min-max expert load ratio from 1e-6 to 0.70, achieving near-perfect load balancing.

Conclusion: LPR effectively balances expert utilization in MoE architectures, enhancing resource efficiency without compromising performance.
...
### [184] [Strict Subgoal Execution: Reliable Long-Horizon Planning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.21039)
*Jaebak Hwang,Sanghyeon Lee,Jeongmo Kim,Seungyul Han*

Main category: cs.LG

TL;DR: SSE is a new framework for long-horizon goal-conditioned RL that uses a graph-based approach to enforce subgoal reachability and improve planning efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of long-horizon goal-conditioned tasks in RL, where goals are distant and rewards are sparse, leading to issues with subgoal infeasibility and inefficient planning.

Method: SSE is a graph-based hierarchical RL framework that enforces single-step subgoal reachability through structural constraints on high-level decisions. It uses a decoupled exploration policy to explore underexplored goal regions and a failure-aware path refinement method that adjusts edge costs based on low-level success rates.

Result: SSE outperforms existing methods in long-horizon benchmarks in terms of efficiency and success rate.

Conclusion: The conclusion is that SSE provides an effective solution for long-horizon goal-conditioned RL tasks by improving subgoal reliability and planning efficiency.
...
### [185] [Efficient Skill Discovery via Regret-Aware Optimization](https://arxiv.org/abs/2506.21044)
...
### [186] [FeDa4Fair: Client-Level Federated Datasets for Fairness Evaluation](https://arxiv.org/abs/2506.21095)
...
### [187] [Interpretable Hierarchical Concept Reasoning through Attention-Guided Graph Learning](https://arxiv.org/abs/2506.21102)
...
### [188] [Robust Policy Switching for Antifragile Reinforcement Learning for UAV Deconfliction in Adversarial Environments](https://arxiv.org/abs/2506.21127)
...
### [189] [Curriculum-Guided Antifragile Reinforcement Learning for Secure UAV Deconfliction under Observation-Space Attacks](https://arxiv.org/abs/2506.21129)
...
### [190] [DBConformer: Dual-Branch Convolutional Transformer for EEG Decoding](https://arxiv.org/abs/2506.21140)
...
### [191] [Linearity-based neural network compression](https://arxiv.org/abs/2506.21146)
*Silas Dobler,Florian Lemmerich*

Main category: cs.LG

TL;DR: This paper introduces linearity-based compression for neural networks, achieving lossless compression to 1/4 the original size and showing compatibility with existing methods.


<details>
  <summary>Details</summary>
Motivation: To augment highly optimized neural network compression solutions by introducing a novel method based on linearity of neurons with ReLU-like activations, allowing for merging of layers.

Method: Proposing linearity-based compression by leveraging the linear behavior of neurons that are almost always activated, merging subsequent layers to reduce weights.

Result: Achieved lossless compression down to 1/4 of the original model size in most tested models, with minimal interference when applied to pruned models.

Conclusion: The work establishes a foundation for a new compression method that enables smaller, more efficient neural network models.
...
### [192] [rQdia: Regularizing Q-Value Distributions With Image Augmentation](https://arxiv.org/abs/2506.21367)
*Sam Lerman,Jing Bi*

Main category: cs.LG

TL;DR: rQdia improves sample efficiency and long-term training in deep reinforcement learning by regularizing Q-value distributions with augmented images.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance the performance of model-free continuous control in pixel-based environments by addressing the limitations of state encoding baselines.
...
Result: rQdia boosts DrQ and SAC on 9/12 and 10/12 tasks in MuJoCo and achieves 18/26 tasks on Atari environments with Data-Efficient Rainbow.

Conclusion: The addition of rQdia significantly improves model-free continuous control from pixels, surpassing state encoding baselines and enhancing sample efficiency and long-term training outcomes.
...
### [193] [Pay Attention to Small Weights](https://arxiv.org/abs/2506.21374)
...
<details>
  <summary>Details</summary>
Motivation: To address the resource-intensive nature of finetuning large neural networks by restricting training to a subset of parameters, leveraging the observed correlation between large gradients and small weights during finetuning.

Method: NANOADAM dynamically selects and updates small-magnitude weights without requiring gradient computation, preserving large-magnitude weights to prevent catastrophic forgetting and allowing higher learning rates.

Result: Experiments show that NANOADAM improves generalization performance and allows larger learning rates compared to traditional finetuning methods, applicable to both NLP and vision tasks.

Conclusion: NANOADAM provides an efficient finetuning approach by focusing on small-magnitude weights, reducing resource demands while maintaining or enhancing model performance across different tasks.
...
### [194] [Temporal-Aware Graph Attention Network for Cryptocurrency Transaction Fraud Detection](https://arxiv.org/abs/2506.21382)
...
### [195] [Optimising 4th-Order Runge-Kutta Methods: A Dynamic Heuristic Approach for Efficiency and Low Storage](https://arxiv.org/abs/2506.21465)
...
### [196] [Process mining-driven modeling and simulation to enhance fault diagnosis in cyber-physical systems](https://arxiv.org/abs/2506.21502)
...
### [197] [mTSBench: Benchmarking Multivariate Time Series Anomaly Detection and Model Selection at Scale](https://arxiv.org/abs/2506.21550)
...
### [198] [Universal and Efficient Detection of Adversarial Data through Nonuniform Impact on Network Layers](https://arxiv.org/abs/2506.20816)
...
### [200] [Personalized Federated Learning via Dual-Prompt Optimization and Cross Fusion](https://arxiv.org/abs/2506.21144)
...
### [201] [Exploring the Effects of Chatbot Anthropomorphism and Human Empathy on Human Prosocial Behavior Toward Chatbots](https://arxiv.org/abs/2506.20748)
...
### [202] [A Systematic Review of Human-AI Co-Creativity](https://arxiv.org/abs/2506.21333)
...
### [203] [Multimodal LLMs for Visualization Reconstruction and Understanding](https://arxiv.org/abs/2506.21319)
*Can Liu,Chunlin Da,Xiaoxiao Long,Yuxiao Yang,Yu Zhang,Yong Wang*

Main category: cs.HC

TL;DR: The paper introduces a new dataset and trains a multimodal LLM for better visualization understanding by combining chart images with vector representations, leading to improved data extraction and chart quality.


<details>
  <summary>Details</summary>
Motivation: Visualizations are important for data communication, but current models can't effectively understand the data-to-visual mapping and structured info in charts.

Method: The paper proposes a dataset that combines chart images with vectorized representations, encoding schemes, and data features to train a specialized multimodal LLM.

Result: Experiments show significant improvements in data extraction accuracy and chart reconstruction quality using the proposed vector format.

Conclusion: The study demonstrates that training multimodal LLMs on the new dataset with vector formats enhances their ability to understand visualizations.
...
### [204] [Hybrid Deep Learning and Signal Processing for Arabic Dialect Recognition in Low-Resource Settings](https://arxiv.org/abs/2506.21386)
...
### [205] [ThinkSound: Chain-of-Thought Reasoning in Multimodal Large Language Models for Audio Generation and Editing](https://arxiv.org/abs/2506.21448)
...
### [206] [The final solution of the Hitchhiker's problem #5](https://arxiv.org/abs/2506.20672)
*Matjaž Omladič,Martin Vuk,Aljaž Zalar*

Main category: stat.ML

TL;DR: This paper provides a complete answer to Open Problem 5 from the 'Hitchhiker's Guide' on quasi-copulas by using an analytical approach, building on previous work that solved the problem up to dimension 17 with linear programming and disproved a conjecture.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the open problem regarding the extreme values of the mass distribution associated with multivariate quasi-copulas, which was highlighted in the 'Hitchhiker's Guide' survey, despite the lack of statistical interpretation of quasi-copulas.

Method: The paper uses an analytical approach to solve the original question about extreme values of the mass distribution for multivariate quasi-copulas, following previous work that employed a linear programming method to solve up to dimension 17 and disprove a conjecture.

Result: The paper presents a complete solution to Open Problem 5 from the 'Hitchhiker's Guide' on quasi-copulas, offering a detailed analysis of the extreme values of the mass distribution associated with multivariate quasi-copulas.

Conclusion: The conclusion is that the analytical approach provides a comprehensive answer to the open problem, advancing the understanding of quasi-copulas in dependence modeling, even though their statistical interpretation remains limited.
...
### [207] [Lower Bounds on the Size of Markov Equivalence Classes](https://arxiv.org/abs/2506.20933)
*Erik Jahn,Frederick Eberhardt,Leonard J. Schulman*

Main category: stat.ML

TL;DR: The paper shows that Markov equivalence classes can become exponentially large when relaxing the assumptions of acyclicity, causal sufficiency, or uniform model prior, which affects the learnability of causal graphs from observational data.


<details>
  <summary>Details</summary>
Motivation: To understand the limitations of causal discovery algorithms in learning the true causal graph from observational data when standard assumptions are not met.

Method: Theoretical analysis and proofs of lower bounds for the expected size of Markov equivalence classes under different graph settings.

Result: Exponentially large lower bounds for the size of Markov equivalence classes in sparse random DAGs, random acyclic mixed graphs, and random cyclic directed graphs when assumptions are relaxed.

Conclusion: Relaxing the standard assumptions in causal discovery leads to significantly larger Markov equivalence classes, implying greater uncertainty in causal inference from observational data.
...
### [208] [Hyperspherical Variational Autoencoders Using Efficient Spherical Cauchy Distribution](https://arxiv.org/abs/2506.21278)
...
### [209] [Wild refitting for black box prediction](https://arxiv.org/abs/2506.21460)
*Martin J. Wainwright*
...
### [210] [Transformer-Based Spatial-Temporal Counterfactual Outcomes Estimation](https://arxiv.org/abs/2506.21154)
*He Li,Haoang Chi,Mingyu Liu,Wanrong Huang,Liyang Xu,Wenjing Yang*

Main category: stat.ME

TL;DR: The paper introduces a Transformer-based framework for spatial-temporal counterfactual outcome estimation, showing better performance than classical models and validating its effectiveness through simulations and real-world data on conflict's impact on forest loss in Colombia.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of classical statistical models in estimating counterfactual outcomes with spatial-temporal attributes, which are crucial for real-world applications.

Method: Proposes a novel framework using Transformer models to estimate counterfactual outcomes with spatial-temporal attributes, ensuring consistency and asymptotic normality under mild assumptions.

Result: Simulation experiments demonstrate superior estimation capability compared to baseline methods, and real data experiments reveal the causal effect of conflicts on forest loss in Colombia.

Conclusion: The proposed Transformer-based framework effectively improves the estimation of spatial-temporal counterfactual outcomes, offering better performance and generalization than traditional models.
...
### [211] [IMC-PINN-FE: A Physics-Informed Neural Network for Patient-Specific Left Ventricular Finite Element Modeling with Image Motion Consistency and Biomechanical Parameter Estimation](https://arxiv.org/abs/2506.20696)
...
### [212] [Automatic Reviewers Assignment to a Research Paper Based on Allied References and Publications Weight](https://arxiv.org/abs/2506.21331)
...
### [213] [Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis](https://arxiv.org/abs/2506.20806)
...
### [214] [ZKPROV: A Zero-Knowledge Approach to Dataset Provenance for Large Language Models](https://arxiv.org/abs/2506.20915)
...
### [215] [PhishKey: A Novel Centroid-Based Approach for Enhanced Phishing Detection Using Adaptive HTML Component Extraction](https://arxiv.org/abs/2506.21106)
*Felipe Castaño,Eduardo Fidalgo,Enrique Alegre,Rocio Alaiz-Rodríguez,Raul Orduna,Francesco Zola*

Main category: cs.CR

TL;DR: PhishKey is a new phishing detection method that uses automatic feature extraction from URLs and HTML content, combining CNN and CAPE for accurate classification with strong resistance to attacks.


<details>
  <summary>Details</summary>
Motivation: Phishing attacks are a major cybersecurity threat that quickly adapt to evade detection and take advantage of human weaknesses. Current methods may not be effective enough against these evolving threats.

Method: PhishKey uses a combination of character-level processing with CNNs for URL classification and a Centroid-Based Key Component Phishing Extractor (CAPE) for HTML content at the word level. The results from both are combined with a soft-voting ensemble.

Result: PhishKey achieved up to 98.70% F1 Score on four datasets and showed strong resistance to adversarial attacks with little performance loss.

Conclusion: PhishKey is effective for detecting phishing attacks, offering accurate and reliable results through its hybrid approach and ensemble method.
...
### [216] [ClusterRCA: Network Failure Diagnosis in HPC Systems Using Multimodal Data](https://arxiv.org/abs/2506.20673)
*Yongqian Sun,Xijie Pan,Xiao Xiong,Lei Tao,Jiaju Wang,Shenglin Zhang,Yuan Yuan,Yuqi Li,Kunlin Jian*

Main category: cs.DC

TL;DR: This paper introduces ClusterRCA, a framework for network failure diagnosis in HPC systems using multimodal data and combining classifier-based and graph-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for network failure diagnosis are not accurate enough for HPC systems due to data heterogeneity.

Method: ClusterRCA uses multimodal data from NIC pairs, combines classifier-based and graph-based approaches, and performs a customized random walk on a failure graph constructed from the state classifier's output.

Result: ClusterRCA achieves high accuracy in diagnosing network failures for HPC systems and maintains robust performance across different application scenarios.

Conclusion: ClusterRCA is effective for network failure diagnosis in HPC systems, leveraging multimodal data and hybrid approaches for accurate and robust results.
...
### [217] [Utility-Driven Speculative Decoding for Mixture-of-Experts](https://arxiv.org/abs/2506.20675)
...
### [218] [Real-time and personalized product recommendations for large e-commerce platforms](https://arxiv.org/abs/2506.21368)
*Matteo Tolloso,Davide Bacciu,Shahab Mokarizadeh,Marco Varesi*

Main category: cs.IR

TL;DR: The paper introduces a real-time, personalized product recommendation system for e-commerce, using Graph Neural Networks and parsimonious learning to ensure accuracy, scalability, and minimal response time.


<details>
  <summary>Details</summary>
Motivation: To address the need for accurate, scalable, and fast personalized recommendations in large e-commerce platforms, particularly in fashion retail, where user satisfaction is critical.

Method: The methodology combines Graph Neural Networks (GNNs) with parsimonious learning to forecast purchase sequences and manage multi-interaction scenarios efficiently.

Result: The approach was tested on large e-commerce datasets and showed effectiveness in providing efficient personalized recommendations under real-world constraints.

Conclusion: The proposed system effectively meets the challenges of real-time and personalized product recommendations in e-commerce, enhancing user satisfaction through accurate and scalable solutions.
...
### [219] [Central limit theory for Peaks-over-Threshold partial sums of long memory linear time series](https://arxiv.org/abs/2506.20789)
...
### [220] [DRAGON: Distributional Rewards Optimize Diffusion Generative Models](https://arxiv.org/abs/2504.15217)
...
### [221] [Exploring Adapter Design Tradeoffs for Low Resource Music Generation](https://arxiv.org/abs/2506.21298)
...
### [222] [A Hierarchical Deep Learning Approach for Minority Instrument Detection](https://arxiv.org/abs/2506.21167)
*Dylan Sechet,Francesca Bugiotti,Matthieu Kowalski,Edouard d'Hérouville,Filip Langiewicz*

Main category: cs.SD

TL;DR: This paper focuses on improving musical instrument recognition in audio excerpts using hierarchical classification, particularly for instruments with limited data, and evaluates their approach on the MedleyDB dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance music information retrieval by accurately identifying instrument activities, which is crucial for music cataloging and discovery, especially when dealing with instruments that have limited data availability.

Method: The method involves using hierarchical classification based on the Hornbostel-Sachs system and integrating hierarchical structures into models to test a new class of models for hierarchical music prediction.
...
Conclusion: The research suggests that their hierarchical approach has potential for further advancements in musical instrument recognition within orchestral music, even with limited fine-grained annotations.
...
### [223] [Integrating Vehicle Acoustic Data for Enhanced Urban Traffic Management: A Study on Speed Classification in Suzhou](https://arxiv.org/abs/2506.21269)
...
### [224] [SmoothSinger: A Conditional Diffusion Model for Singing Voice Synthesis with Multi-Resolution Architecture](https://arxiv.org/abs/2506.21478)
...
### [225] [CovDocker: Benchmarking Covalent Drug Design with Tasks, Datasets, and Solutions](https://arxiv.org/abs/2506.21085)
...
### [226] [Enhancing Homophily-Heterophily Separation: Relation-Aware Learning in Heterogeneous Graphs](https://arxiv.org/abs/2506.20980)
...
TL;DR: This paper evaluates CoT techniques in APR and introduces T^3, a framework combining LLMs with tree search to improve repair solution precision and provide guidance for optimization.


<details>
  <summary>Details</summary>
Motivation: To enhance APR by leveraging LLMs and CoT techniques, addressing the gap in their application due to complex logic and multi-step reasoning requirements.

Method: Systematically evaluate existing CoT techniques in APR tasks and propose T^3, integrating LLMs' reasoning with tree search.

Result: T^3 improves the precision of generating candidate repair solutions and offers guidance for sample selection and repair strategies.

Conclusion: T^3 establishes a robust framework for efficient automated debugging by combining LLMs with tree search.
...
### [233] [From On-chain to Macro: Assessing the Importance of Data Source Diversity in Cryptocurrency Market Forecasting](https://arxiv.org/abs/2506.21246)
*Giorgos Demosthenous,Chryssis Georgiou,Eliada Polydorou*

Main category: q-fin.PM

TL;DR: This study explores how using different types of data improves cryptocurrency prediction models, finding that diverse data sources enhance accuracy, especially with on-chain metrics being crucial for both short and long-term predictions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how data source diversity affects cryptocurrency forecasting model performance and to identify key factors driving short-term and long-term market movements.

Method: The study integrates various data categories (technical indicators, on-chain metrics, sentiment, traditional market indices, macroeconomic indicators) and introduces the Crypto100 index. It also proposes a novel feature reduction algorithm to select impactful features.

Result: Experiments show that data diversity significantly improves predictive performance across time horizons. On-chain metrics are vital for both short and long-term predictions, while traditional and macroeconomic indicators become more relevant for longer-term forecasts. Model accuracy improves with diverse data sources.

Conclusion: The study highlights the importance of diverse data sources in cryptocurrency forecasting, identifies key factors for different time horizons, and provides a foundation for more accurate and resilient models.
...
### [234] [On Uniform Weighted Deep Polynomial approximation](https://arxiv.org/abs/2506.21306)
...
### [235] [Consistent Zero-shot 3D Texture Synthesis Using Geometry-aware Diffusion and Temporal Video Models](https://arxiv.org/abs/2506.20946)
*Donggoo Kang,Jangyeong Kim,Dasol Jeong,Junyoung Choi,Jeonga Wi,Hyunmin Lee,Joonho Gwon,Joonki Paik*

Main category: cs.GR

TL;DR: VideoTex is a new method for making textures that uses video models to fix problems with where they go and how they look over time.


<details>
  <summary>Details</summary>
Motivation: Current texture synthesis methods have issues because they don't consider the whole picture or geometry, leading to inconsistencies. Video models are good at making consistent videos over time, so we want to use that for textures.

Method: VideoTex uses video generation models and adds geometry-aware conditions to work with 3D meshes. It also uses a structure-wise UV diffusion strategy to keep important details and make textures better in areas that are hidden.

Result: VideoTex makes textures that look better with fewer seams and stays consistent over time in videos. It works better than old methods in keeping details, blending seams, and staying stable.

Conclusion: VideoTex is good for real-time apps that need high-quality visuals and consistent look over time, and it helps make textures better by using video models and geometry info.
...
### [236] [Generative Blocks World: Moving Things Around in Pictures](https://arxiv.org/abs/2506.20703)
*Vaibhav Vavilala,Seemandhar Jain,Rahul Vasanth,D. A. Forsyth,Anand Bhattad*

Main category: cs.GR

TL;DR: The paper introduces Generative Blocks World, a method for interacting with generated images by manipulating 3D geometric primitives, enabling accurate edits and preserving object identity through texture hints.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the interaction with generated images by allowing more flexible and accurate editing of scene geometry and textures.

Method: The method involves representing scenes as assemblies of convex 3D primitives and using a flow-based image generation technique conditioned on depth and texture hints that account for modified 3D structures.

Result: The results show that the approach outperforms prior works in visual fidelity, editability, and compositional generalization through both quantitative and qualitative experiments.

Conclusion: The paper concludes that Generative Blocks World effectively enables precise scene manipulation and maintains texture consistency, leading to better image editing capabilities.
...
### [237] [3DGH: 3D Head Generation with Composable Hair and Face](https://arxiv.org/abs/2506.20875)
...
### [238] [FairyGen: Storied Cartoon Video from a Single Child-Drawn Character](https://arxiv.org/abs/2506.21272)
...
### [239] [ResQ: A Novel Framework to Implement Residual Neural Networks on Analog Rydberg Atom Quantum Computers](https://arxiv.org/abs/2506.21537)
*Nicholas S. DiBrita,Jason Han,Tirthak Patel*

Main category: quant-ph

TL;DR: This paper explores the use of analog Rydberg atom quantum computers for neural ODE-based ResNets, introducing ResQ to optimize quantum dynamics for machine learning classification.


<details>
  <summary>Details</summary>
Motivation: To leverage quantum computing's potential to accelerate machine learning, particularly in optimizing ResNets using analog Rydberg atom systems.

Method: Introducing ResQ, a framework that uses analog quantum neural ODEs to optimize the dynamics of Rydberg atom quantum computers for classification tasks.

Result: Not specified in the abstract, but the framework ResQ is proposed to enhance the performance of quantum machine learning models.

Conclusion: Analog Rydberg atom quantum computers are well-suited for ResNets, and ResQ offers a novel approach to optimize their dynamics for machine learning applications.
...
### [240] [Parallels Between VLA Model Post-Training and Human Motor Learning: Progress, Challenges, and Trends](https://arxiv.org/abs/2506.20966)
...
### [241] [V2X-REALM: Vision-Language Model-Based Robust End-to-End Cooperative Autonomous Driving with Adaptive Long-Tail Modeling](https://arxiv.org/abs/2506.21041)
...
### [242] [WorldVLA: Towards Autoregressive Action World Model](https://arxiv.org/abs/2506.21539)
...
### [243] [Model-Based Real-Time Pose and Sag Estimation of Overhead Power Lines Using LiDAR for Drone Inspection](https://arxiv.org/abs/2506.20812)
*Alexandre Girard,Steven A. Parkison,Philippe Hamelin*

Main category: cs.RO

TL;DR: This paper introduces a method for drone localization using LiDAR by minimizing error between measurements and a geometric model of the conductor array, achieving accurate tracking with fast computation and robustness to outliers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to simplify the inspection process of overhead power lines by accurately localizing drones using onboard LiDAR sensors, despite challenges like limited surface area for LiDAR beams, inconsistent detection, and difficulty distinguishing conductors from other objects.

Method: The method proposed is an estimation approach that uses a single geometric model for the entire conductor array, minimizing the error between LiDAR measurements and this model instead of tracking individual conductors.

Result: The method achieves accurate tracking with a solver converging in under 50 ms per frame, even with partial observations, noise, and outliers. It can tolerate up to twice as many outlier points as valid measurements.

Conclusion: The conclusion is that the proposed estimation approach is effective for drone localization in power line inspections, offering fast computation and robustness against various real-world challenges.
...
### [244] [ThermalDiffusion: Visual-to-Thermal Image-to-Image Translation for Autonomous Navigation](https://arxiv.org/abs/2506.20969)
*Shruti Bansal,Wenshan Wang,Yifei Liu,Parv Maheshwari*

Main category: cs.RO

TL;DR: This paper addresses the challenge of limited thermal imagery data in robotics by using conditional diffusion models to generate synthetic thermal images from RGB data.


<details>
  <summary>Details</summary>
Motivation: Autonomous systems need reliable environment estimation, but traditional sensors like cameras, LiDARs, and RADARs struggle in nighttime or degraded conditions. Thermal cameras offer better object detection in such scenarios, but there's a lack of thermal data for training.

Method: The paper proposes using conditional diffusion models with self-attention mechanisms to convert RGB images into synthetic thermal images, augmenting existing multi-modal datasets.

Result: The method successfully generates synthetic thermal data, enabling more effective adaptation of thermal cameras in robotics and automation applications.

Conclusion: Enhancing multi-modal datasets with synthetic thermal imagery through diffusion models can improve autonomous systems' performance in challenging environmental conditions.
...
TL;DR: The paper proposes AJQE, an advanced method for enhancing JPEG image quality directly in the DCT domain, leveraging correlations in DCT coefficients to improve performance and reduce computational costs compared to pixel-domain approaches.


<details>
  <summary>Details</summary>
Motivation: Existing JPEG quality enhancement methods in the pixel domain are computationally expensive due to decoding requirements. Direct enhancement in the DCT domain is desired but current methods have limited performance.

Method: The authors identify two critical correlations in DCT coefficients and propose AJQE, which allows adapting pixel-domain models to the DCT domain efficiently.

Result: AJQE shows a 0.35 dB improvement in PSNR and 60.5% higher throughput compared to pixel-domain methods.

Conclusion: The AJQE method effectively enhances JPEG image quality in the DCT domain by exploiting correlations, leading to better performance and lower computational complexity.
...
### [250] [GANet-Seg: Adversarial Learning for Brain Tumor Segmentation with Hybrid Generative Models](https://arxiv.org/abs/2506.21245)
*Qifei Cui,Xinyu Lu*

Main category: eess.IV

TL;DR: This paper presents a new framework for brain tumor segmentation using GANs and Unet, improving accuracy and robustness with multi-modal MRI and synthetic data augmentation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of limited annotated datasets in brain tumor segmentation by leveraging pre-trained GANs and multi-modal MRI data.

Method: The method combines a global anomaly detection module with a refined mask generation network, using adversarial loss constraints for iterative improvement. It also employs multi-modal MRI data and synthetic image augmentation.

Result: The results show high sensitivity and accuracy in lesion-wise Dice and HD95 metrics on the BraTS dataset, outperforming the baseline.

Conclusion: The proposed method is scalable and reduces dependency on fully annotated data, making it suitable for real-world clinical applications.
...
### [251] [Lightweight Physics-Informed Zero-Shot Ultrasound Plane Wave Denoising](https://arxiv.org/abs/2506.21499)
...
### [252] [Exploring the Design Space of 3D MLLMs for CT Report Generation](https://arxiv.org/abs/2506.21535)
...
### [253] [Evaluating PDE discovery methods for multiscale modeling of biological signals](https://arxiv.org/abs/2506.20694)
...
### [254] [Proof of The TAP Free Energy for High-Dimensional Linear Regression with Spherical Priors at All Temperatures](https://arxiv.org/abs/2506.20768)
...
### [255] [Prokhorov Metric Convergence of the Partial Sum Process for Reconstructed Functional Data](https://arxiv.org/abs/2506.21172)
...
### [256] [Bayes linear estimator in the general linear model](https://arxiv.org/abs/2506.21192)
*Hirai Mukasa*
...
### [257] [Causal inference via implied interventions](https://arxiv.org/abs/2506.21501)
...
### [258] [Robust Alignment via Partial Gromov-Wasserstein Distances](https://arxiv.org/abs/2506.21507)
...
### [259] [Asymptotic Inference for Exchangeable Gibbs Partition](https://arxiv.org/abs/2506.21527)
*Takuya Koriyama*
...
### [260] [Detecting weighted hidden cliques](https://arxiv.org/abs/2506.21543)
