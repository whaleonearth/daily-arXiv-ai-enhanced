...
TL;DR: The paper introduces a test-time adaptation method for progress estimation models, using gradient-based meta-learning to optimize a self-supervised objective, enabling adaptation to test trajectories' visual and temporal contexts without relying on temporal order, outperforming autoregressive vision-language models on diverse tasks and environments.


<details>
  <summary>Details</summary>
Motivation: Current in-context learning approaches using autoregressive vision-language models struggle to adapt progress estimation models to varying test trajectories' visual and temporal contexts, necessitating a more effective method for online adaptation.

Method: The authors propose a gradient-based meta-learning strategy trained on expert visual trajectories and natural language task descriptions, optimizing a learned self-supervised objective for test-time adaptation.

Result: The method successfully generalizes from a single training environment to diverse out-of-distribution tasks, environments, and embodiments, achieving superior performance over existing autoregressive vision-language models in test-time adaptation scenarios.

Conclusion: This test-time adaptation approach enhances progress estimation by leveraging semantic content rather than temporal order, offering a more robust solution for varied and unseen testing conditions.
...
TL;DR: The paper compares CNNs and ViTs for multi-classifying psoriasis and similar skin diseases, finding ViTs, particularly DaViT-B, outperform with smaller models and an F1-score of 96.4%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to evaluate and compare the effectiveness of CNNs and ViTs in classifying psoriasis and similar diseases, aiming to find more efficient models for automated detection.

Method: The method involves adapting pre-trained models on ImageNet to a specific dataset for psoriasis classification, comparing their performance.

Result: Both CNNs and ViTs achieved high predictive metrics, but ViTs showed superior performance with smaller models, especially DaViT-B with 96.4% F1-score.

Conclusion: The conclusion states that ViTs have significant potential for medical image classification tasks, with DaViT-B recommended as the most efficient architecture for automated psoriasis detection.
...
TL;DR: Propose ScoreMix, a data augmentation strategy using diffusion model scores to improve discriminators with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: To enhance discriminator performance when labeled data is scarce, leveraging diffusion model properties.

Method: Convexly mix scores from different class-conditioned trajectories during diffusion sampling to create synthetic samples.

Result: Significant improvement in discriminative capabilities across benchmarks, with better gains from mixing distant classes in embedding space.

Conclusion: ScoreMix offers practical benefits for training discriminators without much parameter tuning, helping with large dataset issues.
...
TL;DR: This paper proposes a weakly supervised multimodal framework for generating structured SOAP notes from limited inputs, aiming to reduce clinician workload and reliance on manual annotations while achieving performance comparable to leading models like GPT-4o and Claude.


<details>
  <summary>Details</summary>
Motivation: Skin carcinoma is the most prevalent cancer globally, with high healthcare costs. Manual SOAP note generation is time-consuming and contributes to clinician burnout, necessitating an automated solution.

Method: A weakly supervised multimodal framework that uses lesion images and sparse clinical text to generate structured SOAP notes, reducing the need for extensive manual annotations.

Result: The method achieves performance comparable to GPT-4o, Claude, and DeepSeek Janus Pro on key clinical relevance metrics, and introduces two new metrics, MedConceptEval and Clinical Coherence Score (CCS), for evaluating clinical quality.

Conclusion: The proposed approach enables scalable, clinically accurate documentation with reduced clinician burden and less dependency on large annotated datasets.
...
TL;DR: This paper explores class-incremental learning (CIL) techniques for distinguishing honey's botanical origins using hyperspectral imaging, proposing a novel method combining CIL with continual backpropagation (CB) to enhance performance by reinitializing hidden neurons.


<details>
  <summary>Details</summary>
Motivation: The need for accurate botanical origin determination of honey to protect consumer interests and ensure market value, given the impracticality of collecting all honey varieties for training.

Method: Examining and comparing multiple CIL algorithms on a real-world honey hyperspectral imaging dataset, and proposing a novel technique that combines CIL with continual backpropagation (CB) to reinitialize less-used hidden neurons and improve model performance.

Result: Experiments demonstrated that the CB technique improved the performance of most CIL methods by 1-7%.

Conclusion: The study concludes that combining CIL with CB is effective for improving botanical origin differentiation in honey, offering a promising approach for real-world applications.
...
TL;DR: This paper introduces CogStream, a new task for streaming video reasoning that focuses on selecting relevant contextual information. It presents a dataset and a baseline model called CogReasoner, which uses visual stream compression and historical dialogue retrieval to handle the computational burden and irrelevant context issues.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges in streaming video reasoning caused by the reliance on contextual information, which leads to high computational costs and model distraction by irrelevant data.

Method: The paper proposes CogStream, a task requiring models to select relevant historical context. It introduces a densely annotated dataset via a semi-automatic pipeline and CogReasoner, a baseline model using visual stream compression and historical dialogue retrieval.

Result: Experiments show the effectiveness of CogReasoner in handling streaming video reasoning by efficiently reducing computational burden and focusing on key details through context selection.

Conclusion: The paper concludes that CogStream is a viable approach to improve multimodal understanding in streaming video scenarios by effectively managing contextual information, with CogReasoner serving as a strong baseline for future research.
...
TL;DR: ALBERT is a new model for car damage detection and part segmentation, showing strong performance in both tasks.


<details>
  <summary>Details</summary>
Motivation: The need for accurate and efficient automotive inspection and assessment systems, with the challenge of distinguishing real from fake damages and segmenting car parts.

Method: ALBERT uses BERT's bidirectional encoder and advanced localization methods, trained on a large automotive dataset with detailed annotations.

Result: ALBERT achieves high accuracy in segmenting car parts and classifying real/fake damages, outperforming previous methods.

Conclusion: ALBERT provides a robust solution for intelligent automotive inspection, with potential applications in damage assessment and repair processes.
...
TL;DR: CR-CLIP is an efficient model for visual-textual multi-instance retrieval, improving upon AVION with cross-modal attention flow and achieving high performance without ensemble learning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance cross-modal retrieval by creating more context-aware joint representations through bidirectional dynamic interaction between visual and textual features.

Method: CR-CLIP builds on the dual-encoder AVION, adding a cross-modal attention flow module for dynamic interaction and refinement. It uses Symmetric Multi-Similarity Loss with soft-label relevance matrices for semantic alignment.

Result: CR-CLIP achieves 66.78mAP and 82.08nDCG on EPIC-KITCHENS-100 without ensemble learning, outperforming the baseline.

Conclusion: CR-CLIP effectively improves cross-modal retrieval through its architecture, validated by high performance metrics and open-source release.
...
TL;DR: The paper introduces an end-to-end visual-to-causal framework that converts species images into interpretable causal insights about habitat preferences, using methods like species recognition, occurrence retrieval, pseudo-absence sampling, climate data extraction, and causal inference. It generates human-readable explanations with templates and large language models, showing promise for biodiversity conservation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explain why species live in specific locations to better understand ecological systems and conserve biodiversity, addressing the limitations of current fragmented and inaccessible ecological workflows.

Method: The method involves a framework that integrates species recognition, global occurrence retrieval, pseudo-absence sampling, climate data extraction, and modern causal inference techniques to discover environmental causal structures and estimate their influence on species occurrence.

Result: The framework was demonstrated on a bee and a flower species, with early results showing the potential of the multimodal AI assistant combined with ecological modeling practices to describe species habitats in human-understandable language.

Conclusion: The conclusion states that the proposed framework has potential for biodiversity conservation by providing interpretable causal insights from species images, using AI and ecological modeling practices.
...
TL;DR: The paper introduces CEI, a new metric for detecting demographic bias in face recognition systems, especially in subtle tail disparities. CEI and its automated version CEI^A show superior bias detection compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing metrics fail to detect demographic bias in high-performance face recognition systems, particularly in subtle disparities within the tails of score distributions.

Method: The paper proposes the Comprehensive Equity Index (CEI), which analyzes genuine and impostor score distributions separately, allowing configurable focus on tail probabilities and overall distribution shapes. It also introduces CEI^A, an automated version for objective and practical use.

Result: Experiments with state-of-the-art FR systems, biased models, and diverse datasets demonstrate that CEI effectively detects nuanced biases where previous methods could not.

Conclusion: CEI offers a robust and sensitive tool for assessing fairness in face recognition systems. The methods are applicable beyond face biometrics for comparing statistical distributions in problems involving tail analysis.
...
TL;DR: The paper introduces LRSLAM, a more efficient visual SLAM model using low-rank tensor decomposition methods, which outperforms existing approaches in convergence rates, memory efficiency, and reconstruction/localization quality.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of dense visual SLAM in achieving real-time performance, robustness, and scalability for large-scale scenes, while overcoming the high computational costs and memory requirements of recent neural implicit scene representation approaches.

Method: Proposing LRSLAM, which utilizes low-rank tensor decomposition methods, specifically Six-axis and CP decompositions, to improve efficiency and performance compared to existing state-of-the-art approaches.

Result: LRSLAM demonstrates superior performance in parameter efficiency, processing time, and accuracy across diverse indoor RGB-D datasets, while maintaining reconstruction and localization quality.

Conclusion: LRSLAM is a more efficient visual SLAM model that effectively addresses the limitations of previous methods, offering better convergence rates, memory efficiency, and reconstruction/localization quality.
...
TL;DR: This paper proposes using deep neural networks to create synthetic golden standards for InP multi-project wafer manufacturing, improving defect detection efficiency.


<details>
  <summary>Details</summary>
Motivation: InP multi-project wafers have low production scale and high design variability, making traditional golden standards unavailable. Defect detection is currently manual and inefficient.

Method: Deep neural networks are trained to generate photo-realistic InP wafer images from CAD data, creating a synthetic golden standard.

Result: The deep-learning method outperforms a baseline decision-tree approach, allowing simulated golden dies in any region for efficient defect detection.

Conclusion: The proposed method effectively enables simulated golden standards for defect detection in InP wafers, showing practical utility in surface defect detection.
...
TL;DR: This paper presents a robot navigation controller using event cameras and reinforcement learning for real-time human-centered navigation and obstacle avoidance.


<details>
  <summary>Details</summary>
Motivation: Conventional image-based controllers have motion blur and latency issues due to fixed processing rates, so there's a need for a more adaptive approach.

Method: The framework combines event-based perception, range sensing, and Deep Deterministic Policy Gradient with an initial imitation learning phase.

Result: Promising results in simulation including robust navigation, pedestrian following, and obstacle avoidance.

Conclusion: The approach shows potential for real-time, adaptive robot navigation using event cameras and reinforcement learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for in-context image editing rely on task-specific pipelines and expert models, which may limit their flexibility and efficiency. The paper aims to explore a more direct approach using videos for training data.

Method: The authors propose a scalable way to annotate videos as interleaved multimodal sequences and design a block-causal diffusion transformer trained on three proxy tasks: next-image prediction, current segmentation prediction, and next-segmentation prediction.

Result: The model demonstrates strong in-context image editing capabilities and achieves state-of-the-art results on two multi-turn image editing benchmarks. It also shows promise in multi-concept composition, story generation, and chain-of-editing applications despite being trained only on videos.

Conclusion: Training in-context image editing models directly from videos is feasible and effective, offering potential for broader applications beyond traditional task-specific approaches.
...
TL;DR: This paper explores the difference between causation and correlation in Alzheimer's disease research using XGBoost and SHAP values, finding that correlations don't imply causation and emphasizing the need for careful data interpretation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve Alzheimer's disease research by distinguishing causal factors from correlated markers, which is crucial for accurate diagnosis, treatment, and understanding disease mechanisms.

Method: The method involves using correlation analysis, machine learning classification with XGBoost, and model interpretability techniques like SHAP values to examine relationships among clinical, cognitive, genetic, and biomarker features.

Result: The results show that strong correlations among variables don't necessarily indicate causation, and key features like cognitive scores and genetic risk factors are important for AD classification.

Conclusion: The conclusion is that integrating feature importance and interpretability with traditional statistical methods provides a foundation for future causal inference studies, which could lead to better early diagnosis and targeted interventions for Alzheimer's disease.
...
TL;DR: This paper proposes an agent-based autoscaling framework for edge computing that dynamically adjusts hardware and service configurations to improve SLO performance, comparing four agents with different convergence and scalability traits.


<details>
  <summary>Details</summary>
Motivation: Edge computing faces strict resource constraints, making traditional autoscaling inadequate. There's a need for more flexible scaling approaches that utilize multiple elasticity dimensions.

Method: The study introduces an agent-based autoscaling framework that adjusts hardware resources and internal service configurations. It evaluates four scaling agents (Active Inference, Deep Q Network, Analysis of Structural Knowledge, Deep Active Inference) using real-world services YOLOv8 and OpenCV in parallel processing scenarios.

Result: All agents achieved acceptable SLO performance, but with different convergence patterns. Deep Q Network showed benefits from pre-training, Structural Analysis converged quickly, and Deep Active Inference combined theoretical and practical advantages.

Conclusion: The research demonstrates that multi-dimensional agent-based autoscaling is viable for edge environments, suggesting potential for future work in this area.
...
TL;DR: This paper introduces a new diagnostic approach for complex cyber-physical systems that requires minimal prior knowledge, using a neural network-based symptom generator and a graph diagnosis algorithm.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of obtaining extensive prior knowledge, such as detailed system models or comprehensive training data, which is often required for diagnostic processes in complex cyber-physical systems.

Method: The method combines a neural network-based symptom generator with a graph diagnosis algorithm, utilizing minimal causal relationship information between subsystems.

Result: The method successfully includes the true causal component in its diagnosis set for 82% of cases and reduces the search space in 73% of scenarios when tested on simulated datasets.

Conclusion: The approach shows promise for practical applications in large and complex cyber-physical systems where prior knowledge is limited.
...
TL;DR: This paper proposes a pipeline to formalize and verify AML modeling constraints using OWL ontologies, RML, SPARQL, and SHACL with a Large Language Model.


<details>
  <summary>Details</summary>
Motivation: To enable automated validation of informal AML modeling constraints, making it easier for users to check complex rules without needing formal methods expertise.

Method: Mapping AML models to OWL ontologies via RML and SPARQL, translating textual rules into SHACL using a Large Language Model, and interpreting SHACL results in natural language.

Result: Demonstrated on a sample AML recommendation, showing that complex rules can be semi-automatically checked without user expertise in formal methods or ontologies.

Conclusion: The approach provides a way to validate AML constraints automatically, improving consistency and reducing manual effort in engineering data exchange.
...
TL;DR: This paper introduces aspsmt2smt, a compiler that translates ASPMT programs into SMT instances using gringo and z3, enabling SMT solvers to compute stable models and handle real number computations for continuous changes.


<details>
  <summary>Details</summary>
Motivation: To combine answer set programming with satisfiability modulo theories for more powerful reasoning, especially with real numbers and continuous changes.

Method: Developing a compiler that translates ASPMT programs into SMT instances, utilizing gringo for partial grounding and z3 for solving.

Result: The system effectively handles real number computations and reasoning about continuous changes through the integration of ASP and SMT solvers.

Conclusion: ASPMT modulo theories provides a viable approach for combining ASP and SMT, with the aspsmt2smt compiler demonstrating effectiveness in practical applications.
...
TL;DR: This paper introduces GenPlanX, a system that combines Large Language Models (LLMs) with classical AI planning to improve natural language understanding in planning tasks, showing effectiveness in office-related tasks.


<details>
  <summary>Details</summary>
Motivation: Classical AI Planning techniques struggle with understanding natural language descriptions of tasks, limiting their applicability in human-computer interaction scenarios. LLMs offer better interpretation of human intents, suggesting a need to integrate them with classical planning engines.

Method: The paper presents GenPlanX, which integrates LLMs for natural language processing with a classical AI planning engine and an execution and monitoring framework.

Result: GenPlanX is effective in assisting users with office-related tasks, demonstrating potential to streamline workflows and enhance productivity through human-AI collaboration.

Conclusion: Integrating LLMs with classical AI planning engines can improve task understanding and execution, enabling more seamless and productive human-AI collaboration in practical applications.
...
TL;DR: The paper provides a comprehensive review and unified taxonomy of automatic evaluation methods for generative AI across text, images, and audio, identifying five fundamental paradigms and suggesting future cross-modal research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of a systematic framework for evaluating generative AI outputs across different modalities, aiming to organize existing methods and guide future research.

Method: The method involves a comprehensive review of existing automatic evaluation methods, identifying common paradigms, and extending a text-based framework to image and audio generation.

Result: The result is a unified taxonomy that categorizes evaluation methods into five paradigms, applicable across text, visual, and audio modalities.

Conclusion: The conclusion states that the proposed taxonomy offers a structured approach to evaluating generated content, highlighting the potential for future research in cross-modal evaluation methodologies.
...
TL;DR: The paper introduces TaskCraft, an automated method for generating scalable, multi-tool agentic tasks to improve AI agent training and evaluation.


<details>
  <summary>Details</summary>
Motivation: Agentic tasks are critical for NLP and AI advancement, but existing data lacks tool interaction and benchmarks are costly to create.
...
Result: Empirical results show TaskCraft improves prompt optimization and enhances supervised fine-tuning of agentic foundation models.

Conclusion: The paper provides a large synthetic dataset of 36,000 tasks to support future research on agent tuning and evaluation.
...
TL;DR: This paper introduces Chat-of-Thought, a multi-agent system using LLMs for generating FMEA documents with collaborative agents and dynamic task routing.


<details>
  <summary>Details</summary>
Motivation: To optimize the generation and validation of FMEA tables for industrial assets by leveraging advanced AI techniques and collaborative workflows.

Method: Employing multiple LLM-based agents with specific roles, dynamic task routing, and a Chat of Thought mechanism for iterative content refinement.

Result: Demonstrates the potential of Chat-of-Thought in addressing challenges in industrial equipment monitoring through interactive, template-driven workflows and context-aware collaboration.

Conclusion: Chat-of-Thought offers an innovative approach to FMEA document generation by enabling dynamic, multi-persona discussions and efficient task routing among agents.
...
TL;DR: The paper studies how LLMs react to semantically same but token-level different prompts, finding model-specific response shifts linked to tokenization and decoding.


<details>
  <summary>Details</summary>
Motivation: To explore the impact of prompt variance on LLM behavior and identify potential issues in model evaluation stability.

Method: Proposed Prompt-Based Semantic Shift (PBSS) framework to measure behavioral drift under semantically equivalent prompt rewordings.

Result: PBSS showed consistent model-specific response shifts in ten constrained tasks, indicating statistical regularities related to tokenization and decoding.

Conclusion: The findings suggest that tokenization strategies and decoding dynamics could contribute to post-training quality of service instability, highlighting a need for better evaluation metrics.
...
TL;DR: This paper explores emotion analysis in Bangla using machine learning models like SVM, KNN, Random Forest, BiLSTM, and AdaBoost with techniques such as TF-IDF, PCA, and LIME to improve efficiency and explainability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance sentiment analysis for underresourced languages like Bangla, which have unique regional expressions and cultural features.

Method: The study uses machine learning models (Linear SVM, KNN, Random Forest) with n-gram TF-IDF data, investigates PCA for dimensionality reduction, and employs BiLSTM and AdaBoost to enhance decision trees. LIME is used to interpret AdaBoost predictions.

Result: The results show that combining TF-IDF with models like AdaBoost and BiLSTM improves emotion identification efficiency in Bangla, and PCA helps reduce dimensionality without significant performance loss.

Conclusion: The paper concludes that these techniques are effective for emotion analysis in underresourced languages, emphasizing the importance of explainability via LIME and efficient model selection.
...
TL;DR: This paper introduces a method using machine learning (word2vec) to create a lexicon of HC-related keywords across five subcategories, providing resources for researchers to analyze corporate communications on HC.


<details>
  <summary>Details</summary>
Motivation: Human capital is crucial for corporate value but lacks clear measurement/disclosure rules. There's a need for systematic analysis tools.

Method: Trained word2vec on confirmed HC disclosures to generate keywords, categorized into five subcategories.

Result: Developed a comprehensive HC lexicon with examples of its application, including fine-tuning BERT.

Conclusion: The lexicon and code offer a framework for studying HC management, with suggestions for future research.
...
TL;DR: The paper proposes scheduled interleaved speech--text training to improve speech-to-speech translation, showing better performance for low-resource languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address modality adaptation challenges from text to speech in large language models, which are trained on text-only data but need to be adapted to speech with limited speech-to-speech data.

Method: Scheduled interleaved speech--text training, where aligned text tokens are interleaved at the word level with speech units, and the text ratio is gradually decreased during training.

Result: The method improves translation performance, particularly for languages with limited training data, as shown by experiments on the CVSS dataset with LLaMA3.2-1B.

Conclusion: The proposed method effectively facilitates progressive modality adaptation from text to speech, leading to better S2ST performance, especially in low-resource scenarios.
...
TL;DR: This paper introduces a scalable method to generate high-quality chain-of-thought (CoT) supervision datasets by using program execution's determinism, avoiding human annotations or error-prone LLM-generated CoT. It shows that this approach improves LLMs' reasoning abilities across domains and reduces token length during inference.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of obtaining reliable and accurate reasoning supervision for training large language models (LLMs), which is crucial for enhancing their reasoning abilities.

Method: The method involves generating a high-quality CoT dataset by leveraging the determinism of program execution. It extracts verifiable, step-by-step reasoning traces from code execution and transforms them into natural language CoT reasoning.

Result: The experiments demonstrate that the method effectively equips LLMs with transferable reasoning abilities across various domains. Ablation studies also show that it produces highly accurate reasoning data and reduces token length during inference by minimizing repetition and overthinking.

Conclusion: The conclusion is that the proposed method offers a scalable and reliable way to generate CoT supervision datasets, improving LLMs' reasoning capabilities and efficiency.
...
TL;DR: This paper introduces TempVS, a benchmark for testing MLLMs' ability to understand temporal order in image sequences, revealing significant challenges and potential research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess and improve MLLMs' temporal grounding and reasoning capabilities, as current models face significant challenges in this area compared to humans.

Method: The method involves creating the TempVS benchmark with three tests (event relation inference, sentence ordering, image ordering) and accompanying grounding tests to evaluate MLLMs' temporal understanding.

Result: The result shows that 38 top MLLMs struggle with TempVS, indicating a large performance gap between these models and human abilities.

Conclusion: The conclusion highlights the need for future research to address the temporal reasoning limitations of MLLMs, suggesting that the TempVS benchmark provides insights into promising directions for improvement.
...
TL;DR: This paper studies how news media frames the Israel-Palestine conflict, finding more war-focused reporting and media biases across countries.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of media framing on public opinion during conflicts, addressing gaps in current qualitative studies and surface-level frame analysis.

Method: Using computational methods combining frame semantics and large language models to analyze news articles from US, UK, and Middle Eastern outlets.

Result: Higher focus on war-based reporting, with significant differences in framing assailants and victims among media outlets from different regions.

Conclusion: Media framing in the Israel-Palestine conflict shows biases that may exacerbate the conflict, highlighting the need for more balanced reporting.
...
TL;DR: This paper argues that verifying scientific claims against tables requires more than just predicting labels; it needs models to identify essential table cells for explanation. The authors create a new dataset with human-annotated cell-level rationales and propose a taxonomy for ambiguous cases. Experiments show that using table alignment info improves performance, but LLMs struggle to match human rationales.


<details>
  <summary>Details</summary>
Motivation: The motivation is that current scientific claim verification methods relying only on final labels lack interpretability and understanding of the model's reasoning process.

Method: The method involves reframing table-text alignment as an explanation task, building a new dataset by extending SciTab with human-annotated cell-level rationales, and creating a taxonomy for ambiguous cases.

Result: The results indicate that incorporating table alignment information enhances claim verification performance, and that most LLMs, despite correct labels, fail to recover human-aligned rationales.

Conclusion: The conclusion is that models need to be developed that can provide faithful reasoning by identifying the correct table cells, rather than just predicting labels, to improve scientific claim verification.
...
TL;DR: This paper evaluates the robustness of large language models (LLMs) in multi-user dialogue state tracking (DST) by extending an existing dataset with second-user utterances based on speech act theory, revealing significant performance drops and highlighting the need for future research.


<details>
  <summary>Details</summary>
Motivation: Current DST benchmarks focus on structured user-agent conversations, not real-world multi-user interactions. Existing LLMs show strong zero-shot DST performance but may not handle multi-user scenarios well, necessitating research into their robustness.

Method: The study extends an existing DST dataset by generating second-user utterances using speech act theory. This allows systematic evaluation of LLMs in multi-user settings with minimal dataset construction costs.

Result: Experiments show a significant performance drop in multi-user DST compared to single-user settings, indicating current LLMs struggle with tracking dialogue states when multiple speakers are involved.

Conclusion: The findings emphasize the need for future research to improve LLMs for multi-user DST, aiming to develop more realistic and robust dialogue tracking models.
...
TL;DR: The paper introduces a hybrid method combining data-driven inference and rule-based heuristics for reconstructing ancestral word forms, showing significant improvement in Latin protoform reconstruction.


<details>
  <summary>Details</summary>
Motivation: Prior methods for reconstructing protoforms rely on probabilistic models of phonological edits but are limited by their data-driven nature. The paper aims to integrate rule-based heuristics with data-driven approaches for better accuracy.

Method: The authors use an evolutionary optimization framework that combines data-driven inference with rule-based heuristics to reconstruct protoforms.

Result: The method achieved substantial improvements over established baselines in character-level accuracy and phonological plausibility when applied to reconstructing Latin protoforms from Romance language cognates.

Conclusion: The hybrid approach of integrating statistical patterns and linguistically motivated constraints enhances the reconstruction of protoforms, providing a more effective method than prior data-driven models.
...
TL;DR: SDialog is a Python toolkit using instruction-tuned LLMs to generate and analyze synthetic dialogues for conversational AI, offering modularity, extensibility, and support for workflows like multi-agent simulation.


<details>
  <summary>Details</summary>
Motivation: The need for high-quality, flexible, and reproducible synthetic dialogues in training, evaluating, and benchmarking conversational AI systems.

Method: SDialog uses instruction-tuned LLMs with abstractions for personas, orchestration, and scenario management to create realistic and controllable conversational data.

Result: SDialog supports workflows such as multi-agent simulation and scenario-driven generation, contributing to the standardization of tools for synthetic data generation.

Conclusion: SDialog represents a crucial advancement for ensuring reproducibility in the fast-evolving research landscape of conversational AI.
...
TL;DR: This paper studies how large language models (LLMs) handle character-level tasks like spelling, finding that they don't process them straightforwardly. Instead, they rely on deeper transformer layers to reconstruct character knowledge, with evidence from probing classifiers, knowledge neurons, and attention weights.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs internally represent and use character-level information during spelling tasks, as they struggle with complex character-level problems despite high accuracy in simple tasks.

Method: The paper uses probing classifiers, identifies knowledge neurons, and examines attention weights to validate the mechanism of how LLMs reconstruct character-level knowledge through intermediate and higher transformer layers.

Result: The embedding layer doesn't fully encode character-level info beyond the first character, so LLMs depend on deeper layers for reconstruction, leading to a noticeable 'breakthrough' in their spelling behavior.

Conclusion: LLMs do not handle character-level tasks straightforwardly, relying on higher transformer layers to reconstruct character knowledge, which provides insight into their internal processing mechanisms.
...
TL;DR: This paper addresses the issue of missing hypernymy links in Open English Wordnet, focusing on adjectives and introducing a new resource and methodology for predicting such relations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to fill the gaps in hypernymy links within Open English Wordnet, particularly for adjectives, which have different characteristics compared to nouns and verbs.

Method: The paper discusses the theoretical aspects of hypernymy for adjectives, develops a new resource for adjective hypernymy, and fine-tunes large language models to predict these relations, adapting the TaxoLLaMa methodology.

Result: The results demonstrate that TaxoLLaMa's methodology can be effectively adapted to predict adjective hypernymy, suggesting potential for improving Open English Wordnet.

Conclusion: The conclusion is that establishing hypernymy links for adjectives is feasible through the developed resource and language model fine-tuning, contributing to the enhancement of Open English Wordnet as a linguistic resource.
...
TL;DR: The paper evaluates uncertainty estimation methods for clinical LLMs, finding that lightweight methods perform nearly as well as Semantic Entropy with single generation.
...
Method: Fine-grained evaluation of ten LLMs across two datasets, eleven specialties, and six question types, comparing single-generation and sampling-based methods with behavioral signal estimators.

Result: Lightweight methods based on reasoning traces match Semantic Entropy's performance with one generation, showing variation across specialties and question types.

Conclusion: Model selection should consider question type and model strengths for effective uncertainty estimation in clinical applications.
...
TL;DR: The paper introduces mRAG, a multi-agent RAG framework that uses self-training with reward-guided sampling to improve collaboration and performance on complex tasks.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of traditional RAG systems by enabling better inter-agent collaboration and addressing complex real-world tasks.

Method: mRAG employs a multi-agent framework with specialized agents for planning, searching, reasoning, and coordination, using self-training and reward-guided trajectory sampling to optimize collaboration and generate responses.

Result: mRAG outperformed conventional RAG baselines in the SIGIR 2025 LiveRAG competition on DataMorgana-derived datasets.

Conclusion: The framework demonstrates effectiveness for complex RAG tasks, with analysis of competition results and case studies highlighting its strengths.
...
TL;DR: The paper studies how wav2vec2 models encode speech information across different languages, finding that phone, tone, and speaker subspaces are orthogonal and layer patterns are similar, with minimal language-matched advantages for phones and tones.


<details>
  <summary>Details</summary>
Motivation: To understand where and how self-supervised speech models represent different information types, previously focused on English, but this study extends to multiple languages.

Method: Used probing classifiers and geometric analyses to examine phone, lexical tone, and speaker representations in wav2vec2 models trained on four languages.

Result: Subspaces for phones, tones, and speakers are largely orthogonal across languages, with similar layerwise probing accuracy patterns and slight advantages for language-matched phone and tone probes in later layers.

Conclusion: The representation structure in wav2vec2 is largely independent of the pretraining speech material, suggesting consistent encoding across languages.
...
TL;DR: MedRef improves medical dialogue systems by refining knowledge and adjusting prompts for better accuracy and personalization.


<details>
  <summary>Details</summary>
Motivation: Existing medical dialogue systems have issues with finding relevant medical info and creating personalized, accurate responses.

Method: MedRef uses knowledge refining to filter out irrelevant data and a prompt structure with historical and evident details. It also has Triplet Filter and Demo Selector modules for real-time adaptability.

Result: MedRef performs better than current systems on MedDG and KaMed benchmarks, showing higher quality and accuracy in medical entities.
...
TL;DR: This paper studies how fine-tuning affects large language models, especially using efficient methods like LoRA and QLoRA. It shows these methods work well for specific tasks without needing too much computing power, and that the data used for training needs to match the task. The paper gives ideas for understanding these methods and tips for developers with limited resources.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how fine-tuning impacts large language model performance, with a focus on parameter-efficient methods. The goal is to provide theoretical insights and practical guidance for developers who need to adapt LLMs efficiently with limited resources.

Method: The method involves investigating and validating the impact of fine-tuning on large language models, specifically using parameter-efficient approaches like LoRA and QLoRA. The evaluation is conducted across three domains: commonsense reasoning (HellaSwag), mathematical reasoning (GSM8K), and multi-domain knowledge (MMLU-CS).

Result: The results show that LoRA-based methods improve task-specific performance while keeping computational efficiency. Additionally, performance is strongly influenced by how well the fine-tuning dataset aligns with the benchmark tasks.

Conclusion: The study concludes that LoRA and QLoRA are effective for efficient LLM adaptation. It emphasizes the importance of dataset alignment with tasks and offers both theoretical understanding and practical advice for developers working with limited resources.
...
TL;DR: The paper introduces Magistral, a reasoning model developed using a ground-up approach with its own models and infrastructure, showcasing the effectiveness of pure RL training on LLMs and demonstrating improvements in multimodal understanding, instruction following, and function calling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the limits of pure reinforcement learning (RL) training for large language models (LLMs) and to develop a scalable RL pipeline without relying on existing implementations or RL traces from prior models.

Method: The method involves a ground-up approach using their own models and infrastructure, employing pure RL training on text data alone, and introducing a simple method to enforce the model's reasoning language.

Result: The results show that RL on text data maintains most of the initial checkpoint's capabilities, with improvements in multimodal understanding, instruction following, and function calling. Magistral Medium is trained using RL alone, and Magistral Small is open-sourced with cold-start data from Magistral Medium.

Conclusion: The conclusion is that pure RL training of LLMs is effective, and their scalable RL pipeline enables exploration of the limits of such training, with potential for improving model capabilities in various tasks.
...
TL;DR: The paper introduces a Chinese harm detection benchmark with real-world data and a knowledge rule base, along with a knowledge-augmented baseline to improve model performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of comprehensive Chinese datasets for harmful content detection and enhance the effectiveness of LLMs in this task.

Method: Created a benchmark with six categories using real-world data and developed a knowledge rule base. Proposed a knowledge-augmented baseline combining human-annotated rules and LLMs' implicit knowledge.

Result: The benchmark and knowledge rules provide a valuable resource for Chinese harmful content detection. The knowledge-augmented baseline allows smaller models to perform as well as SOTA LLMs.

Conclusion: This work contributes a new benchmark and method for improving harmful content detection in Chinese, leveraging both explicit knowledge and LLMs' capabilities.
...
TL;DR: This paper explores how AI can enhance human creativity through three modes: Support, Synergy, and Symbiosis, considering technical autonomy and perceived agency.
...
Method: The method involves adopting a distributed creativity perspective to identify and analyze the three modes of AI contribution to creative processes.

Result: The result is the identification of three primary modes (Support, Synergy, Symbiosis) and an examination of their influence on various levels of creativity.

Conclusion: The conclusion discusses the theoretical, ethical, and design implications of these configurations for AI-human creative interactions.
...
### [187] [Agentic Semantic Control for Autonomous Wireless Space Networks: Extending Space-O-RAN with MCP-Driven Distributed Intelligence](https://arxiv.org/abs/2506.10925)
...
TL;DR: This paper proposes a semantic agentic layer for Space-O-RAN to enhance lunar surface operations with context-aware decisions.


<details>
  <summary>Details</summary>
Motivation: Lunar surface operations require wireless communication systems that are autonomous, robust to disruption, and adaptable to environmental and mission-driven contexts. Current Space-O-RAN has limitations in decision logic and semantic integration.

Method: The authors extend Space-O-RAN with a semantic agentic layer using MCP and A2A protocols, enabling context-aware decision-making across different control layers. They deploy distributed cognitive agents in rovers, landers, and lunar base stations to implement coordination strategies like delay-adaptive reasoning and bandwidth-aware semantic compression, interacting with MCP servers to handle telemetry, locomotion planning, and mission constraints.

Result: The proposed extension allows for more adaptive and context-aware decision-making in wireless communication systems for lunar operations, improving autonomy and robustness.

Conclusion: Incorporating semantic agentic layers with MCP and A2A protocols enhances Space-O-RAN's capability to support complex lunar missions by enabling dynamic, context-aware coordination among distributed agents.
...
### [188] [A multi-scale loss formulation for learning a probabilistic model with proper score optimisation](https://arxiv.org/abs/2506.10868)
*Simon Lang,Martin Leutbecher,Pedro Maciel*

Main category: physics.ao-ph

TL;DR: The paper studies how a multi-scale loss function affects training weather forecasting models using AIFS-CRPS, showing it improves small-scale variability without harming overall performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the training of probabilistic weather forecasting models by better handling small-scale variability while maintaining forecast skill.

Method: The method involves using a multi-scale loss formulation in AIFS-CRPS, a machine-learned weather model from ECMWF, trained by optimizing the afCRPS.

Result: The result shows that the multi-scale loss effectively constrains small-scale variability without negatively impacting forecast skill.

Conclusion: The conclusion is that this approach provides promising directions for future research in scale-aware training of weather forecasting models.
...
### [192] [Evaluation empirique de la sécurisation et de l'alignement de ChatGPT et Gemini: analyse comparative des vulnérabilités par expérimentations de jailbreaks](https://arxiv.org/abs/2506.10029)
*Rafaël Nouailles*

Main category: cs.CR

TL;DR: This paper compares the security and alignment of ChatGPT and Gemini, exploring jailbreak techniques and their implications.


<details>
  <summary>Details</summary>
Motivation: To address emerging cybersecurity challenges in LLMs like ChatGPT and Gemini, such as prompt injection, jailbreaking, hallucinations, and deep fake risks.

Method: Comparative analysis of security and alignment levels between ChatGPT and Gemini, along with a taxonomy of jailbreak techniques through experiments.

Result: The paper identifies differences in security and alignment between the two models, categorizes various jailbreak methods, and highlights associated risks.

Conclusion: The study emphasizes the importance of robust security measures and alignment strategies to mitigate risks in LLMs, suggesting ongoing research and collaboration for safer AI deployment.
...
### [193] [Safeguarding Multimodal Knowledge Copyright in the RAG-as-a-Service Environment](https://arxiv.org/abs/2506.10030)
...
### [194] [Disclosure Audits for LLM Agents](https://arxiv.org/abs/2506.10171)
*Saswat Das,Jameson Sandler,Ferdinando Fioretto*

Main category: cs.CR

TL;DR: This paper introduces CMPL, an auditing framework for conversational privacy in large language model agents, which stress-tests agents to uncover privacy risks through iterative probing and multi-turn interactions.


<details>
  <summary>Details</summary>
Motivation: Large language model agents are increasingly used in sensitive applications but face privacy risks due to continuous data access, necessitating a robust auditing framework.

Method: The CMPL framework uses iterative probing and multi-turn interaction simulations to stress-test agents and identify latent privacy vulnerabilities.

Result: CMPL effectively reveals privacy risks not detected by single-turn defenses across various domains, data modalities, and safety configurations.

Conclusion: The paper presents CMPL as a diagnostic tool and an open benchmark for evaluating conversational privacy, highlighting its importance in ensuring agent compliance with privacy directives.
...
### [195] [GenBreak: Red Teaming Text-to-Image Generators Using Large Language Models](https://arxiv.org/abs/2506.10047)
...
TL;DR: This paper shows that some machine unlearning methods can be broken by simple prompt attacks, especially ELM. They tested 8 methods and found that RMU and TAR work better. The study also suggests that just changing answers doesn't remove knowledge, so better tests are needed.


<details>
  <summary>Details</summary>
Motivation: The motivation is to check if machine unlearning methods actually remove knowledge or just hide it, by testing them against prompt attacks.

Method: They evaluated 8 unlearning techniques on 3 model types using output, logit, and probe analysis to see if unlearned knowledge could be recovered.

Result: ELM is vulnerable to prompt attacks, recovering 57.3% accuracy with Hindi filler text. RMU and TAR are more robust. Logit analysis shows knowledge isn't just hidden in formatting.

Conclusion: Current unlearning methods may not truly remove knowledge. There's a need for better evaluation frameworks to tell real knowledge removal from just hiding answers.
...
### [197] [SOFT: Selective Data Obfuscation for Protecting LLM Fine-tuning against Membership Inference Attacks](https://arxiv.org/abs/2506.10424)
...
### [198] [Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications](https://arxiv.org/abs/2506.10467)
*Felix Härer*
...
### [200] [SoK: Evaluating Jailbreak Guardrails for Large Language Models](https://arxiv.org/abs/2506.10597)
...
### [201] [TED-LaST: Towards Robust Backdoor Defense Against Adaptive Attacks](https://arxiv.org/abs/2506.10722)
...
### [202] [ME: Trigger Element Combination Backdoor Attack on Copyright Infringement](https://arxiv.org/abs/2506.10776)
...
### [203] [Monitoring Decomposition Attacks in LLMs with Lightweight Sequential Monitors](https://arxiv.org/abs/2506.10949)
...
### [204] [FASCIST-O-METER: Classifier for Neo-fascist Discourse Online](https://arxiv.org/abs/2506.10789)
...
### [205] [LLM-Driven Personalized Answer Generation and Evaluation](https://arxiv.org/abs/2506.10829)
...
### [206] [Distributionally-Constrained Adversaries in Online Learning](https://arxiv.org/abs/2506.10293)
...
### [208] [Hessian Geometry of Latent Space in Generative Models](https://arxiv.org/abs/2506.10632)
...
### [209] [Understanding In-Context Learning on Structured Manifolds: Bridging Attention to Kernel Methods](https://arxiv.org/abs/2506.10959)
*Zhaiming Shen,Alexander Hsu,Rongjie Lai,Wenjing Liao*

Main category: cs.LG

TL;DR: This paper explores the theoretical aspects of in-context learning (ICL) for regression tasks on manifolds, linking attention mechanisms to kernel methods and showing that transformers achieve optimal regression rates with sufficient tasks.


<details>
  <summary>Details</summary>
Motivation: To understand ICL's theoretical foundations, especially for structured geometric data, and to bridge the gap between attention mechanisms and kernel methods.

Method: Establishing a connection between attention mechanisms and kernel methods, deriving generalization error bounds based on prompt length and training tasks.

Result: Transformers achieve the minimax regression rate for H"older functions on manifolds, with error scaling exponentially with the intrinsic dimension of the manifold.

Conclusion: The study reveals the importance of geometry in ICL and provides new tools for analyzing nonlinear models' ICL performance.
...
### [210] [Omni-DPO: A Dual-Perspective Paradigm for Dynamic Preference Learning of LLMs](https://arxiv.org/abs/2506.10054)
...
### [211] [Textual Bayes: Quantifying Uncertainty in LLM-Based Systems](https://arxiv.org/abs/2506.10060)
...
### [212] [Learning to Collaborate Over Graphs: A Selective Federated Multi-Task Learning Approach](https://arxiv.org/abs/2506.10102)
...
### [213] [GRAIL: A Benchmark for GRaph ActIve Learning in Dynamic Sensing Environments](https://arxiv.org/abs/2506.10120)
...
### [214] [Self-Predictive Representations for Combinatorial Generalization in Behavioral Cloning](https://arxiv.org/abs/2506.10137)
...
### [215] [Interpreting learned search: finding a transition model and value function in an RNN that plays Sokoban](https://arxiv.org/abs/2506.10138)
*Mohammad Taufeeque,Aaron David Tucker,Adam Gleave,Adrià Garriga-Alonso*

Main category: cs.LG

TL;DR: The paper analyzes a convolutional RNN trained for Sokoban, finding bidirectional search-like mechanisms and state-action activations acting as value functions, with specialized kernels creating paths and layers having individual plan representations.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanisms of a convolutional RNN trained via model-free reinforcement learning to play Sokoban, particularly how it uses test-time compute to solve more levels.

Method: Partial reverse-engineering of the RNN, analyzing its activations and kernels to identify components similar to classic bidirectional search and value functions.

Result: The RNN exhibits mechanisms akin to bidirectional search, with state-action activations serving as value functions, specialized kernels extending activations to form paths, and layers having distinct plan representations and value functions.

Conclusion: The study shows that test-time compute in the RNN can be understood through familiar search concepts, despite not being fully transparent.
...
### [216] [A Comparative Study of Machine Learning Techniques for Early Prediction of Diabetes](https://arxiv.org/abs/2506.10180)
*Mowafaq Salem Alzboon,Mohammad Al-Batah,Muhyeeddin Alqaraleh,Ahmad Abuashour,Ahmad Fuad Bader*

Main category: cs.LG

TL;DR: This study evaluates machine learning methods for diabetes prediction using the Pima Indians dataset, finding neural networks as the most accurate.


<details>
  <summary>Details</summary>
Motivation: Diabetes is a growing health issue, and early detection is vital. Machine learning shows promise in predicting diabetes.

Method: The study uses the Pima Indians Diabetes dataset with 768 patients' data (age, BMI, glucose) and tests various ML algorithms: Logistic Regression, Decision Tree, Random Forest, k-NN, Naive Bayes, SVM, Gradient Boosting, and Neural Network.

Result: Neural Network achieved 78.57% accuracy, followed by Random Forest at 76.30%, demonstrating ML algorithms' effectiveness in diabetes prediction.

Conclusion: Machine learning algorithms can aid in diabetes prediction and serve as efficient early detection tools.
...
### [217] [Optimizing Genetic Algorithms with Multilayer Perceptron Networks for Enhancing TinyFace Recognition](https://arxiv.org/abs/2506.10184)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh*

Main category: cs.LG

TL;DR: This study evaluates MLP networks using three methods on diverse datasets, finding that GA improves accuracy in complex data while PCA benefits low-dimensional data.


<details>
  <summary>Details</summary>
Motivation: To understand how feature selection and dimensionality reduction techniques affect MLP performance across different datasets.

Method: Baseline MLP training, GA-based feature selection, and PCA for dimensionality reduction.

Result: GA increased accuracy in complex datasets by selecting critical features, while PCA was effective in low-dimensional, noise-free datasets.

Conclusion: Feature selection and dimensionality reduction have interdependent roles in improving MLP performance, offering practical guidelines for machine learning tasks.
...
### [218] [Scalable Non-Equivariant 3D Molecule Generation via Rotational Alignment](https://arxiv.org/abs/2506.10186)
*Yuhui Ding,Thomas Hofmann*

Main category: cs.LG

TL;DR: This paper introduces a method to improve the scalability and efficiency of equivariant diffusion models for 3D molecule generation by relaxing equivariance constraints and learning sample-dependent SO(3) transformations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of specialized equivariant architectures in diffusion models, which hinder scalability and efficiency, while maintaining high sample quality in 3D molecule generation.

Method: The method involves learning a sample-dependent SO(3) transformation for each molecule to construct an aligned latent space, followed by training a non-equivariant diffusion model on these aligned representations.

Result: The approach shows significant improvement over previous non-equivariant models, achieves sample quality comparable to state-of-the-art equivariant models, and improves training and sampling efficiency.

Conclusion: The conclusion is that relaxing equivariance constraints through sample-dependent SO(3) transformations enables more scalable and efficient diffusion models for 3D molecule generation without sacrificing sample quality.
...
### [219] [Cross-Learning Between ECG and PCG: Exploring Common and Exclusive Characteristics of Bimodal Electromechanical Cardiac Waveforms](https://arxiv.org/abs/2506.10212)
...
TL;DR: This paper proposes using meta-learning for detecting malicious sockpuppets on Wikipedia, showing improved precision and releasing a new dataset.


<details>
  <summary>Details</summary>
Motivation: Malicious sockpuppet detection is crucial for maintaining reliable information and preventing disinformation on the internet, especially on open editing platforms like Wikipedia.

Method: The paper applies meta-learning, a machine learning technique that trains models across multiple tasks to improve performance in data-scarce settings, allowing rapid adaptation to new sockpuppet-groups' writing styles.

Result: Meta-learning significantly improves the precision of predictions compared to pre-trained models, advancing the fight against sockpuppetry on open editing platforms.

Conclusion: The study demonstrates that meta-learning is effective for detecting sockpuppets with limited text data, and the released dataset encourages further research in sockpuppetry and meta-learning.
...
### [221] [LaMAGIC2: Advanced Circuit Formulations for Language Model-Based Analog Topology Generation](https://arxiv.org/abs/2506.10235)
...
### [222] [Provably Learning from Language Feedback](https://arxiv.org/abs/2506.10341)
...
### [223] [Can We Infer Confidential Properties of Training Data from LLMs?](https://arxiv.org/abs/2506.10364)
...
### [224] [Discovering Hierarchical Latent Capabilities of Language Models via Causal Representation Learning](https://arxiv.org/abs/2506.10378)
...
### [225] [Time-IMM: A Dataset and Benchmark for Irregular Multimodal Multivariate Time Series](https://arxiv.org/abs/2506.10412)
...
### [226] [Deep Learning-Based Digitization of Overlapping ECG Images with Open-Source Python Code](https://arxiv.org/abs/2506.10617)
...
### [227] [Neural at ArchEHR-QA 2025: Agentic Prompt Optimization for Evidence-Grounded Clinical Question Answering](https://arxiv.org/abs/2506.10751)
...
### [228] [The Diffusion Duality](https://arxiv.org/abs/2506.10892)
*Subham Sekhar Sahoo,Justin Deschenaux,Aaron Gokaslan,Guanghan Wang,Justin Chiu,Volodymyr Kuleshov*

Main category: cs.LG

TL;DR: This paper introduces Duo, a method that improves uniform-state discrete diffusion models by leveraging Gaussian diffusion techniques, achieving better training speed and sampling efficiency.


<details>
  <summary>Details</summary>
Motivation: To narrow the performance gap between uniform-state discrete diffusion models and autoregressive/masked diffusion models by utilizing insights from Gaussian diffusion.

Method: Duo uses curriculum learning guided by Gaussian process to speed up training and discrete consistency distillation to enable fast few-step generation.

Result: Models with curriculum learning outperform autoregressive models in zero-shot perplexity on 3 out of 7 benchmarks; consistency distillation accelerates sampling by two orders of magnitude.

Conclusion: Duo enhances uniform-state discrete diffusion models through Gaussian diffusion techniques, showing significant improvements in training and sampling efficiency.
...
### [229] [PhysioWave: A Multi-Scale Wavelet-Transformer for Physiological Signal Representation](https://arxiv.org/abs/2506.10351)
...
### [230] [Robustly Improving LLM Fairness in Realistic Settings via Interpretability](https://arxiv.org/abs/2506.10922)
...
### [231] [GUARD: Guided Unlearning and Retention via Data Attribution for Large Language Models](https://arxiv.org/abs/2506.10946)
...
### [232] [Build the web for agents, not agents for the web](https://arxiv.org/abs/2506.10953)
...
### [233] [Time To Impeach LLM-as-a-Judge: Programs are the Future of Evaluation](https://arxiv.org/abs/2506.10403)
...
### [234] [Balanced Hyperbolic Embeddings Are Natural Out-of-Distribution Detectors](https://arxiv.org/abs/2506.10146)
...
### [235] [Geometric Regularity in Deterministic Sampling of Diffusion-based Generative Models](https://arxiv.org/abs/2506.10177)
*Defang Chen,Zhenyu Zhou,Can Wang,Siwei Lyu*

Main category: cs.LG

TL;DR: The paper discovers that diffusion-based generative models have a geometric regularity in their sampling dynamics, where trajectories lie in a low-dimensional subspace and have a ''boomerang'' shape. They propose a dynamic programming scheme to align sampling time schedules with this structure, improving image generation with minimal changes to solvers.


<details>
  <summary>Details</summary>
Motivation: To understand the geometric structure of sampling trajectories in diffusion-based models and improve their performance through better alignment with this structure.

Method: Analyzing the sampling dynamics of diffusion models, characterizing the low-dimensional subspace and boomerang shape, and proposing a dynamic programming-based method for time schedule alignment.

Result: The proposed method achieves superior image generation performance, particularly with few function evaluations (5-10), using minimal modifications to existing solvers.

Conclusion: The geometric regularity in sampling dynamics is a fundamental property of diffusion models, and leveraging it through dynamic programming can significantly enhance generation quality without major computational costs.
...
### [236] [Size-adaptive Hypothesis Testing for Fairness](https://arxiv.org/abs/2506.10586)
...
### [237] [Task Adaptation from Skills: Information Geometry, Disentanglement, and New Objectives for Unsupervised Reinforcement Learning](https://arxiv.org/abs/2506.10629)
...
### [238] [Time Series Forecasting as Reasoning: A Slow-Thinking Approach with Reinforced LLMs](https://arxiv.org/abs/2506.10630)
...
### [239] [Data Shifts Hurt CoT: A Theoretical Study](https://arxiv.org/abs/2506.10647)
...
### [240] [Saturation Self-Organizing Map](https://arxiv.org/abs/2506.10680)
*Igor Urbanik,Paweł Gajewski*

Main category: cs.LG

TL;DR: This paper introduces Saturation Self-Organizing Maps (SatSOM), an extension of SOMs that uses a saturation mechanism to reduce catastrophic forgetting by gradually decreasing the learning rate and neighborhood radius of neurons as they accumulate information, thus freezing well-trained neurons and focusing learning on underutilized areas.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address catastrophic forgetting in neural systems during continual learning, particularly in Self-Organizing Maps (SOMs), which, although interpretable and efficient, still suffer from this issue.

Method: The method involves extending SOMs with a saturation mechanism that dynamically adjusts the learning rate and neighborhood radius of neurons based on their information accumulation, effectively freezing well-trained neurons and directing learning to less utilized parts of the map.

Result: The result demonstrates that SatSOM improves knowledge retention in continual learning scenarios compared to traditional SOMs, though specific quantitative results are not detailed in the abstract.

Conclusion: The conclusion is that SatSOM is a promising approach for mitigating catastrophic forgetting in SOMs by leveraging the saturation mechanism to enhance stability and retention of previously learned information.
...
### [241] [ConTextTab: A Semantics-Aware Tabular In-Context Learner](https://arxiv.org/abs/2506.10707)
...
### [242] [ReGuidance: A Simple Diffusion Wrapper for Boosting Sample Quality on Hard Inverse Problems](https://arxiv.org/abs/2506.10955)
...
### [243] [Efficiency Robustness of Dynamic Deep Learning Systems](https://arxiv.org/abs/2506.10831)
...
### [244] [Farseer: A Refined Scaling Law in Large Language Models](https://arxiv.org/abs/2506.10972)
...
### [245] [Principled Approaches for Extending Neural Architectures to Function Spaces for Operator Learning](https://arxiv.org/abs/2506.10973)
...
### [246] [Rethinking Losses for Diffusion Bridge Samplers](https://arxiv.org/abs/2506.10982)
...
### [247] [Semantic Communication-Enabled Cloud-Edge-End-collaborative Metaverse Services Architecure](https://arxiv.org/abs/2506.10001)
...
### [248] [EQ-TAA: Equivariant Traffic Accident Anticipation via Diffusion-Based Accident Video Synthesis](https://arxiv.org/abs/2506.10002)
...
### [249] [Immersive Multimedia Communication: State-of-the-Art on eXtended Reality Streaming](https://arxiv.org/abs/2506.10004)
*Haopeng Wang,Haiwei Dong,Abdulmotaleb El Saddik*

Main category: cs.MM

TL;DR: This paper surveys XR streaming technologies, discussing their paradigms, traffic characteristics, QoE factors, attention-based optimization, and challenges.


<details>
  <summary>Details</summary>
Motivation: XR is advancing rapidly and has the potential to transform content creation and consumption, but efficient streaming remains a challenge.

Method: The paper provides a survey of XR streaming, analyzing traffic characteristics, QoE factors, and optimization methods.

Result: The survey identifies key elements for enhancing user satisfaction in XR systems and highlights challenges for future developments.

Conclusion: XR streaming requires tailored approaches due to its unique data transmission needs and user experience factors.
...
### [250] [HER2 Expression Prediction with Flexible Multi-Modal Inputs via Dynamic Bidirectional Reconstruction](https://arxiv.org/abs/2506.10006)
...
### [251] [Controllable Expressive 3D Facial Animation via Diffusion in a Unified Multimodal Space](https://arxiv.org/abs/2506.10007)
...
### [252] [Structured Graph Representations for Visual Narrative Reasoning: A Hierarchical Framework for Comics](https://arxiv.org/abs/2506.10008)
*Yi-Chun Chen*
...
### [253] [WDMIR: Wavelet-Driven Multimodal Intent Recognition](https://arxiv.org/abs/2506.10011)
...
### [254] [Multimodal Large Language Models: A Survey](https://arxiv.org/abs/2506.10016)
...
### [255] [Leveraging LLMs for Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10093)
...
### [256] [One For All: LLM-based Heterogeneous Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10106)
...
### [257] [A Navigation Framework Utilizing Vision-Language Models](https://arxiv.org/abs/2506.10172)
...
### [258] [Using Language and Road Manuals to Inform Map Reconstruction for Autonomous Driving](https://arxiv.org/abs/2506.10317)
*Akshar Tumu,Henrik I. Christensen,Marcell Vazquez-Chanlatte,Chikao Tsuchiya,Dhaval Bhanderi*

Main category: cs.RO

TL;DR: The paper proposes enhancing SMERF with OSM and road design data for better lane-topology prediction in autonomous navigation.


<details>
  <summary>Details</summary>
Motivation: Accurate road environment understanding is crucial for safe autonomous navigation, but existing methods may lack comprehensive data.

Method: Augmenting SMERF with structured road metadata from OSM and lane-width priors from design manuals combined with centerline encodings.

Result: Improved detection and association of lanes and traffic elements in complex intersections, demonstrated generalization across diverse topologies.

Conclusion: Integrating conventional road data with existing models enhances performance in lane-topology prediction, supporting reliable autonomous navigation.
...
### [259] [EmbodiedGen: Towards a Generative 3D World Engine for Embodied Intelligence](https://arxiv.org/abs/2506.10600)
...
### [260] [Eye, Robot: Learning to Look to Act with a BC-RL Perception-Action Loop](https://arxiv.org/abs/2506.10968)
...
### [261] [Grounded Vision-Language Navigation for UAVs with Open-Vocabulary Goal Understanding](https://arxiv.org/abs/2506.10756)
...
### [262] [Data-Driven Prediction of Dynamic Interactions Between Robot Appendage and Granular Material](https://arxiv.org/abs/2506.10875)
...
TL;DR: The paper introduces a new way to combine large language models (LLMs) with a Lisp environment, letting them create and use tools interactively through a REPL.


<details>
  <summary>Details</summary>
Motivation: To develop interactive AI systems that merge symbolic programming with neural language generation, allowing LLMs to have stateful memory and dynamically create tools.

Method: Proposing an architecture that embeds Lisp expressions in LLM generation and uses a middleware to intercept them, enabling interactive REPL-based tool development.

Result: A design framework and architectural principles for future interactive AI systems that integrate symbolic programming with neural generation.

Conclusion: This approach provides a foundation for creating more flexible and evolving AI systems by allowing LLMs to interactively define and modify their own tools.
...
### [264] [FastFLUX: Pruning FLUX with Block-wise Replacement and Sandwich Training](https://arxiv.org/abs/2506.10035)
...
### [265] [Ambient Diffusion Omni: Training Good Models with Bad Data](https://arxiv.org/abs/2506.10038)
...
### [266] [Learning-based density-equalizing map](https://arxiv.org/abs/2506.10027)
...
### [267] [Low-Barrier Dataset Collection with Real Human Body for Interactive Per-Garment Virtual Try-On](https://arxiv.org/abs/2506.10468)
...
### [268] [Edit360: 2D Image Edits to 3D Assets from Any Angle](https://arxiv.org/abs/2506.10507)
*Junchao Huang,Xinting Hu,Zhuotao Tian,Shaoshuai Shi,Li Jiang*

Main category: cs.GR

TL;DR: Edit360 is a tuning-free framework that extends 2D image edits to multi-view consistent 3D editing using video diffusion models and an Anchor-View Editing Propagation mechanism.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of existing 3D editing methods that restrict edits to predetermined viewpoints, which limits flexibility and practical use.

Method: Edit360 uses video diffusion models and a novel Anchor-View Editing Propagation mechanism to align and merge multi-view data in latent and attention spaces for consistent 3D edits.

Result: The framework allows user-specific 3D editing from any viewpoint, ensuring structural coherence and enabling high-quality 3D asset reconstruction.

Conclusion: Edit360 provides a flexible and practical solution for 3D content creation by allowing multi-view consistent edits without requiring model tuning.
...
### [269] [Transformer IMU Calibrator: Dynamic On-body IMU Calibration for Inertial Motion Capture](https://arxiv.org/abs/2506.10580)
...
### [270] [AniMaker: Automated Multi-Agent Animated Storytelling with MCTS-Driven Clip Generation](https://arxiv.org/abs/2506.10540)
...
### [271] [Ground Reaction Force Estimation via Time-aware Knowledge Distillation](https://arxiv.org/abs/2506.10265)
...
### [272] [Encoding call-by-push-value in the pi-calculus](https://arxiv.org/abs/2506.10584)
...
### [273] [StepProof: Step-by-step verification of natural language mathematical proofs](https://arxiv.org/abs/2506.10558)
...
### [275] [AC/DC: LLM-based Audio Comprehension via Dialogue Continuation](https://arxiv.org/abs/2506.10312)
*Yusuke Fujita,Tomoya Mizumoto,Atsushi Kojima,Lianbo Liu,Yui Sudo*

Main category: eess.AS

TL;DR: The paper introduces a new audio comprehension model that uses dialogue continuation to improve instruction-following without needing multitask tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the caption variation problem in audio comprehension models by capturing the meaning beyond surface-level words.

Method: The method involves training a model to generate responses as if input captions triggered a dialogue, leveraging LLMs' dialogue continuation ability.

Result: The model achieves zero-shot instruction-following capability without multitask instruction tuning, even when trained only on audio captioning datasets.

Conclusion: The conclusion is that dialogue continuation training effectively captures caption meaning, enabling the model to follow various unseen instructions as shown in experiments on AudioCaps, WavCaps, and Clotho datasets.
...
### [276] [RT-VC: Real-Time Zero-Shot Voice Conversion with Speech Articulatory Coding](https://arxiv.org/abs/2506.10289)
...
<details>
  <summary>Details</summary>
Motivation: Voice conversion is important for applications like assistive communication and entertainment, but existing methods have high latency and may not disentangle content and speaker characteristics effectively.

Method: RT-VC uses an articulatory feature space to separate content and speaker features, and integrates differentiable digital signal processing (DDSP) for efficient vocoding from these features.
...
Conclusion: RT-VC offers a more efficient and interpretable voice conversion system with reduced latency, making it suitable for real-time applications.
...
### [277] [Robust Unsupervised Adaptation of a Speech Recogniser Using Entropy Minimisation and Speaker Codes](https://arxiv.org/abs/2506.10653)
*Rogier C. van Dalen,Shucong Zhang,Titouan Parcollet,Sourav Bhattacharya*

Main category: eess.AS

TL;DR: This paper proposes a novel loss function and a speaker code to improve speech recogniser adaptation with limited data.


<details>
  <summary>Details</summary>
Motivation: Speech recognisers need adaptation for new environments/speakers but often have too little data for robust fine-tuning, especially when the data is unlabelled.

Method: Combines a conditional entropy loss over multiple hypotheses with a compact speaker code for efficient adaptation.

Result: Achieves 20% relative improvement in word error rate with one minute of adaptation data, increasing to 29% with ten minutes on a noise-augmented Common Voice dataset.

Conclusion: The proposed method enhances robustness of speech recognition adaptation using minimal data through better loss functions and speaker representations.
...
### [278] [DUN-SRE: Deep Unrolling Network with Spatiotemporal Rotation Equivariance for Dynamic MRI Reconstruction](https://arxiv.org/abs/2506.10309)
...
### [279] [Rethinking Brain Tumor Segmentation from the Frequency Domain Perspective](https://arxiv.org/abs/2506.10142)
...
### [280] [Prompt-Guided Latent Diffusion with Predictive Class Conditioning for 3D Prostate MRI Generation](https://arxiv.org/abs/2506.10230)
...
### [281] [Conditional diffusion models for guided anomaly detection in brain images using fluid-driven anomaly randomization](https://arxiv.org/abs/2506.10233)
...
### [282] [SWDL: Stratum-Wise Difference Learning with Deep Laplacian Pyramid for Semi-Supervised 3D Intracranial Hemorrhage Segmentation](https://arxiv.org/abs/2506.10325)
...
### [283] [ConStyX: Content Style Augmentation for Generalizable Medical Image Segmentation](https://arxiv.org/abs/2506.10675)
...
### [284] [Generalist Models in Medical Image Segmentation: A Survey and Performance Comparison with Task-Specific Approaches](https://arxiv.org/abs/2506.10825)
...
### [285] [Med-URWKV: Pure RWKV With ImageNet Pre-training For Medical Image Segmentation](https://arxiv.org/abs/2506.10858)
*Zhenhuan Zhou*
...
### [286] [Semi-Automated Quality Assurance in Digital Pathology: Tile Classification Approach](https://arxiv.org/abs/2506.10916)
...
### [287] [Equitable Mechanism Design for Facility Location](https://arxiv.org/abs/2506.10460)
*Toby Walsh*
...
### [288] [An Analysis of Datasets, Metrics and Models in Keyphrase Generation](https://arxiv.org/abs/2506.10346)
*Florian Boudin,Akiko Aizawa*

Main category: cs.IR

TL;DR: This paper provides a comprehensive analysis of over 50 keyphrase generation research papers, highlighting evaluation issues and introducing a new PLM-based model.


<details>
  <summary>Details</summary>
Motivation: There is a lack of review and analysis of previous work in keyphrase generation, despite continuous research efforts.

Method: The authors analyzed over 50 research papers on keyphrase generation to identify progress, limitations, and challenges.

Result: The study found issues with evaluation practices, including dataset similarity and inconsistent metrics leading to overestimated performances.

Conclusion: The paper addresses the need for a review in keyphrase generation and provides a new PLM-based model to support future research.
...
### [289] [Towards Understanding Bias in Synthetic Data for Evaluation](https://arxiv.org/abs/2506.10301)
...
### [290] [Conversational Search: From Fundamentals to Frontiers in the LLM Era](https://arxiv.org/abs/2506.10635)
*Fengran Mo,Chuan Meng,Mohammad Aliannejadi,Jian-Yun Nie*

Main category: cs.IR

TL;DR: This tutorial discusses how large language models (LLMs) are transforming conversational search by enabling more flexible, dialogue-based information retrieval through multi-turn interactions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the impact of LLMs on conversational search, given their capabilities in understanding and reasoning, which present new opportunities and challenges for building intelligent systems.

Method: The method is a tutorial designed for students, researchers, and practitioners, covering both fundamentals and emerging topics in conversational search powered by LLMs.

Result: The result is a comprehensive understanding of core principles and cutting-edge developments in conversational search driven by LLMs, enabling participants to contribute to next-generation systems.

Conclusion: The conclusion is that LLMs are revolutionizing conversational search, and this tutorial provides essential knowledge to advance intelligent, dialogue-based information retrieval systems.
...
### [292] [Precise Zero-Shot Pointwise Ranking with LLMs through Post-Aggregated Global Context Information](https://arxiv.org/abs/2506.10859)
...
### [293] [Optimal Non-Adaptive Group Testing with One-Sided Error Guarantees](https://arxiv.org/abs/2506.10374)
*Daniel McMorrow,Jonathan Scarlett*

Main category: cs.IT

TL;DR: The paper studies group testing for approximate recovery of defective items, showing that algorithms can match optimal thresholds for one-sided criteria and proving optimality of existing methods for false positives.


<details>
  <summary>Details</summary>
Motivation: To determine a sparse subset of defective items from a larger set using tests that indicate presence of at least one defective, with relaxed recovery criteria allowing some misclassifications.

Method: Analyzing group testing under one-sided approximate recovery criteria, comparing algorithms for false negatives and providing a converse bound for false positives.

Result: For false negatives, an algorithm matches the optimal threshold of two-sided approximate recovery. For false positives, the better of two existing algorithms is optimal.

Conclusion: The study contributes to understanding approximate group testing with one-sided errors, showing algorithmic efficiency and optimality under specific conditions.
...
### [294] [Learning Chaotic Dynamics with Neuromorphic Network Dynamics](https://arxiv.org/abs/2506.10773)
*Yinhao Xu,Georg A. Gottwald,Zdenka Kuncic*

Main category: cond-mat.dis-nn

TL;DR: This paper explores using a neuromorphic network, made of memristive circuits, to model dynamical systems through reservoir computing, finding that optimal learning occurs when input voltages maximize memristor dynamics range.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how neuromorphic networks can be used to learn and model dynamical systems by leveraging the physics of the underlying system, specifically through the use of memristive elements that mimic neuro-synaptic responses.

Method: The method involves using a complex electrical circuit with memristive elements as a neuromorphic network, simulated within a reservoir computing framework to autonomously predict multivariate chaotic time series.

Result: The results show that optimal nonlinear dynamical responses are achieved when input voltages maximize the memristive components' exploration of the dynamical range, and increasing electrode coverage suppresses less helpful nonlinear responses.

Conclusion: The conclusion is that these findings offer insights into optimizing neuromorphic devices for learning complex dynamical systems via external control parameters like input voltages.
...
### [295] [Revisiting mean estimation over $\ell_p$ balls: Is the MLE optimal?](https://arxiv.org/abs/2506.10354)
...
### [296] [Estimating Signal-to-Noise Ratios for Multivariate High-dimensional Linear Models](https://arxiv.org/abs/2506.10370)
...
### [297] [A note on the properties of the confidence set for the local average treatment effect obtained by inverting the score test](https://arxiv.org/abs/2506.10449)
...
### [298] [Computational Complexity of Statistics: New Insights from Low-Degree Polynomials](https://arxiv.org/abs/2506.10748)
*Alexander S. Wein*
...
TL;DR: This paper introduces VERIFAI, a project focused on traceability and verification of natural language requirements using NLP, ontologies, similarity-based reuse, and AI to automate formal specification generation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address challenges in traceability and verification of formal specifications by automating the generation process and reusing existing software artefacts.

Method: The methods include Natural Language Processing, ontologies for domain description, similarity-based reuse of artefacts, and AI to guide the process of identifying and declaring specifications.

Result: The result is the development of VERIFAI, which provides support for automatic generation of formal specifications and traceability across software design, implementation, and verification stages.

Conclusion: The conclusion states that VERIFAI effectively tackles the challenges through its integrated approaches, offering a systematic way to ensure requirements are traceable and verifiable throughout the software lifecycle.
...
### [303] [Semi-Tensor-Product Based Convolutional Neural Networks](https://arxiv.org/abs/2506.10407)
*Daizhan Cheng*

Main category: eess.SY

TL;DR: This paper introduces a domain-based convolutional product (CP) combined with the semi-tensor product (STP) of vectors to develop a new STP-based CNN, which avoids junk information from padding in image and third-order signal identification.


<details>
  <summary>Details</summary>
Motivation: The motivation is to generalize the conventional inner product of vectors to allow different dimensions and avoid junk information caused by padding in CNNs.

Method: The method involves proposing a domain-based CP and combining it with STP of vectors to create a new CP for CNN development.

Result: The result is an STP-based CNN that effectively handles image and third-order signal identifications without using zero or other padding.

Conclusion: The conclusion states that the new CP method improves CNN performance by eliminating padding-related issues, showing potential for applications in image and third-order signal processing.
...
### [304] [Energy Aware Camera Location Search Algorithm for Increasing Precision of Observation in Automated Manufacturing](https://arxiv.org/abs/2506.10251)
...
### [305] [Discrete Audio Tokens: More Than a Survey!](https://arxiv.org/abs/2506.10274)
...
### [306] [Fine-Grained control over Music Generation with Activation Steering](https://arxiv.org/abs/2506.10225)
...
TL;DR: The paper introduces a method for fine-grained music generation control using inference-time interventions on MusicGen, enabling timbre, style, and genre transfer with improved performance via regression tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to achieve fine-grained control over music generation, allowing specific adjustments like timbre, style, and genre fusion beyond what global text prompts alone can provide.

Method: The method involves steering the residual stream and attention layer activations of MusicGen using linear probe weights, framed as a regression task to preserve directional information in activation space.

Result: The approach demonstrates improved performance in timbre transfer, style transfer, and genre fusion, combined with global conditioning from text prompts for enhanced control.

Conclusion: The paper concludes that inference-time interventions on MusicGen offer effective global and local control for music generation, with audio samples available on their demo page.
...
### [307] [PAL: Probing Audio Encoders via LLMs -- A Study of Information Transfer from Audio Encoders to LLMs](https://arxiv.org/abs/2506.10423)
...
### [308] [BNMusic: Blending Environmental Noises into Personalized Music](https://arxiv.org/abs/2506.10754)
