...
TL;DR: The paper introduces Perfecting Depth, a two-stage framework that enhances sensor depth by using diffusion models to detect unreliable regions and then refining them for better accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: Current sensor depth enhancement methods often struggle with unreliable measurements and artifacts, leading to less accurate depth maps. The need for a more reliable and artifact-free approach is critical for applications like autonomous driving and robotics.

Method: Perfecting Depth uses a two-stage approach: first, stochastic estimation to identify unreliable depth measurements and infer geometric structure through a training-inference domain gap; second, deterministic refinement to enforce structural consistency and pixel-level accuracy using an uncertainty map from the first stage.

Result: The method produces dense, artifact-free depth maps with improved reliability, showing effectiveness in diverse real-world scenarios. It also demonstrates robustness and scalability through theoretical analysis, experiments, and visualizations.

Conclusion: Perfecting Depth sets a new baseline for sensor depth enhancement, offering potential applications in autonomous driving, robotics, and immersive technologies due to its improved accuracy and reliability.
...
TL;DR: This paper introduces TADiSR, a diffusion-based SR framework with text-aware attention and joint segmentation decoders to improve text structural fidelity and legibility in real-world images.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address fidelity issues in generative models for image super-resolution, especially the distortion of textual structures in real-world degraded images.

Method: The method involves a novel diffusion-based SR framework called TADiSR, which incorporates text-aware attention mechanisms and joint segmentation decoders to recover text regions and natural details.

Result: The approach significantly improves text legibility in super-resolved images, achieving state-of-the-art results across multiple metrics and showing strong generalization to real-world scenarios.

Conclusion: The conclusion states that TADiSR effectively enhances structural fidelity of text and natural details in SR, outperforming existing methods and demonstrating robustness in real-world applications.
...
TL;DR: MARS is a radio map super-resolution method using CNNs and Transformers with multi-scale features and residual connections, showing better performance than baselines with low computational cost.


<details>
  <summary>Details</summary>
Motivation: Accurate radio map reconstruction from sparse measurements is challenging due to limitations in traditional methods and deep learning approaches that require detailed scene data.

Method: MARS combines CNNs and Transformers with multi-scale feature fusion and residual connections to capture global and local features for better reconstruction.

Result: MARS outperforms baseline models in MSE and SSIM across different scenes and antenna locations while maintaining low computational cost.

Conclusion: MARS demonstrates strong practical potential for radio map reconstruction due to its effectiveness and efficiency.
...
TL;DR: This paper explores how multimodal language models like LlaVA-Next represent images in their hidden activations, finding linearly decodable features for ImageNet classes and showing shared representations in deeper layers through SAEs.


<details>
  <summary>Details</summary>
Motivation: Understanding how multimodal models represent images in their hidden states to improve interpretability and cross-modal alignment.

Method: Fine-tuning language models on visual embeddings, analyzing residual streams, and training multimodal SAEs to create an interpretable feature dictionary.

Result: Linearly decodable image features for diverse ImageNet classes, causal features validated by targeted edits, and increased shared representations in deeper layers.

Conclusion: Multimodal models have disparate representations across modalities, but deeper layers show more shared features, suggesting potential for better cross-modal understanding.
...
TL;DR: This paper introduces an AI auto-labeling system for display panel defects using SegGPT and scribble annotations, improving IoU and recall with 60% coverage.


<details>
  <summary>Details</summary>
Motivation: Reduce manual annotation efforts in industrial inspection systems.

Method: Enhanced SegGPT with domain-specific techniques and scribble-based annotations via two-stage training.

Result: Achieved 0.22 average IoU increase, 14% recall improvement, and 60% auto-labeling coverage.

Conclusion: AI auto-labeling with SegGPT and scribbles matches human-labeled models, offering a practical solution for industrial inspection.
...
TL;DR: A new underwater image enhancement method using dual-stream architecture, combining physical models with capsule clustering for better performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Underwater images often suffer from poor visibility and distorted colors due to light absorption and scattering. Existing methods may not efficiently handle these issues while preserving details.

Method: The paper introduces a dual-stream network: one stream estimates transmission maps and background light using the Jaffe-McGlamery model, while the other uses capsule clustering to extract entity-level features. The method integrates physical constraints with feature learning.

Result: The proposed method achieves +0.5dB PSNR improvement over existing methods with one-third the computational complexity, or +1dB improvement compared to methods with similar budgets.

Conclusion: The physics-guided dual-stream approach effectively enhances underwater images by balancing physical accuracy and perceptual quality, offering significant efficiency gains.
...
TL;DR: This paper introduces a modified SSIM loss function using addition instead of multiplication for better performance in unsupervised monocular depth estimation.


<details>
  <summary>Details</summary>
Motivation: Previous works on unsupervised monocular depth learning use MAE and SSIM but neglect the impact of SSIM components and hyperparameters.

Method: Proposed a new SSIM form by combining luminance, contrast, and structural components with addition, leading to smoother gradients and improved performance.

Result: The optimized SSIM loss function outperforms the baseline on KITTI-2015 dataset when integrated with MonoDepth.

Conclusion: The new SSIM form with additive combination of components enhances unsupervised depth estimation by addressing limitations in traditional SSIM usage.
...
TL;DR: This paper introduces LotusFilter, a post-processing module that diversifies ANNS results by removing redundant vectors using a precomputed cutoff table, showing fast performance in RAG applications.


<details>
  <summary>Details</summary>
Motivation: In some RAG applications, ANNS results need to be both similar to the query and diverse, but current methods may produce overly similar results.

Method: LotusFilter uses a precomputed cutoff table to greedily delete redundant vectors from ANNS candidates during filtering.

Result: LotusFilter achieves 0.02 ms/query performance in real-world RAG settings with OpenAI embeddings, and the code is publicly available.

Conclusion: LotusFilter effectively diversifies ANNS results while maintaining speed, making it suitable for RAG applications where diversity is important.
...
TL;DR: Spike-TBR is a new way to convert event streams into something that works with normal computer vision stuff, using spiking neurons to handle noise better.


<details>
  <summary>Details</summary>
Motivation: Traditional sensors have issues with noise when converting event streams, and there's a need for a better method that's robust and compatible with existing systems.

Method: Spike-TBR uses Temporal Binary Representation combined with spiking neural networks to create a noise-resistant encoding strategy.

Result: Evaluated four versions of Spike-TBR on different datasets, showed better performance in noisy situations and also improved results on clean data.

Conclusion: Spike-TBR helps connect spike-based and frame-based processing, providing a simple and resilient solution for event-driven vision tasks.
...
TL;DR: This paper demonstrates adversarial patch attacks on traffic light detectors in autonomous vehicles using printed patches, showing successful label-flipping and pictogram classification attacks in both lab and real-world settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of research on adversarial attacks specifically targeting traffic light detectors in autonomous vehicles, which are critical for safe navigation.

Method: The method involves proposing a threat model where printed patches are placed under traffic light instances to attack CNNs used for traffic light detection, along with a training strategy for creating these adversarial patches.

Result: The result shows successful adversarial patch attacks in universal settings, including realistic targeted red-to-green label-flipping and attacks on pictogram classification, validated through lab and real-world experiments.

Conclusion: The conclusion emphasizes the vulnerability of traffic light detection systems to adversarial attacks with printed patches, highlighting the need for robust defense mechanisms in autonomous vehicle perception systems.
...
TL;DR: This paper introduces OpenMaskDINO3D, a LLM for 3D understanding and segmentation, using point cloud data and text prompts to generate accurate instance masks, validated on ScanNet datasets.


<details>
  <summary>Details</summary>
Motivation: Current perception systems excel in 2D reasoning segmentation but depend on human instruction or predefined categories, lacking a comparable 3D framework.

Method: Introduce a SEG token and object identifier for high-precision 3D segmentation, allowing direct generation of masks from natural language instructions.

Result: OpenMaskDINO3D shows effectiveness in 3D tasks with accurate segmentation masks on ScanNet datasets.

Conclusion: The paper concludes that OpenMaskDINO3D provides a comprehensive 3D understanding and segmentation framework, filling the gap in 3D reasoning segmentation.
...
TL;DR: The paper introduces a tensor completion approach for reconstructing geological fields from sparse data, outperforming ordinary kriging.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reconstruction of multidimensional geological fields from sparse observations by leveraging tensor completion and geostatistics.

Method: The method involves modeling the spatial field as a multidimensional tensor and enforcing a global low-rank structure to recover missing values.

Result: Experiments show that the tensor completion method significantly improves reconstruction accuracy compared to ordinary kriging across different observed data percentages.
...
TL;DR: The paper introduces a transformer encoder trained with supervised contrastive learning to model human intuition in chess, enabling move selection through latent space planning without deep search, achieving high performance with limited search depth.


<details>
  <summary>Details</summary>
Motivation: To model human intuition-driven move selection in chess, which differs from traditional deep search approaches used by modern engines.

Method: Training a transformer encoder via supervised contrastive learning to embed board states into a latent space structured by positional evaluation, allowing move selection by navigating towards favorable regions.

Result: The model achieved an Elo rating of 2593 using only a 6-ply beam search, with performance improving as model size and embedding dimensions increased.

Conclusion: Latent planning via embeddings could serve as a viable alternative to traditional search methods in chess and other perfect-information games with learnable state evaluations.
...
TL;DR: The paper discusses the importance of creating digital twins for heritage objects, emphasizing that no single 3D digitization method works for all cases. It advocates for a collaborative approach between heritage experts and digital professionals to select the best tool for each object based on its features and intended use.


<details>
  <summary>Details</summary>
Motivation: The motivation is to highlight the necessity of digitizing heritage objects and the challenge of selecting appropriate 3D digitization methods. The paper aims to show that a one-size-fits-all approach is ineffective and that collaboration between different fields is essential for successful digital twin creation.

Method: The paper examines various photographic 3D digitization methods and uses examples from the mus{\'e}e d'Arch{\'e}ologie nationale's collections to analyze their suitability. It does not present a new method but evaluates existing ones through case studies.

Result: The result is that no single 3D digitization method is universally applicable. The effectiveness of a method depends on the object's characteristics and the intended use of its digital twin, necessitating tailored solutions and adaptation of tools.

Conclusion: The conclusion is that instead of trying to classify 3D digitization methods absolutely, there is a need to find the most suitable digital tool for each heritage object, considering both its physical attributes and the future applications of the digital twin.
...
TL;DR: APVR (Adaptive Pivot Visual information Retrieval) is a training-free framework that uses hierarchical visual info retrieval to handle hour-long videos efficiently, showing significant performance improvements on benchmarks like LongVideoBench and VideoMME.


<details>
  <summary>Details</summary>
Motivation: Current video-based multimodal large language models face challenges with hour-level video understanding due to computational limits and inefficiency in extracting info from long sequences.

Method: APVR uses two components: Pivot Frame Retrieval (semantic expansion + multi-modal confidence scoring) to find relevant frames, and Pivot Token Retrieval (query-aware attention-driven selection) within those frames for efficient processing.

Result: Experimental results on LongVideoBench and VideoMME show APVR achieves state-of-the-art performance, improving both training-free and training-based methods with plug-and-play compatibility.

Conclusion: APVR's dual approach of hierarchical retrieval and attention-driven selection effectively addresses memory and computational challenges, enabling efficient understanding of long videos without retraining existing models.
...
TL;DR: This workshop focuses on making facial affect analysis systems more trustworthy through fairness, explainability, and safety.


<details>
  <summary>Details</summary>
Motivation: As Emotion AI tools become more common, ensuring their reliability and ethical use is crucial.

Method: Bringing together researchers to discuss challenges in trustworthiness like interpretability, uncertainty, biases, and privacy.

Result: Promotes research and dialogue on trustworthy FAA to align with ethical standards.

Conclusion: The workshop supports FG2025's ethical focus by addressing key issues in FAA systems.
...
TL;DR: ASTRAEA is an automatic framework for optimizing video diffusion transformers, achieving significant speedups with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: To address the high computational demands of video diffusion transformers in practical deployment by reducing workload without sacrificing generation quality.

Method: ASTRAEA uses a lightweight token selection mechanism and a memory-efficient, GPU-parallel sparse attention strategy, combined with an evolutionary algorithm for optimal token reduction distribution.

Result: Up to 2.4x speedup on a single GPU and 13.2x on 8 GPUs, with less than 0.5% loss in VBench score compared to baseline vDiT models.

Conclusion: ASTRAEA provides an effective solution for accelerating video generation while maintaining high video quality, showing strong scalability and applicability.
...
TL;DR: This paper introduces FRED, a new dataset combining RGB and event cameras for drone detection, tracking, and trajectory forecasting, with 7+ hours of annotated data under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB cameras struggle with fast-moving drones in poor lighting; event cameras have potential but lack suitable benchmarks.

Method: Created FRED dataset with RGB video and event streams, annotated trajectories, and evaluation protocols.

Result: FRED provides a comprehensive benchmark for high-speed drone perception and multimodal spatiotemporal tasks.

Conclusion: The authors aim for FRED to advance research in drone perception and multimodal understanding, addressing current benchmark limitations.
...
TL;DR: This paper introduces TAO, a new framework for granular video anomaly detection that tracks anomalous objects at the pixel level, improving accuracy and eliminating the need for threshold tuning.


<details>
  <summary>Details</summary>
Motivation: Existing VAD methods focus on objects or frames, but neglect finer-grained analysis like anomalous pixels, limiting their ability to detect a broader range of anomalies.

Method: TAO framework integrates multiple fine-grained anomalous object detection into a unified pipeline, transforming the problem into pixel-level tracking of anomalies by linking scores to segmentation and tracking tasks.

Result: Experiments show TAO achieves new benchmarks in accuracy and robustness for video anomaly detection.

Conclusion: TAO provides a more precise and efficient approach for detecting anomalies in complex video sequences without threshold tuning.
...
TL;DR: The paper introduces VLG, a vision-language-garment model for generating garments from text and images, showing good zero-shot transfer to new styles and prompts.


<details>
  <summary>Details</summary>
Motivation: To explore how well multimodal foundation models can transfer knowledge to specialized domains like garment generation.

Method: Introduce VLG, which combines textual and visual inputs to synthesize garments.

Result: Preliminary results show VLG has good zero-shot generalization, transferring web-scale reasoning to unseen garment styles and prompts.

Conclusion: Multimodal foundation models can effectively adapt to specialized domains such as fashion design, suggesting future potential in this area.
...
TL;DR: This paper addresses the challenge of generalisation in presentation attack detection (PAD) for ID cards across different countries by leveraging Foundation Models (FMs) trained on large datasets, showing that bona fide images are crucial for effective adaptation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitation of PAD systems that are typically trained on a limited number of ID documents due to privacy concerns, leading to poor performance when applied to new, unknown countries.

Method: The method involves using Foundation Models (FMs) and evaluating their performance through different test protocols, including zero-shot and fine-tuning, on two ID card datasets: a private Chilean dataset and an open-set dataset from Finland, Spain, and Slovakia.

Result: The results show that FMs can improve generalisation capabilities for PAD in ID documents, with the key factor being the availability of bona fide images.

Conclusion: The conclusion is that Foundation Models are effective in adapting PAD systems to new ID card countries, and the inclusion of genuine images is essential for achieving good generalisation.
...
TL;DR: The paper introduces Rectified Point Flow, a method that unifies point cloud registration and shape assembly into a single generative problem, learning symmetries without labels and achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a unified approach for point cloud registration and multi-part shape assembly, addressing limitations in existing methods that require symmetry labels and struggle with joint training across datasets.

Method: The method involves learning a continuous point-wise velocity field to move noisy points to their target positions, along with a self-supervised encoder focusing on overlapping points to handle symmetries inherently.

Result: The method achieves state-of-the-art performance on six benchmarks covering both pairwise registration and shape assembly tasks, demonstrating effectiveness in joint training and accuracy improvement.

Conclusion: The conclusion states that the unified formulation of Rectified Point Flow enables effective joint training across diverse datasets, leading to better geometric priors and enhanced accuracy in point cloud registration and shape assembly.
...
TL;DR: The paper introduces a framework for long-term consistent video world models using geometry-grounded spatial memory, showing improvements in quality and consistency.


<details>
  <summary>Details</summary>
Motivation: To address the issue of scene inconsistency in video world models during revisits due to limited temporal context windows, inspired by human memory mechanisms.

Method: A novel framework with mechanisms for storing and retrieving information from a long-term spatial memory, using custom datasets with explicitly stored 3D memory mechanisms.

Result: Improved quality, consistency, and context length compared to baselines.

Conclusion: The approach paves the way for long-term consistent world generation by leveraging geometry-grounded spatial memory.
...
TL;DR: RaySt3R addresses 3D shape completion limitations by using novel view synthesis with a transformer, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing 3D shape completion methods lack 3D consistency, are computationally expensive, and struggle with sharp object boundaries.

Method: RaySt3R recasts 3D shape completion as a novel view synthesis problem, using a feedforward transformer to predict depth maps, object masks, and confidence scores from a single RGB-D image and query rays.

Result: RaySt3R achieves state-of-the-art performance with up to 44% improvement in 3D chamfer distance over baselines on synthetic and real-world datasets.

Conclusion: By fusing predictions across multiple query views, RaySt3R effectively reconstructs complete 3D shapes, addressing key limitations in current methods.
...
TL;DR: The paper proposes 3D-aware pseudo-labeling to improve semantic correspondence estimation in computer vision, achieving better results than previous methods without requiring dataset-specific annotations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of finding correspondences between semantically similar points in images and object instances, especially for symmetric objects or repeated parts where existing methods fail.

Method: The method involves training an adapter to refine features using pseudo-labels generated through 3D-aware chaining, relaxed cyclic consistency for filtering, and 3D spherical prototype mapping constraints.
...
Conclusion: The generality of the approach allows easy extension to other data sources, as shown in the experiments.
...
TL;DR: MARBLE is a method for material editing in images using CLIP-space embeddings and pre-trained text-to-image models, allowing for blending and controlling fine-grained material properties through a denoising UNet and a shallow network for parametric control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance exemplar-based material editing by leveraging CLIP-space embeddings and pre-trained models to achieve more accurate and controllable material blending and attribute manipulation.

Method: MARBLE uses CLIP-space embeddings to find material directions for blending and employs a denoising UNet to identify the block responsible for material attribution. A shallow network predicts directions for specific material attributes like roughness and metallic.

Result: The method shows effectiveness through qualitative and quantitative analysis, enabling multiple edits in a single pass and application in painting.

Conclusion: MARBLE provides a novel approach for material editing in images by combining CLIP-space analysis with pre-trained models, offering improved control over material properties and potential for various applications.
...
TL;DR: The paper proposes PM-Loss, a regularization loss using a pre-trained transformer's pointmap to improve depth maps in 3D Gaussian Splatting, enhancing rendering quality by enforcing geometric smoothness, especially at object boundaries.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address depth discontinuities in 3DGS pipelines, which lead to fragmented point clouds and degrade rendering quality. The goal is to improve geometric smoothness without sacrificing efficiency or accuracy.

Method: The method involves introducing PM-Loss, a regularization loss that leverages a pointmap from a pre-trained transformer. This pointmap, though less accurate than depth maps, helps enforce geometric smoothness, particularly around object boundaries. The approach combines the benefits of depth maps with improved regularization to enhance 3DGS performance.

Result: The results show that PM-Loss significantly improves feed-forward 3DGS across different architectures and scenes, leading to consistently better rendering outcomes. The method addresses the issue of fragmented point clouds caused by depth discontinuities, resulting in higher quality 3D reconstructions.

Conclusion: The paper concludes that PM-Loss is an effective solution for enhancing 3DGS by improving depth maps through geometric regularization. It offers a practical approach to overcome the limitations of depth-based representations, providing better rendering quality without compromising training efficiency or camera pose utilization.
...
TL;DR: Pipeline for generating defurnished indoor space replicas using mesh segmentation, plane extension, and ControlNet inpainting for high-quality assets.


<details>
  <summary>Details</summary>
Motivation: To create high-quality defurnished indoor space replicas that overcome limitations of existing methods like neural radiance fields and RGB-D inpainting.

Method: Segments and removes furniture from textured meshes, extends planes, fills holes to create SDM, extracts Canny edges from SDM depth and normal images, uses ControlNet inpainting to remove furniture from panoramas, and textures the mesh with inpainted panoramas.

Result: Higher quality assets compared to neural radiance fields and RGB-D inpainting methods, with clearer structures and fewer hallucinations.

Conclusion: The proposed pipeline effectively generates defurnished indoor space replicas by leveraging geometric information and ControlNet inpainting, providing a robust solution for scene reconstruction.
...
TL;DR: ContentV is an 8B-parameter text-to-video model that achieves SOTA performance with efficient training.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the growing computational costs in video generation by developing more efficient training methods.

Method: The method includes a minimalist architecture, multi-stage training with flow matching, and a cost-effective RLHF framework.

Result: The result is a model that generates diverse, high-quality videos across multiple resolutions and durations.

Conclusion: The conclusion is that ContentV demonstrates significant efficiency and performance improvements in text-to-video generation.
...
TL;DR: FreeTimeGS is a novel 4D representation for dynamic 3D scene reconstruction, offering better flexibility and motion modeling than existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of handling complex motions in dynamic 3D scenes, which current methods struggle with due to difficulties in optimizing deformation fields.

Method: FreeTimeGS introduces a 4D representation where Gaussian primitives can appear at any time and location, and each primitive has a motion function to move to neighboring regions, reducing temporal redundancy.

Result: Experiments show that FreeTimeGS achieves significantly better rendering quality compared to recent methods.

Conclusion: FreeTimeGS improves the modeling of dynamic 3D scenes through its flexible 4D representation and motion functions, leading to superior rendering results.
...
TL;DR: ProLoRA enables zero-shot parameter-efficient fine-tuning for text-to-image diffusion models by transferring pre-trained low-rank adjustments without needing additional data.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for adapting text-to-image diffusion models require retraining when switching base models, which is challenging due to data constraints. ProLoRA aims to overcome these limitations by enabling zero-shot adaptation.

Method: ProLoRA transfers pre-trained low-rank adjustments (like LoRA) from a source model to a target model by projecting them into the target model's weight space, utilizing subspace and null space similarities and focusing on aligned layers.

Result: Evaluations on established text-to-image models show that ProLoRA successfully transfers knowledge without retraining, achieving performance comparable to traditional methods.

Conclusion: ProLoRA provides an effective solution for zero-shot adaptation in text-to-image diffusion models, eliminating the need for retraining and addressing data constraints.
...
TL;DR: This paper addresses contextual integrity in autonomous agents by using LLMs and a reinforcement learning framework to reduce inappropriate information disclosure without compromising task performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure that autonomous agents share the appropriate information in different contexts, which is crucial as they make decisions on behalf of users.

Method: The method involves prompting LLMs to explicitly reason about contextual integrity and developing a reinforcement learning framework to enhance this reasoning.

Result: The results show that the proposed method reduces inappropriate information disclosure while maintaining task performance across various model sizes and families, and improvements transfer to real-world benchmarks like PrivacyLens.

Conclusion: The conclusion states that instilling contextual integrity reasoning in models through explicit prompting and RL frameworks is effective and transferable to established benchmarks.
...
TL;DR: This paper proposes a statistical physics framework to understand the continuous-time chain-of-thought reasoning dynamics in Transformer LMs, revealing latent regimes and enabling low-cost simulations for predicting critical transitions.


<details>
  <summary>Details</summary>
Motivation: To provide a mechanistic understanding of the emergent reasoning in Transformer LMs, which currently resists such analysis.

Method: Modeling sentence-level hidden state trajectories as a stochastic dynamical system on a lower-dimensional manifold with latent regime switching.

Result: A rank-40 projection explains ~50% variance in empirical trajectories, and four latent reasoning regimes are identified.

Conclusion: The proposed framework allows for low-cost reasoning simulations and provides tools to study and predict critical transitions in LM behavior.
...
TL;DR: The paper proposes a graph-based framework for multi-table QA using human-curated relational knowledge to improve schema linking and handle complex datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of extending LLMs to multi-table QA due to unreliable schema linking in complex tables.

Method: The method involves a graph-based framework that uses human-curated relational knowledge to encode schema links and join paths, along with pruning and sub-path merging strategies for efficiency and coherence.

Result: Experiments show the approach is effective on standard benchmarks and a realistic, large-scale dataset, making it the first multi-table QA system for complex industrial tabular data.

Conclusion: The conclusion is that the graph-based framework effectively enhances multi-table QA by explicitly encoding schema links, leading to better performance in real-world scenarios.
...
TL;DR: The paper proposes a ranking-based monitoring system using AI anomaly detection to improve public health data analysis, showing a 54x increase in efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional alert-based systems are inefficient for large health data monitoring due to needing frequent threshold adjustments and data volume issues.

Method: Developed a ranking-based paradigm with AI anomaly detection through interdisciplinary collaboration.

Result: Deployed system at a national organization, monitoring 5 million data points daily, with 54x faster reviewer efficiency in a 3-month study.

Conclusion: Human-centered AI can significantly enhance public health decision-making.
...
TL;DR: This paper explores automatic schema generation using large language models (LLMs) and introduces two datasets, YAGO Schema and Wikidata EntitySchema, along with evaluation metrics. Experiments show LLMs can generate high-quality ShEx schemas, enabling scalable automated schema creation for large knowledge graphs (KGs) and presenting a new challenge for structured generation in syntactically rich formalisms.


<details>
  <summary>Details</summary>
Motivation: Schemas are crucial for data quality in the Semantic Web and NLP, but traditional creation requires significant expert input. LLMs have shown promise in related tasks like ontology engineering, suggesting potential for automated schema generation.

Method: The paper introduces two datasets (YAGO Schema and Wikidata EntitySchema) and evaluation metrics to assess LLMs' ability to generate schemas in Shape Expressions (ShEx). It explores LLM-based pipelines that leverage KG information to create validating schemas.

Result: Experiments indicate that LLMs can generate high-quality ShEx schemas by effectively using local and global information from KGs, suggesting scalability for large KGs.

Conclusion: LLMs have strong potential in automated schema generation, offering a pathway for scalable solutions. The introduced benchmark presents a new challenge for structured generation in syntactically rich formalisms.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to systematically analyze judicial permissions in criminal trials, highlighting their significance.

Method: The method involves introducing a dialogue game model to address judicial permissions, considering different standards of proof and argumentation semantics.

Result: The result is a systematic approach to handling judicial permissions through the dialogue game model.

Conclusion: The conclusion states that the dialogue game model provides an effective framework for analyzing judicial permissions in criminal trials.
...
TL;DR: The paper discusses the variability in benchmark evaluation results of Deepseek-R1-Distill series models and other similar models, highlighting the need for a more rigorous evaluation paradigm.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of inconsistent performance evaluation results in open-source models based on Deepseek-R1-Distill, which affects the reliability of their claimed improvements.

Method: The method involves analyzing the variability in benchmark results and conducting empirical assessments of the Deepseek-R1-Distill series models.

Result: The results show that evaluation results for Deepseek-R1-Distill series models and others like QwQ-32B vary significantly with slight changes in evaluation conditions, making reliable reproduction of performance improvements difficult.

Conclusion: The conclusion emphasizes the need for a more rigorous evaluation paradigm to ensure consistent and reliable model performance assessments.
...
TL;DR: This paper presents an ontology-based framework for bone disease diagnosis using a hierarchical neural network, ontology-enhanced VQA, and multimodal deep learning, aiming to integrate domain expertise for reliable medical AI systems.


<details>
  <summary>Details</summary>
Motivation: Medical AI systems often lack systematic domain expertise integration, which can affect diagnostic reliability. The motivation is to develop a framework that better incorporates clinical knowledge for more accurate bone disease diagnosis.

Method: The framework includes a hierarchical neural network guided by a bone disease ontology, an ontology-enhanced VQA system for clinical reasoning, and a multimodal deep learning model integrating imaging, clinical, and lab data via ontological relationships. It emphasizes clinical interpretability through knowledge digitization, terminology mapping, and modular design.

Result: The framework shows potential for extension beyond bone diseases due to its standardized structure and reusable components, though experimental validation is pending due to dataset and resource limitations.

Conclusion: The study establishes theoretical foundations for integrating domain expertise into medical AI, but future work is needed to expand datasets and validate the system comprehensively.
...
TL;DR: This paper introduces a Markovian simulator for modeling interactions between cyber-defender and cyber-attacker agents in networked systems, aiming to create an experimental framework for assessing dynamic defense strategies against complex, coordinated attacks.


<details>
  <summary>Details</summary>
Motivation: As cyber-attacks become more complex and coordinated, traditional defense strategies may not be sufficient. Multi-agent approaches are needed to better tackle these threats at entry points in networked systems.

Method: The paper presents a Markovian model and a simulator where cyber-defender and cyber-attacker agents interact on network nodes. Network nodes are abstracted with properties, and agent actions determine how the network reacts, influencing proximity to attack or defense goals.

Result: The simulator was implemented with realistic scenarios, demonstrating how collective agent actions affect the environment's alignment with attack or defense objectives.

Conclusion: The proposed framework offers a practical way to test and improve dynamic cyber defense strategies by simulating coordinated attack and defense interactions.
...
TL;DR: The paper studies strategic deception in CoT-enabled LLMs, using LAT to detect deception with 89% accuracy and activation steering to induce it with 40% success rate.


<details>
  <summary>Details</summary>
Motivation: To address the critical alignment challenge of LLM honesty, especially as CoT reasoning may lead to intentional misinformation.

Method: Representation engineering with Linear Artificial Tomography (LAT) for inducing, detecting, and controlling deception in LLMs.

Result: Achieved 89% deception detection accuracy using LAT and 40% success rate in eliciting context-appropriate deception without explicit prompts.

Conclusion: The study reveals specific honesty-related issues in reasoning models and provides tools for trustworthy AI alignment.
...
TL;DR: This paper studies how training data variables affect ASR robustness to unseen accents, finding that more speakers (even with less audio per speaker) improves performance, especially when scaling hours.
...
Result: Increasing the number of speakers (with less audio per speaker) is more beneficial for ASR robustness than increasing audio duration per speaker. More speakers allow performance gains when scaling the number of training hours.

Conclusion: Practitioners should prioritize increasing speaker count in ASR training data for new languages to improve robustness against unseen accents.
...
TL;DR: This paper proposes a method to decompose sentence embeddings into interpretable components using dictionary learning on token-level representations, revealing linear semantic and syntactic encoding.


<details>
  <summary>Details</summary>
Motivation: The internal structure of sentence embeddings is not well understood, and their content is hard to trace due to complex neural transformations and pooling operations.
...
Result: The study revealed that semantic and syntactic aspects are linearly encoded in sentence embeddings, and the method bridges token-level interpretability with sentence-level analysis.

Conclusion: The proposed method enhances transparency and controllability of sentence embeddings by making their internal structure more interpretable.
...
TL;DR: This paper introduces EmoRAG, a system for multi-label emotion detection in texts without extra training, using an ensemble of models. It performs as well as top systems but is more efficient and scalable.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create an efficient and scalable system for detecting perceived emotions in text, avoiding the need for additional model training.

Method: The method involves using an ensemble of models for emotion detection without requiring additional training.

Result: EmoRAG achieves results comparable to the best systems while being more efficient and scalable.

Conclusion: The conclusion is that EmoRAG offers a competitive approach to emotion detection in texts with improved efficiency and scalability.
...
TL;DR: This paper introduces ZOES, a method for entity structure extraction without needing predefined schemas or annotated data, showing improved performance across domains.


<details>
  <summary>Details</summary>
Motivation: Existing LLM methods for entity structure extraction rely on predefined schemas or annotated datasets, leading to incomplete results. The motivation is to create a method that doesn't require these, improving extraction completeness.

Method: ZOES uses an enrichment, refinement, and unification mechanism, leveraging the mutual reinforcement between entities and their structures without schema or annotated samples.

Result: Experiments show ZOES enhances LLMs' ability to extract more complete entity structures in three domains, proving its effectiveness and generalizability.

Conclusion: The enrichment, refinement, and unification mechanism in ZOES offers a principled approach to improve LLM-based entity structure discovery across various scenarios.
...
TL;DR: This paper studies how well large language models (LLMs) understand the different meanings of the word 'just' in English, finding that they can grasp broad categories but miss finer nuances.


<details>
  <summary>Details</summary>
Motivation: Discourse particles like 'just' are important for text meaning, but their multiple functions make them hard to model. The paper aims to test LLMs' ability to capture these nuances.

Method: The study uses carefully created and labeled data by expert linguists to analyze how well LLMs distinguish the various senses of 'just'.

Result: LLMs show some ability to differentiate broader categories of 'just' but fail to fully capture the subtle nuances of its meaning.

Conclusion: The results highlight a gap in LLMs' understanding of discourse particles, suggesting areas for improvement in modeling nuanced language aspects.
...
TL;DR: The paper suggests that benchmarking large language models (LLMs) on unanswerable questions is valuable and presents a new benchmark with methods to adapt existing datasets, showing that current models struggle with such tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the effectiveness of LLMs in handling questions that lack reasonable answers, which can reveal model limitations and improve robustness.

Method: The method involves creating a benchmark for unanswerable questions by modifying existing datasets and evaluating LLMs on these tasks.

Result: The results show that existing models perform far from perfect on unanswerable questions, indicating room for improvement.

Conclusion: The conclusion is that benchmarking LLMs on unanswerable questions is a worthwhile endeavor to better understand and enhance model capabilities.
...
TL;DR: This paper introduces SUCEA, a modular framework for automatic fact-checking that improves accuracy by rephrasing adversarial claims into sub-claims and iteratively retrieving evidence.


<details>
  <summary>Details</summary>
Motivation: Fact-checking systems struggle with adversarial claims designed to challenge them, so a new method is needed to improve their accuracy.

Method: SUCEA uses three steps: Claim Segmentation and Decontextualization to split claims into sub-claims, Iterative Evidence Retrieval and Claim Editing to refine sub-claims with evidence, and Evidence Aggregation and Label Prediction to determine the final label.

Result: Experiments show SUCEA improves retrieval and entailment accuracy, outperforming four baselines on two datasets.

Conclusion: SUCEA is a effective training-free method for handling adversarial claims in fact-checking, enhancing the system's ability to detect misinformation.
...
TL;DR: LESS is a framework that uses LLMs to improve pseudo labels from ASR or AST, leading to better performance in speech tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance the accuracy of pseudo labels in semi-supervised learning for speech processing by utilizing LLMs.

Method: LESS refines pseudo-labeled text from ASR/AST using LLMs and applies a data filtering strategy to improve knowledge transfer efficiency.

Result: LESS reduced WER by 3.77% on Wenet Speech and improved BLEU scores to 34.0 and 64.7 on Callhome and Fisher test sets.

Conclusion: LESS is adaptable across languages, tasks, and domains, and provides insights into using LLMs for speech processing.
...
TL;DR: This paper introduces ADAPT, a lightweight method for Test-Time Scaling (TTS) that enhances reasoning performance in LLMs by focusing on generative diversity, achieving 80% accuracy with 8x less compute than strong baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve TTS effectiveness by addressing the issue of reduced output diversity in reasoning-optimized LLMs, which limits their performance during inference with additional compute.

Method: ADAPT (A Diversity Aware Prefix fine-Tuning) is a lightweight method that uses prefix tuning combined with a diversity-focused data strategy to enhance generative diversity.

Result: Experiments on mathematical reasoning tasks show ADAPT achieves 80% accuracy while using eight times less compute than strong baselines.

Conclusion: The conclusion is that generative diversity plays an essential role in maximizing TTS effectiveness, and ADAPT provides a promising approach to achieve this with reduced computational cost.
...
TL;DR: This paper introduces static word embeddings enhanced for sentence semantics through PCA, distillation, or contrastive learning, achieving strong performance with simple averaging.


<details>
  <summary>Details</summary>
Motivation: Existing static word embeddings are not optimized for sentence semantic tasks, while dynamic models like Sentence Transformers are computationally heavy. The goal is to create a more efficient alternative that maintains high performance.

Method: Extract word embeddings from a pre-trained Sentence Transformer, apply sentence-level PCA, then refine with knowledge distillation or contrastive learning. Inference uses simple averaging of word embeddings.

Result: The proposed model outperforms existing static models on monolingual and cross-lingual tasks, and rivals SimCSE on some datasets.

Conclusion: The method effectively removes irrelevant embedding components and adjusts vector norms based on word influence, providing an efficient alternative to dynamic models.
...
TL;DR: This paper proposes an automated method to create AVSR datasets from raw video, enabling robust speech recognition in noisy environments and expanding AVSR to under-resourced languages.


<details>
  <summary>Details</summary>
Motivation: AVSR models are limited by scarce datasets, especially for non-English languages. Automated data collection can address this issue.

Method: The paper presents a practical approach to generate AVSR datasets from raw video, refining existing techniques for efficiency and accessibility.

Result: The automatically collected dataset achieves competitive performance with robust ASR in clean conditions and significantly outperforms them in noisy environments.

Conclusion: The efficient method provides a pathway to expand AVSR to more languages, particularly under-resourced ones.
...
TL;DR: This paper introduces MMRefine, a benchmark for evaluating error refinement in multimodal large language models (MLLMs) across six scenarios and error types, identifying performance bottlenecks and areas for improvement.


<details>
  <summary>Details</summary>
Motivation: The motivation is to shift focus towards enhancing reasoning during inference by evaluating MLLMs' ability to detect and correct errors beyond just final accuracy.

Method: The method involves creating a benchmark with six distinct scenarios and categorizing errors into six types to analyze refinement performance.

Result: Experiments show bottlenecks and factors impeding refinement performance, highlighting areas for improvement in effective reasoning enhancement.

Conclusion: The conclusion is that MMRefine provides a framework for evaluating error refinement in MLLMs, aiding in the identification of key areas for development.
...
### [202] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
### [203] [Cracking the Code: Enhancing Implicit Hate Speech Detection through Coding Classification](https://arxiv.org/abs/2506.04693)
*Lu Wei,Liangzhi Li,Tong Xiang,Xiao Liu,Noa Garcia*

Main category: cs.CL

TL;DR: This paper introduces a new taxonomy for detecting implicit hate speech using codetypes, showing improved performance in both Chinese and English datasets with LLMs.


<details>
  <summary>Details</summary>
Motivation: The internet is a hotspot for hate speech, which threatens societal harmony and individual well-being. Automatic detection methods are effective for explicit hate speech but struggle with implicit forms.

Method: The paper presents two methods for integrating codetypes into im-HS detection: 1) prompting LLMs directly to classify sentences based on generated responses, and 2) using LLMs as encoders with codetypes embedded during encoding.

Result: Experiments demonstrate that codetypes enhance im-HS detection in both Chinese and English datasets, proving the approach's effectiveness across languages.

Conclusion: The study validates the effectiveness of the proposed codetypes for improving implicit hate speech detection using large language models.
...
TL;DR: This paper discusses the submission of IIITH-BUT to the IWSLT 2025 shared task for low-resource Bhojpuri-Hindi speech translation, focusing on hyperparameter optimization, data augmentation, and cross-lingual training.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve speech translation performance for low-resource languages by optimizing hyperparameters and using data augmentation techniques.

Method: The method involves hyperparameter optimization (learning rate schedules, update steps, warm-up steps, label smoothing, batch sizes) and data augmentation (speed perturbation, SpecAugment) for the SeamlessM4T model, along with cross-lingual training using Marathi and Bhojpuri speech data.

Result: The results show that careful hyperparameter selection and effective augmentation techniques significantly improve performance in low-resource settings, with analysis of translation errors affecting BLEU scores.

Conclusion: The conclusion is that hyperparameter optimization and data augmentation are effective for low-resource speech translation, and cross-lingual training provides additional benefits.
...
TL;DR: This paper evaluates text revision in scientific writing by analyzing limitations of traditional metrics and exploring alternative methods, finding that a hybrid approach combining LLMs and domain-specific metrics is most effective.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of evaluating text revisions in scientific writing, as traditional metrics like ROUGE and BERTScore focus on similarity rather than meaningful improvements.

Method: The method involves a manual annotation study, investigation of reference-free NLP metrics, and analysis of LLM-as-a-judge approaches for assessing revisions with and without a gold reference.

Result: Results show LLMs are effective at instruction-following but struggle with correctness, while domain-specific metrics offer complementary insights. A hybrid approach combining both is most reliable.

Conclusion: The conclusion is that combining LLM-as-a-judge evaluation with task-specific metrics provides the most reliable assessment of revision quality in scientific writing.
...
TL;DR: This paper evaluates the effectiveness of vision-language and large language models in grading and providing feedback on handwritten exam responses from Indonesian grade 4 students, highlighting challenges with handwriting recognition and limitations in personalized feedback.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the effectiveness of VLM and LLM in real-world classroom settings, especially in underrepresented educational contexts like Indonesia.

Method: The study evaluated a state-of-the-art VLM and several LLMs on 646 handwritten exam responses from grade 4 students in six Indonesian schools, covering Mathematics and English, with over 14K student answers including multiple choice, short answer, and essay questions.

Result: The VLM struggles with accurate handwriting recognition, causing errors in downstream LLM grading. However, LLM-generated feedback still has some utility despite imperfect input, with limitations in personalization and contextual relevance.

Conclusion: The paper concludes that while VLMs face challenges in recognizing handwriting, LLMs can still provide useful feedback, but improvements are needed for better personalization and contextual relevance in educational assessment.
...
TL;DR: This paper presents an LLM-based system for solving cryptic crosswords, achieving state-of-the-art results on the Cryptonite dataset by generating hypotheses, wordplay explanations, and using a verifier with codified reasoning steps.


<details>
  <summary>Details</summary>
Motivation: Cryptic crosswords are complex language tasks requiring both definition and wordplay understanding. Existing systems may lack interpretability and effectiveness on standard datasets.

Method: The system uses open-licensed LLM components to hypothesize answers, generate wordplay explanations, and verify solutions through codified reasoning steps in Python.

Result: It achieves the new state-of-the-art performance on the Cryptonite dataset, which includes clues from The Times and The Telegraph.

Conclusion: The approach provides interpretable wordplay reasoning for solved clues, enhancing transparency and reliability in cryptic crossword solving.
...
TL;DR: This paper challenges the belief that fine-tuning multilingual E2E models is the best approach for ASR in low-resource languages, showing that combining hybrid HMMs with self-supervised models can significantly improve performance with limited data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to question the common belief that fine-tuning existing multilingual end-to-end (E2E) models is the most effective method for developing ASR systems for low-resource languages, especially when the target language wasn't in the original training data.
...
Result: The approach achieved a 32% relative reduction in Word Error Rate (WER) compared to the best fine-tuned Whisper model on Scottish Gaelic.

Conclusion: The paper concludes that hybrid HMMs with self-supervised models can outperform fine-tuned multilingual E2E models for ASR in low-resource scenarios, suggesting a need to reconsider the dominant approach.
...
TL;DR: This paper introduces ConECT, a Czech-to-Polish e-commerce dataset with images and metadata, and shows that adding context (like images or product info) improves NMT quality.


<details>
  <summary>Details</summary>
Motivation: Neural Machine Translation (NMT) struggles with word ambiguity and context, especially in domain-specific applications like e-commerce where sentences may be unclear or data poor.

Method: Created ConECT dataset with 11,400 sentence pairs, images, and metadata. Tested vision-language models (VLMs) and incorporated contextual info (category path, image descriptions) into text-to-text models.

Result: Incorporating contextual information (visual or product metadata) improved machine translation quality in e-commerce tasks.

Conclusion: Adding contextual information enhances NMT performance, and the ConECT dataset is made publicly available for further research.
...
TL;DR: The paper evaluates LLMs on university-level algorithms exams, finding they perform well but struggle with graph tasks, and suggests using them for educational content.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' effectiveness in educational settings, particularly in algorithm exams and feedback generation.

Method: Testing state-of-the-art LLMs on Romanian and English algorithm exams to evaluate problem-solving and multilingual skills.

Result: Recent LLMs match top student scores and show strong reasoning, but have issues with graph-based problems.

Conclusion: LLMs can aid education by creating quality content and improving feedback, suggesting future integration in algorithm teaching.
...
TL;DR: The paper proposes an incremental semi-supervised learning pipeline for fine-tuning ASR models in domains with scarce labeled data, using auxiliary datasets and filtering pseudo-labels via multi-model consensus or NER to achieve significant relative improvements in performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of fine-tuning pretrained ASR models for specific domains when labeled data is scarce, leveraging available unlabeled audio and labeled data from related domains.

Method: The method involves an incremental semi-supervised learning pipeline that first combines a small in-domain labeled set with an auxiliary dataset from a related domain, then applies filtering based on multi-model consensus or NER to select and refine pseudo-labels iteratively.

Result: The pipeline achieves a relative improvement of 4% over no auxiliary data, with consensus-based filtering providing up to 22.3% relative improvement on Wow and 24.8% on Fisher compared to single-step fine-tuning with random selection. NER is also effective but with lower computational cost.

Conclusion: The paper concludes that incremental semi-supervised learning with consensus-based filtering is effective for domain-specific ASR model fine-tuning, offering better performance saturation and significant improvements over single-step methods.
...
TL;DR: The paper introduces a simple method to control the length of text summaries by prioritizing the EOS token in the loss function, compatible with various models and not affecting summary quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controlling text generation length require complex model changes, making them incompatible with pre-trained models. The need for a simpler, more compatible approach is addressed here.

Method: The method involves adjusting the cross-entropy loss by increasing the weight of the EOS token prediction, allowing control over summary length without altering the model architecture or decoding algorithms.

Result: The approach effectively controls the generation length across different models like encoder-decoder and GPT-style LLMs, often maintaining summary quality.

Conclusion: The proposed method is architecture-agnostic, works with various decoding algorithms, and is orthogonal to other techniques, offering a flexible and efficient way to manage text summary lengths without compromising quality.
...
TL;DR: This paper introduces TALL, a method that enhances LLM performance in low-resource languages by integrating with bilingual translation models and using dimension alignment layers and custom transformers.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) perform well in high-resource languages but have limitations in low-resource languages due to lack of training data. The motivation is to address this gap by improving LLM performance for low-resource languages.

Method: TALL integrates an LLM with two bilingual translation models. It uses dimension alignment layers and custom transformers to transform low-resource inputs into high-resource representations, preserving linguistic features.

Result: Experiments on Hebrew showed significant improvements over baselines like direct use, naive translation, and fine-tuning approaches. The architecture is parameter-efficient, freezing pre-trained components and training only lightweight adapters.

Conclusion: TALL effectively enhances LLM performance for low-resource languages by combining translation models with LLMs, using efficient adaptation strategies to balance computational resources and performance.
...
TL;DR: Introduce Debate Speech Evaluation as a new benchmark for LLM judges, highlighting differences in judgment behavior between large models and humans.


<details>
  <summary>Details</summary>
Motivation: To address the lack of systematic benchmarking for cognitive abilities required in evaluating debate speeches.

Method: Used a dataset of 600+ annotated debate speeches to analyze how LLMs compare to humans in judgment tasks.

Result: Larger models can mimic some human judgments but show significant differences in overall judgment behavior.

Conclusion: LLMs may achieve human-level performance in generating persuasive speeches but vary in evaluative judgments.
...
TL;DR: This paper discusses how to interpret self-reports from large language models (LLMs) and examines two examples to determine if they demonstrate introspection, suggesting one may be a minimal example without consciousness.


<details>
  <summary>Details</summary>
Motivation: To understand the implications of LLMs' self-reports on their potential introspection and consciousness, given their advanced linguistic and cognitive abilities.

Method: Presenting and critiquing two instances of self-reports from LLMs to assess whether they qualify as introspection.

Result: One example of an LLM describing its creative writing process was not valid introspection, while another where it inferred its temperature parameter was considered a minimal introspection example without conscious experience.

Conclusion: The concept of introspection may be applied to LLMs in a minimal sense, but it's unclear if it involves conscious experience.
...
TL;DR: This paper introduces Knowledgeable-r1, a novel RAG method that balances parametric and contextual knowledge by using joint sampling and multi-policy distributions, leading to improved robustness and reasoning accuracy, especially in counterfactual scenarios.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems overemphasize retrieved contexts, leading to reliance on inaccurate sources and underutilization of the model's inherent knowledge, particularly when dealing with misleading or excessive information.

Method: The paper proposes Knowledgeable-r1, which employs joint sampling and defines multi-policy distributions to encourage large language models to better integrate parametric and contextual knowledge.

Result: Experiments demonstrate that Knowledgeable-r1 significantly improves robustness and reasoning accuracy in parameter and contextual conflict tasks as well as general RAG tasks, outperforming baselines by 17.07% in counterfactual scenarios with consistent gains across RAG tasks.

Conclusion: The study concludes that balancing parametric and contextual knowledge through Knowledgeable-r1 enhances RAG system performance, particularly in challenging scenarios, and the code is available on GitHub.
...
TL;DR: This paper uses mechanistic interpretability to study how biases are structured in LLMs like GPT-2 and Llama2, finding that bias-related computations are localized in specific layers and removing them impacts other NLP tasks.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) often exhibit social, demographic, and gender biases due to their training data. Understanding how these biases are structurally represented is crucial for mitigating them effectively.

Method: The paper employs a mechanistic interpretability approach to analyze biases in LLMs. It explores various metrics to identify internal edges responsible for biased behavior and evaluates the stability, localization, and generalizability of these components across different datasets and languages.

Result: Bias-related computations are highly localized, often confined to a small subset of layers. The identified components vary with fine-tuning settings, even those not related to bias. Removing these components reduces biased outputs but also affects other NLP tasks like named entity recognition and linguistic acceptability judgment.

Conclusion: The study shows that biases in LLMs are structurally represented in specific layers, and targeted removal of these components can mitigate bias but may also have unintended effects on other tasks due to shared components.
...
TL;DR: This paper explores self-supervised auxiliary tasks for morphological inflection in low-resource languages, finding that autoencoding works best with very limited data and CMLM improves with more data, while morpheme-aware masking enhances performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of large-scale unlabeled data for many languages by exploring self-supervised methods for character-level tasks like morphological inflection, which is crucial for language documentation.

Method: The method involves training encoder-decoder transformers on 19 languages with 13 different self-supervised auxiliary tasks, comparing the effectiveness of autoencoding and CMLM under varying data conditions.

Result: Autoencoding performs best with very limited unlabeled data, while CMLM becomes more effective as data increases. Morpheme boundary-based sampling of masks consistently improves results.

Conclusion: The conclusion is that self-supervised learning is effective for low-resource morphological inflection, with morpheme-aware masking showing particular promise despite stronger inductive biases not always outperforming standard CMLM.
...
TL;DR: ProRefine improves agentic workflows by optimizing prompts during inference, boosting accuracy and enabling smaller models to rival larger ones.


<details>
  <summary>Details</summary>
Motivation: To address error propagation and sub-optimal performance in agentic workflows caused by poor prompts.

Method: ProRefine uses textual feedback from LLMs to dynamically refine prompts for multi-step reasoning tasks without extra training.

Result: ProRefine outperforms zero-shot Chain-of-Thought baselines by 3-37 percentage points on five math reasoning datasets.

Conclusion: ProRefine enhances accuracy and scalability, making high-performing AI more accessible with efficient model deployment.
...
### [258] [Classification of Extremal Dependence in Financial Markets via Bootstrap Inference](https://arxiv.org/abs/2506.04656)
...
### [259] [A dimension reduction for extreme types of directed dependence](https://arxiv.org/abs/2506.04825)
...
### [260] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
...
### [261] [At the edge of Donsker's Theorem: Asymptotics of multiscale scan statistics](https://arxiv.org/abs/2506.05112)
...
### [262] [Statistical microlocal analysis in two-dimensional X-ray CT](https://arxiv.org/abs/2506.05113)
...
### [267] [On the Spherical Dirichlet Distribution: Corrections and Results](https://arxiv.org/abs/2506.04441)
*Jose H Guardiola*

Main category: stat.ME

TL;DR: This note corrects an error in Guardiola (2020) and introduces the Spherical-Dirichlet Distribution for data in the positive orthant of a hypersphere, with applications in data mining and gene expressions.


<details>
  <summary>Details</summary>
Motivation: To address the need for a probability distribution that fits vectors in the positive orthant of a hypersphere, avoiding unnecessary probability mass in data mining and gene expression analysis.

Method: Correcting technical errors from Guardiola (2020), deriving the Spherical-Dirichlet Distribution, and exploring its properties, normalizing constants, moments, and relationships with other distributions.

Result: The Spherical-Dirichlet Distribution was applied to simulated data and a real text mining example, demonstrating its effectiveness in fitting data within the positive orthant of a hypersphere.

Conclusion: The Spherical-Dirichlet Distribution provides a suitable model for data in the positive orthant of a hypersphere, with potential applications in data mining and gene expressions, and the paper offers an extended discussion of its properties.
...
### [268] [Robust Estimation in Step-Stress Experiments under Exponential Lifetime Distributions](https://arxiv.org/abs/2506.04445)
...
### [269] [The Spurious Factor Dilemma: Robust Inference in Heavy-Tailed Elliptical Factor Models](https://arxiv.org/abs/2506.05116)
...
### [270] [Can we reconstruct a dysarthric voice with the large speech model Parler TTS?](https://arxiv.org/abs/2506.04397)
*Ariadna Sanchez,Simon King*

Main category: eess.AS

TL;DR: This paper explores using Parler TTS to reconstruct dysarthric speakers' voices, showing it can generate speech but with challenges in intelligibility and speaker identity consistency.


<details>
  <summary>Details</summary>
Motivation: Speech disorders hinder communication, and personalised TTS could help. The goal is to reconstruct a speaker's voice before their condition started using a large speech model.

Method: Curated a dataset with speaker and intelligibility info, fine-tuned Parler TTS to generate speech from challenging data.

Result: The model learned from the data but had difficulties controlling intelligibility and maintaining consistent speaker identity.

Conclusion: Future work should focus on improving controllability of such models for better voice reconstruction.
...
### [271] [Towards Efficient Speech-Text Jointly Decoding within One Speech Language Model](https://arxiv.org/abs/2506.04518)
*Haibin Wu,Yuxuan Hu,Ruchao Fan,Xiaofei Wang,Kenichi Kumatani,Bo Ren,Jianwei Yu,Heng Lu,Lijuan Wang,Yao Qian,Jinyu Li*

Main category: eess.AS

TL;DR: This paper compares joint speech-text decoding strategies for Speech LMs, finding that the interleaved approach offers the best alignment but is slow. It introduces an early-stop interleaved (ESI) method to speed up decoding while maintaining performance and curates high-quality QA datasets to enhance speech QA results.


<details>
  <summary>Details</summary>
Motivation: Speech language models (Speech LMs) enable end-to-end speech-text modelling within a single model, which is promising for spoken dialogue systems. The choice of joint speech-text decoding paradigm is critical for performance, efficiency, and alignment quality. However, existing studies may not have thoroughly compared these strategies under controlled conditions, prompting the need for this research.

Method: The paper systematically compares representative joint speech-text decoding strategies (interleaved and parallel generation) under a controlled experimental setup using the same base language model, speech tokenizer, and training data. It also proposes a novel early-stop interleaved (ESI) pattern to address the slow inference issue of the interleaved approach and curates high-quality question answering (QA) datasets to improve speech QA performance.

Result: The interleaved approach achieves the best alignment but suffers from slow inference due to long token sequence lengths. The proposed early-stop interleaved (ESI) pattern significantly accelerates decoding while yielding slightly better performance. Additionally, the curation of high-quality QA datasets further improves speech QA performance.

Conclusion: The study concludes that while the interleaved decoding strategy provides superior alignment, its inefficiency can be mitigated with the ESI approach. The use of high-quality QA datasets also enhances speech QA performance, suggesting that these methods and data curation are important for advancing spoken dialogue systems using Speech LMs.
...
### [272] [EMO-Debias: Benchmarking Gender Debiasing Techniques in Multi-Label Speech Emotion Recognition](https://arxiv.org/abs/2506.04652)
*Yi-Cheng Lin,Huang-Cheng Chou,Yu-Hsuan Li Liang,Hung-yi Lee*

Main category: eess.AS

TL;DR: The paper presents EMO-Debias, a large-scale comparison of 13 debiasing methods for multi-label speech emotion recognition (SER), evaluating their effectiveness in reducing gender bias while maintaining model performance.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing methods for SER have not been sufficiently explored in multi-label scenarios, and there is a need to address gender bias in these systems.

Method: The study compares 13 debiasing methods across different categories (pre-processing, regularization, adversarial learning, etc.) using acted and naturalistic emotion datasets with WavLM and XLSR representations.

Result: The analysis identifies approaches that reduce gender performance gaps without sacrificing overall model performance, quantifying the trade-offs between fairness and accuracy.

Conclusion: The findings offer practical insights for choosing effective debiasing strategies and emphasize the importance of dataset distributions in mitigating gender bias in SER systems.
...
### [273] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
*Fanxiao Li,Jiaying Wu,Canyuan He,Wei Zhou*

Main category: cs.MM

TL;DR: This paper addresses challenges in detecting out-of-context misinformation using MLLMs by proposing CMIE, which uses coexistence relationships and association scoring to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of existing MLLMs in capturing deeper semantic links between images and text for OOC misinformation detection.

Method: The method involves introducing CMIE with Coexistence Relationship Generation (CRG) and Association Scoring (AS) to identify semantic links and use relevant evidence effectively.

Result: The results show that CMIE outperforms existing methods in detecting OOC misinformation, particularly in cases where image and text are semantically linked but not directly connected.

Conclusion: The conclusion states that CMIE effectively addresses the challenges of MLLM in OOC misinformation detection, improving accuracy through semantic relationship analysis.
...
### [274] [Diffusion Transformer-based Universal Dose Denoising for Pencil Beam Scanning Proton Therapy](https://arxiv.org/abs/2506.04467)
...
### [275] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
...
### [276] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
...
### [277] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
...
### [278] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TL;DR: This paper introduces a fast, unsupervised polygonal approximation method for closed digital curves, featuring three phases: initial segmentation, iterative vertex insertion, and iterative merging, followed by vertex adjustment, demonstrating superior speed and competitive quality with existing methods.


<details>
  <summary>Details</summary>
Motivation: To develop a faster and more efficient polygonal approximation scheme for closed digital curves that outperforms state-of-the-art methods in speed while maintaining competitive quality in terms of Rosin's measure and aesthetic aspects.

Method: The method involves three phases: initial segmentation to detect sharp turnings (high curvature vertices), iterative vertex insertion to add vertices in regions with slow but steady curvature changes, and iterative merging to remove redundant vertices, followed by vertex adjustment for aesthetic improvement.

Result: The proposed scheme is faster than state-of-the-art approximation methods and is competitive with them in Rosin's measure and aesthetic aspects.

Conclusion: The paper concludes that the scheme provides a fast and effective polygonal approximation for closed digital curves, maintaining good quality and robustness against geometric transformations.
...
### [279] [Computationally Intensive Research: Advancing a Role for Secondary Analysis of Qualitative Data](https://arxiv.org/abs/2506.04230)
*Kaveh Mohajeri,Amir Karami*

Main category: cs.DB

TL;DR: The paper discusses the potential of using computational methods for secondary analysis of qualitative data, emphasizing benefits, challenges, and opportunities for innovative research designs.


<details>
  <summary>Details</summary>
Motivation: To highlight the underutilized potential of qualitative data through computational methods, addressing the gap in its reuse and sharing.

Method: Proposing a scheme for computationally intensive secondary analysis and discussing its applications.

Result: The paper presents opportunities for cross-contextual and longitudinal research using data assemblages and outlines challenges in data sharing and reuse.

Conclusion: Computational secondary analysis of qualitative data can lead to innovative research designs but requires addressing data sharing and reuse concerns.
...
### [280] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TL;DR: Introduce a deep learning approach for real-time solar image reconstruction using unpaired image translation.


<details>
  <summary>Details</summary>
Motivation: Current methods for correcting turbulent atmosphere effects in solar telescopes have issues with strong turbulence and high computational costs.

Method: Deep learning model using unpaired image-to-image translation trained on degraded bursts with speckle reconstructions as references.

Result: Improved robustness and perceptual quality, especially when speckle reconstructions have artifacts; best results with full image bursts.

Conclusion: The proposed method efficiently uses combined image information for superior solar image reconstruction.
...
### [281] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*

Main category: q-bio.NC

TL;DR: The GAIN model uses a grid-based structure and the Izhikevich model to create a biologically plausible and computationally efficient neural network simulation.


<details>
  <summary>Details</summary>
Motivation: To enhance biological plausibility and model dynamics by organizing neurons in a grid structure, mimicking real neural interactions.

Method: Adapting the Izhikevich model with a grid-based architecture to facilitate localized neuron interactions and improve connection efficiency.

Result: The model achieves improved dynamics and accuracy, offering efficient simulations suitable for neuroscience research and large-scale applications.

Conclusion: The GAIN model's grid structure and Izhikevich adaptation provide a promising approach for developing accurate and efficient neural network simulations.
...
### [282] [Was Residual Penalty and Neural Operators All We Needed for Solving Optimal Control Problems?](https://arxiv.org/abs/2506.04742)
*Oliver G. S. Lundqvist,Fabricio Oliveira*

Main category: math.OC

TL;DR: This paper proposes a method to solve optimal control problems using neural networks without integrating cost functions into the training process, using DeepONet with an unconstrained optimization routine.


<details>
  <summary>Details</summary>
Motivation: The motivation is to simplify the neural network architecture and training process for optimal control problems by decoupling the control problem from the training phase, avoiding the need to include cost functions during training.

Method: The method involves using a simple neural operator architecture like DeepONet, training it with a loss function that includes data and differential equation residuals, then applying an unconstrained optimization routine with a penalty term based on the equation residual and automatic differentiation to compute control gradients.

Result: The results show that three DeepONet models trained on different differential equations can solve nine distinct optimal control problems with varying cost functions, achieving accurate and consistent performance.

Conclusion: The conclusion is that integrating cost functions during training is unnecessary; a streamlined approach with DeepONet and subsequent optimization can effectively handle multiple optimal control problems.
...
### [283] [Understanding and Meeting Practitioner Needs When Measuring Representational Harms Caused by LLM-Based Systems](https://arxiv.org/abs/2506.04482)
*Emma Harvey,Emily Sheng,Su Lin Blodgett,Alexandra Chouldechova,Jean Garcia-Gathright,Alexandra Olteanu,Hanna Wallach*

Main category: cs.CY

TL;DR: This paper examines the challenges practitioners face in using publicly available instruments to measure representational harms in LLM-based systems, identifying issues of misalignment and practical barriers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand why practitioners are unable to use existing instruments for measuring representational harms despite their availability.

Method: The method involves semi-structured interviews with 12 practitioners to assess the usability and alignment of existing instruments.

Result: The result shows that practitioners often cannot use the instruments due to either misalignment with their goals or practical and institutional barriers.

Conclusion: The conclusion recommends improving instruments based on measurement theory and pragmatic approaches to better meet practitioner needs.
...
### [284] [Oversight Structures for Agentic AI in Public-Sector Organizations](https://arxiv.org/abs/2506.04836)
*Chris Schmitz,Jonathan Rystrøm,Jan Batzner*

Main category: cs.CY

TL;DR: This paper discusses the challenges of overseeing agentic AI systems in the public sector and proposes governance strategies to address them.
...
Method: The method involves a mixed-methods approach, including a literature review and interviews with civil servants in AI-related roles.
...
Conclusion: The paper concludes by proposing approaches to adapt institutional structures and design agent oversight that fits public sector constraints.
...
### [285] [Intentionally Unintentional: GenAI Exceptionalism and the First Amendment](https://arxiv.org/abs/2506.05211)
*David Atkinson,Jena D. Hwang,Jacob Morrison*

Main category: cs.CY

TL;DR: The paper argues that large generative AI models like GPT-4 and Gemini shouldn't receive First Amendment protections because they lack intentionality, and their outputs aren't considered speech under current legal definitions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to challenge the assumption that AI model outputs are protected under the First Amendment, highlighting the potential risks of misinformation and the need for effective government regulation.

Method: The paper uses legal analysis and reasoning based on established precedents to argue that AI outputs aren't speech due to lack of intentionality.

Result: The result is the conclusion that AI model outputs aren't protected speech and extending First Amendment rights to AI would hinder regulation and be harmful to society.

Conclusion: The conclusion is that First Amendment protections shouldn't apply to large AI model outputs, as they don't meet the legal criteria for speech and could impede necessary regulatory measures.
...
### [287] [Artificial Intelligence Should Genuinely Support Clinical Reasoning and Decision Making To Bridge the Translational Gap](https://arxiv.org/abs/2506.05030)
*Kacper Sokol,James Fackler,Julia E Vogt*

Main category: cs.HC

TL;DR: The paper discusses the limitations of AI in medicine due to a translational gap and suggests a sociotechnical approach to create data-driven tools that support doctors' work rather than aiming for superhuman performance on irrelevant benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the translational gap in AI applications in medicine by focusing on real-world impact and aligning AI tools with clinical practices like diagnostic reasoning and decision making.

Method: The method involves proposing a novel sociotechnical conceptualisation of data-driven support tools that complement doctors' cognitive and epistemic activities.

Result: The result is a new framework for AI in medicine that prioritizes real-world impact over achieving high performance on inconsequential benchmarks.

Conclusion: The conclusion states that by adopting a sociotechnical approach, AI systems can be better integrated into clinical practice, making them more effective and impactful in real-world scenarios.
...
### [288] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*
...
### [289] [Through the Stealth Lens: Rethinking Attacks and Defenses in RAG](https://arxiv.org/abs/2506.04390)
...
### [290] [Learning to Diagnose Privately: DP-Powered LLMs for Radiology Report Classification](https://arxiv.org/abs/2506.04450)
...
### [291] [BESA: Boosting Encoder Stealing Attack with Perturbation Recovery](https://arxiv.org/abs/2506.04556)
*Xuhao Ren,Haotian Liang,Yajie Wang,Chuan Zhang,Zehui Xiong,Liehuang Zhu*

Main category: cs.CR

TL;DR: BESA is a new attack method that improves encoder stealing by detecting and removing perturbations, making it more effective against defenses.


<details>
  <summary>Details</summary>
Motivation: Existing encoder stealing attacks are hindered by perturbation-based defenses, so there's a need for a method to overcome these defenses.

Method: BESA has two parts: detecting perturbations using feature vectors and recovering them with a generative model.

Result: BESA improves surrogate encoder accuracy by up to 24.63% against strong and multiple defenses.

Conclusion: BESA is effective in enhancing encoder stealing attacks by overcoming perturbation-based defenses.
...
### [292] [On Automating Security Policies with Contemporary LLMs](https://arxiv.org/abs/2506.04838)
*Pablo Fernández Saura,K. R. Jayaram,Vatche Isahagian,Jorge Bernal Bernabé,Antonio Skarmeta*

Main category: cs.CR

TL;DR: This paper introduces a framework using LLMs, in-context learning, and RAG to automate attack mitigation policy compliance, showing improved performance metrics with empirical evaluation.


<details>
  <summary>Details</summary>
Motivation: Modern computing environments and cyber threats require a more robust, adaptive, and automated security approach.

Method: The framework collects and manages tool and API specs in a vector database, decomposes mitigation policies into tasks, and translates them into API calls using LLMs with RAG.

Result: Empirical evaluation with STIXv2 CTI policies and Windows API docs showed better precision, recall, and F1-score with RAG than non-RAG baseline.

Conclusion: The proposed framework effectively automates security policy compliance using LLMs and RAG, offering a promising approach for adaptive cyber defense.
...
### [293] [Why LLM Safety Guardrails Collapse After Fine-tuning: A Similarity Analysis Between Alignment and Fine-tuning Datasets](https://arxiv.org/abs/2506.05346)
...
### [294] [Transformers Meet In-Context Learning: A Universal Approximation Theory](https://arxiv.org/abs/2506.05200)
*Gen Li,Yuchen Jiao,Yu Huang,Yuting Wei,Yuxin Chen*

Main category: cs.LG

TL;DR: This paper develops a universal approximation theory for transformers, showing they can perform in-context learning without fine-tuning by constructing a transformer that approximates any function class.


<details>
  <summary>Details</summary>
Motivation: To understand how transformers enable in-context learning and provide approximation guarantees beyond convex problems and linear classes.

Method: Developing a universal function approximation approach for transformers, constructing them to approximate any function class without parameter updates.

Result: Transformers can perform reliable prediction with few in-context examples without further weight updates, offering approximation guarantees not limited by optimization algorithms.

Conclusion: The study reveals how transformers learn general representations and adapt to in-context examples, providing a new theoretical framework for in-context learning.
...
### [295] [COSMOS: Predictable and Cost-Effective Adaptation of LLMs](https://arxiv.org/abs/2505.01449)
...
### [296] [A Comprehensive Survey on the Risks and Limitations of Concept-based Models](https://arxiv.org/abs/2506.04237)
...
### [297] [Triple Attention Transformer Architecture for Time-Dependent Concrete Creep Prediction](https://arxiv.org/abs/2506.04243)
...
### [298] [Localized Forest Fire Risk Prediction: A Department-Aware Approach for Operational Decision Support](https://arxiv.org/abs/2506.04254)
...
### [299] [MUC-G4: Minimal Unsat Core-Guided Incremental Verification for Deep Neural Network Compression](https://arxiv.org/abs/2506.04268)
*Jingyang Li,Guoqiang Li*

Main category: cs.LG

TL;DR: This paper introduces MUC-G4, a framework for incremental verification of compressed DNNs using SMT formulas and MUCs to improve verification efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of verifying compressed neural networks on edge devices, especially since existing methods only handle quantization and not structural changes like pruning.

Method: MUC-G4 encodes original and compressed networks into SMT formulas, classifies changes, and uses Minimal Unsat Cores from the original network to guide verification.

Result: Effective in both quantization and pruning, with high proof reuse and faster verification times than traditional methods.

Conclusion: MUC-G4 provides a promising solution for ensuring safety and reliability of compressed DNNs in real-world applications.
...
### [300] [AUTOCT: Automating Interpretable Clinical Trial Prediction with LLM Agents](https://arxiv.org/abs/2506.04293)
*Fengze Liu,Haoyu Wang,Joonhyuk Cho,Dan Roth,Andrew W. Lo*

Main category: cs.LG

TL;DR: AutoCT combines large language models and classical ML for interpretable clinical trial outcome prediction.


<details>
  <summary>Details</summary>
Motivation: Clinical trials are expensive and time-consuming; accurate prediction can reduce costs and speed drug discovery. However, existing deep learning models are not interpretable and prone to label leakage.

Method: AutoCT autonomously generates, evaluates, and refines tabular features using public info without human input, employing Monte Carlo Tree Search for iterative optimization.
...
### [301] [Bridging the Performance Gap Between Target-Free and Target-Based Reinforcement Learning With Iterated Q-Learning](https://arxiv.org/abs/2506.04398)
...
### [302] [Unsupervised Meta-Testing with Conditional Neural Processes for Hybrid Meta-Reinforcement Learning](https://arxiv.org/abs/2506.04399)
...
### [303] [Grokking and Generalization Collapse: Insights from \texttt{HTSR} theory](https://arxiv.org/abs/2506.04434)
...
### [304] [Behavioural vs. Representational Systematicity in End-to-End Models: An Opinionated Survey](https://arxiv.org/abs/2506.04461)
*Ivan Vegner,Sydelle de Souza,Valentin Forch,Martha Lewis,Leonidas A. A. Doumas*

Main category: cs.LG

TL;DR: This paper discusses the importance of systematicity in ML models for strong generalization, differentiates between representation and behavioural systematicity, and explores how existing benchmarks and methods assess these aspects.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of systematicity in ML models, which is crucial for generalization, and to highlight the distinction between representation and behavioural systematicity as often overlooked in current research.

Method: The paper builds on Hadley's taxonomy of systematic generalization and analyzes existing benchmarks and models in the literature, focusing on their ability to test behavioural systematicity in language and vision tasks.

Result: The results show that current benchmarks and models primarily focus on behavioural systematicity rather than representation systematicity, and the paper identifies ways to assess representation systematicity through mechanistic interpretability.

Conclusion: The conclusion emphasizes the need to consider both behavioural and representational systematicity in ML models, with a call for more research into assessing representational systematicity using methods from mechanistic interpretability.
...
### [305] [Classifying Dental Care Providers Through Machine Learning with Features Ranking](https://arxiv.org/abs/2506.04474)
...
### [306] [Comparative performance of ensemble models in predicting dental provider types: insights from fee-for-service data](https://arxiv.org/abs/2506.04479)
...
### [307] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
...
### [308] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
...
### [309] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
...
### [310] [Urania: Differentially Private Insights into AI Use](https://arxiv.org/abs/2506.04681)
...
TL;DR: Urania is a framework for generating insights from LLM chatbot interactions with differential privacy, using clustering and keyword extraction methods.


<details>
  <summary>Details</summary>
Motivation: To provide rigorous differential privacy guarantees while generating meaningful conversational insights from LLM chatbot interactions.

Method: Private clustering mechanism and keyword extraction methods (frequency-based, TF-IDF-based, LLM-guided) with DP tools like clustering, partition selection, and histogram-based summarization.

Result: Urania successfully extracts meaningful conversational insights while maintaining user privacy, showing effectiveness in balancing data utility and privacy preservation.

Conclusion: Urania demonstrates enhanced robustness in DP pipeline with effective balance between data utility and privacy, as shown by empirical evaluation.
...
### [311] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
...
### [312] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
*Jaeseung Heo,Kyeongheung Yun,Seokwon Yoon,MoonJeong Park,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: This paper introduces a proximal Bregman response function for GNNs, which allows accurate influence prediction for edge deletions and insertions without retraining, capturing message propagation changes and being applicable to graph rewiring and adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the interpretability and robustness of GNNs by understanding individual edge influences, addressing limitations in existing methods that have strict convexity assumptions and ignore edge insertions and message propagation effects.

Method: The method is a proximal Bregman response function tailored for GNNs, which relaxes convexity requirements and accounts for message propagation changes, enabling influence prediction for both edge deletions and insertions.

Result: The results show accurate influence predictions on real-world datasets for various GNN characteristics, and the influence function is effective in applications like graph rewiring and adversarial attacks.

Conclusion: The conclusion is that the proposed method improves GNN interpretability and robustness by accurately predicting edge influence, handling both deletions and insertions, and demonstrating versatility in applications.
...
### [313] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
...
### [314] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
...
### [315] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
*Pinak Mandal,Georg A. Gottwald*

Main category: cs.LG

TL;DR: This paper introduces fast unlearning algorithms using loss gradient orthogonalization to efficiently remove specific data influences from generative models without retraining, maintaining model fidelity and achieving faster results than previous methods like gradient surgery.


<details>
  <summary>Details</summary>
Motivation: As generative models grow more powerful and widespread, the need to unlearn certain data for privacy, legal compliance, or correcting harmful content becomes critical. Existing methods often require costly retraining, so there's a need for more efficient unlearning algorithms.

Method: The authors propose algorithms based on loss gradient orthogonalization, which allows selective removal of data points' influence without rebuilding the model from scratch.

Result: The algorithms effectively forget undesired data while preserving generation quality and the influence of desired training data. Experiments on MNIST and CelebA show they are orders of magnitude faster than prior methods such as gradient surgery.

Conclusion: The study concludes that loss gradient orthogonalization provides a practical and efficient approach to unlearning in generative models, balancing forgetting, fidelity, and speed.
...
### [316] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
*Chantal Pellegrini,Ege Özsoy,David Bani-Harouni,Matthias Keicher,Nassir Navab*

Main category: cs.LG

TL;DR: This paper introduces EHR2Path, a model that transforms EHR data into a structured representation to predict patient health trajectories, improving token efficiency and performance over text-only models.


<details>
  <summary>Details</summary>
Motivation: Healthcare systems need better ways to handle complex patient data for personalized care, but existing methods are limited in scope and feature space.

Method: The paper proposes EHR2Path, a holistic pathway prediction model that uses structured EHR data and a novel summary mechanism to embed long-term temporal context into topic-specific tokens.

Result: EHR2Path shows strong performance in next time-step prediction and longitudinal simulation, outperforming baselines and enabling detailed patient trajectory simulations.

Conclusion: EHR2Path offers a promising approach for predictive and personalized healthcare by effectively modeling patient pathways and addressing diverse evaluation tasks.
...
TL;DR: This paper addresses open questions and counterintuitive behaviors in long-context reasoning models trained via reinforcement learning, focusing on data efficiency, generalization, and model stability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore and resolve challenges in RL training for long-context reasoning models, such as data inefficiency and unstable performance.

Method: The method involves analyzing the roles of positive and negative samples in RL, identifying data inefficiency in group relative policy optimization, and investigating unstable performance across models and benchmarks.

Result: The results show that negative samples improve generalization and robustness, training on negative samples alone can match standard RL performance, and strategies like relative length rewards and offline sample injection enhance reasoning efficiency. Also, multiple evaluation runs reduce instability.

Conclusion: The conclusion is that addressing data inefficiency and instability through specific RL strategies can improve the performance of long-context reasoning models.
...
### [318] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
...
### [319] [Mitigating Degree Bias Adaptively with Hard-to-Learn Nodes in Graph Contrastive Learning](https://arxiv.org/abs/2506.05214)
*Jingyu Hu,Hongbo Bo,Jun Hong,Xiaowei Liu,Weiru Liu*

Main category: cs.LG

TL;DR: This paper introduces SHARP, an experimental framework that enhances Graph Neural Networks (GNNs) by addressing degree bias through a new contrastive loss called HAR, which adapts weighting based on learning hardness and increases positive pairs using node labels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address degree bias in GNNs during node classification tasks, where low-degree nodes struggle with insufficient and noisy information due to limited positive pairs and equal weighting in existing Graph Contrastive Learning (GCL) methods.

Method: The paper proposes the Hardness Adaptive Reweighted (HAR) contrastive loss, which adapts the weighting of positive and negative pairs based on their learning hardness and increases the number of positive pairs by leveraging node labels. Additionally, an experimental framework named SHARP is developed to extend HAR to more scenarios.

Result: Experiments on four datasets show that SHARP outperforms baselines in node classification tasks, demonstrating improved performance at both global and degree levels.

Conclusion: The study concludes that SHARP is effective in mitigating degree bias in GNNs, with theoretical analysis and experimental results supporting its superiority over existing methods.
...
### [320] [Diagonal Batching Unlocks Parallelism in Recurrent Memory Transformers for Long Contexts](https://arxiv.org/abs/2506.05229)
...
### [323] [Towards Reasonable Concept Bottleneck Models](https://arxiv.org/abs/2506.05014)
...
### [324] [Kinetics: Rethinking Test-Time Scaling Laws](https://arxiv.org/abs/2506.05333)
...
### [325] [Inference-Time Hyper-Scaling with KV Cache Compression](https://arxiv.org/abs/2506.05345)
...
### [326] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
...
### [327] [TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](https://arxiv.org/abs/2506.05035)
...
### [328] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*
...
### [329] [TreeRPO: Tree Relative Policy Optimization](https://arxiv.org/abs/2506.05183)
...
### [330] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
...
### [331] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
### [332] [Fast-DataShapley: Neural Modeling for Training Data Valuation](https://arxiv.org/abs/2506.05281)
...
### [333] [Sample Complexity and Representation Ability of Test-time Scaling Paradigms](https://arxiv.org/abs/2506.05295)
...
### [334] [Exploring Diffusion Transformer Designs via Grafting](https://arxiv.org/abs/2506.05340)
...
### [335] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
...
### [336] [BEAR: BGP Event Analysis and Reporting](https://arxiv.org/abs/2506.04514)
...
### [337] [Intelligent Channel Allocation for IEEE 802.11be Multi-Link Operation: When MAB Meets LLM](https://arxiv.org/abs/2506.04594)
...
### [338] [Towards Network Data Analytics in 5G Systems and Beyond](https://arxiv.org/abs/2506.04860)
...
### [339] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
...
### [340] [GOLFer: Smaller LM-Generated Documents Hallucination Filter & Combiner for Query Expansion in Information Retrieval](https://arxiv.org/abs/2506.04762)
*Lingyuan Liu,Mengxiang Zhang*
...
### [341] [Towards Storage-Efficient Visual Document Retrieval: An Empirical Study on Reducing Patch-Level Embeddings](https://arxiv.org/abs/2506.04997)
...
### [342] [Reason-to-Recommend: Using Interaction-of-Thought Reasoning to Enhance LLM Recommendation](https://arxiv.org/abs/2506.05069)
...
### [343] [A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair](https://arxiv.org/abs/2506.04987)
*Zanis Ali Khan,Aayush Garg,Qiang Tang*

Main category: cs.SE

TL;DR: This paper studies CodeBERT and CodeT5 for automated vulnerability patching, finding CodeBERT better with fragmented context and CodeT5 better at complex patterns and scalability, but both have generalization issues.


<details>
  <summary>Details</summary>
Motivation: Software vulnerabilities are critical security risks, and automated vulnerability patching is underexplored despite APR advances.

Method: Evaluated CodeBERT and CodeT5 on six datasets and four languages for vulnerability patching, tested fine-tuned models on in and out-of-distribution data.

Result: CodeBERT handles fragmented context better; CodeT5 captures complex patterns and scales better. Fine-tuning helps in-distribution but not out-of-distribution.

Conclusion: The study benchmarks models, identifies generalization limitations, and offers insights to improve automated patching for real-world security.
...
### [344] [An Expansion-Based Approach for Quantified Integer Programming](https://arxiv.org/abs/2506.04452)
...
### [345] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
### [346] [cuVSLAM: CUDA accelerated visual odometry](https://arxiv.org/abs/2506.04359)
*Alexander Korovko,Dmitry Slepichev,Alexander Efitorov,Aigul Dzhumamuratova,Viktor Kuznetsov,Hesam Rabeti,Joydeep Biswas*

Main category: cs.RO

TL;DR: cuVSLAM is a real-time visual simultaneous localization and mapping solution optimized with CUDA for edge devices, supporting various sensor configurations.


<details>
  <summary>Details</summary>
Motivation: Accurate and robust pose estimation is crucial for autonomous robots, and existing solutions may not efficiently support diverse sensor setups or real-time performance on edge devices.

Method: cuVSLAM is designed as a state-of-the-art VSLAM system using CUDA optimization for real-time operation, allowing flexibility with different visual-inertial sensors.

Result: cuVSLAM demonstrates best-in-class performance on benchmarks, operates with various sensor configurations, and has minimal computational overhead on edge devices.

Conclusion: cuVSLAM is a versatile and efficient VSLAM solution suitable for a wide range of robotic applications, particularly those requiring real-time processing on edge hardware.
...
### [347] [Hierarchical Language Models for Semantic Navigation and Manipulation in an Aerial-Ground Robotic System](https://arxiv.org/abs/2506.05020)
...
### [348] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
...
### [349] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
...
### [350] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
...
### [351] [Domain Adaptation Method and Modality Gap Impact in Audio-Text Models for Prototypical Sound Classification](https://arxiv.org/abs/2506.04376)
*Emiliano Acevedo,Martín Rocamora,Magdalena Fuentes*

Main category: cs.SD

TL;DR: The paper discusses how audio-text models struggle with background sounds in environmental classification and proposes a method to improve their performance without retraining.


<details>
  <summary>Details</summary>
Motivation: To address the performance drop of audio-text models in real-world environments with background sounds.

Method: Proposed a novel method to quantify and integrate background sources' contribution into classification, along with domain adaptation and modality gap analysis.

Result: Improved accuracy across various backgrounds and SNR conditions, with effective generalization to state-of-the-art approaches.

Conclusion: The method enhances robustness and scalability of audio-text models in diverse environmental conditions without needing retraining.
...
### [352] [Grapheme-Coherent Phonemic and Prosodic Annotation of Speech by Implicit and Explicit Grapheme Conditioning](https://arxiv.org/abs/2506.04527)
*Hien Ohnaka,Yuma Shirahata,Byeongseon Park,Ryuichi Yamamoto*

Main category: cs.SD

TL;DR: The paper proposes a model that generates phonemic and prosodic speech labels consistent with graphemes using BERT features and hypothesis pruning, improving tasks like text-to-speech and accent estimation.


<details>
  <summary>Details</summary>
Motivation: Previous methods only fine-tuned ASR models with labels, leading to inconsistency between graphemes and predicted labels. This paper aims to address that issue.

Method: The model uses two methods: implicit grapheme conditioning via a prompt encoder with BERT features and explicit pruning of inconsistent label hypotheses during inference.

Result: The proposed method significantly improved grapheme-label consistency. It also enhanced accent estimation accuracy when using the created parallel data.

Conclusion: The model effectively generates coherent speech labels with graphemes, applicable to downstream tasks, and shows improved performance in accent estimation.
...
### [353] [LLM-based phoneme-to-grapheme for phoneme-based speech recognition](https://arxiv.org/abs/2506.04711)
...
### [354] [Survey on the Evaluation of Generative Models in Music](https://arxiv.org/abs/2506.05104)
*Alexander Lerch,Claire Arthur,Nick Bryan-Kinns,Corey Ford,Qianyi Sun,Ashvala Vinay*

Main category: cs.SD

TL;DR: This paper offers an interdisciplinary review of evaluation methods for generative music systems, covering both subjective and objective approaches, as well as qualitative and quantitative methods, and discusses their advantages and challenges from musicological, engineering, and HCI perspectives.


<details>
  <summary>Details</summary>
Motivation: The motivation is to systematically evaluate generative systems in music, which have received significant attention and growth in recent years.

Method: The method involves an interdisciplinary review of evaluation targets, methodologies, and metrics for system output and model usability, considering various approaches such as subjective, objective, qualitative, quantitative, empirical, and computational methods.

Result: The result is a comprehensive review of evaluation methods for generative music systems, including their advantages and challenges from different perspectives.

Conclusion: The conclusion is that the paper provides a thorough analysis of evaluation approaches for generative music systems, highlighting the need to consider multiple perspectives for effective evaluation.
...
### [355] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
### [356] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*
...
### [357] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
...
### [358] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
...
### [359] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
*Usman Muhammad,Jorma Laaksonen*

Main category: eess.IV

TL;DR: This paper proposes DACN, a dual-attention convolutional network for hyperspectral image super-resolution, which addresses the limitations of 2D CNNs by incorporating multi-head and channel attention mechanisms, along with a custom loss function, leading to improved performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of 2D CNNs in hyperspectral image super-resolution, such as reliance on local neighborhoods, lack of global contextual understanding, band correlation issues, and data scarcity.

Method: The method involves using augmented convolutions with multi-head attention to capture local and global dependencies, inferring separate attention maps for channel and spatial dimensions, and introducing a custom loss function combining L2 regularization and spatial-spectral gradient loss.

Result: Experimental results show that combining multi-head and channel attention mechanisms in DACN outperforms using either mechanism alone on two hyperspectral datasets.

Conclusion: The conclusion is that DACN effectively improves hyperspectral image super-resolution by addressing the key limitations through dual attention mechanisms and an optimized loss function.
...
### [360] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
### [361] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*
...
### [362] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
