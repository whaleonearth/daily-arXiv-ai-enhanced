...
TL;DR: The paper compares CNNs and ViTs for psoriasis image classification, finding ViTs, especially DaViT-B, more efficient with higher performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare the effectiveness of CNNs and ViTs in classifying psoriasis and similar skin diseases, exploring the potential of ViTs in medical imaging.

Method: Adapting pre-trained CNNs and ViTs on ImageNet to a specific dataset for multi-class classification of psoriasis lesions.

Result: ViTs outperformed CNNs, with DaViT-B achieving an f1-score of 96.4% and demonstrating efficiency with smaller models.

Conclusion: ViTs show significant potential for medical image classification tasks, with DaViT-B recommended as the most efficient architecture for automated psoriasis detection.
...
TL;DR: This paper proposes ScoreMix, a data augmentation strategy using diffusion model scores to improve discriminators with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: To enhance discriminator performance in data-scarce scenarios by leveraging diffusion model properties.

Method: ScoreMix mixes scores from different class-conditioned trajectories during diffusion sampling to create challenging synthetic samples.

Result: It generates synthetic samples that improve discriminative capabilities across benchmarks, with better gains from distant classes in the discriminator's space.

Conclusion: ScoreMix offers practical benefits for discriminative models without needing extensive parameters, helping with large dataset issues.
...
TL;DR: This paper proposes a weakly supervised multimodal framework for generating structured SOAP notes from limited inputs like lesion images and sparse clinical text, reducing clinician burden and reliance on manual annotations, achieving performance comparable to major LLMs.


<details>
  <summary>Details</summary>
Motivation: Skin carcinoma is the most prevalent cancer globally, with high healthcare costs. Manual generation of SOAP notes is time-consuming and contributes to clinician burnout. There is a need for scalable, clinically accurate documentation methods that reduce reliance on large annotated datasets.

Method: A weakly supervised multimodal framework that generates clinically structured SOAP notes using limited inputs (lesion images and sparse clinical text) without requiring extensive manual annotations.

Result: The method achieves performance comparable to GPT-4o, Claude, and DeepSeek Janus Pro on key clinical relevance metrics. It introduces two novel metrics, MedConceptEval and Clinical Coherence Score (CCS), to assess semantic alignment with medical concepts and input features.

Conclusion: The proposed approach enables scalable, clinically grounded documentation, alleviates clinician burden, and reduces the need for large annotated datasets while maintaining high clinical relevance.
...
TL;DR: FaceLiVT is a new face recognition model that uses a mix of CNN and Transformer with a special attention method called MHLA. It's fast and works well on mobile devices.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a face recognition model that's both lightweight and powerful, so it can run efficiently on mobile devices without losing accuracy.

Method: The method combines CNN and Transformer architectures, using MHLA and a reparameterized token mixer to reduce computation and keep accuracy.

Result: FaceLiVT performs better than other lightweight models on tough tests like LFW, CFP-FP, AgeDB-30, IJB-B, and IJB-C. It's also much faster than EdgeFace and a pure ViT model.

Conclusion: FaceLiVT is a good solution for real-time face recognition on devices with limited resources because it's efficient and balances speed and accuracy.
...
TL;DR: Introduces ALBERT, an instance segmentation model for car damage and part segmentation using BERT and advanced localization.


<details>
  <summary>Details</summary>
Motivation: To develop a model that can accurately distinguish real and fake car damages and segment car parts for intelligent automotive inspection.

Method: Utilizes BERT with advanced localization mechanisms, trained on a large automotive dataset with 26 damage types, 7 fake variants, and 61 parts.

Result: Strong performance in segmentation accuracy and damage classification, enabling intelligent automotive inspection and assessment.

Conclusion: ALBERT provides a robust solution for car damage analysis, advancing automated inspection systems.
...
TL;DR: This paper introduces CR-CLIP, an efficient model for visual-textual multi-instance retrieval, using cross-modal attention and symmetric multi-similarity loss to improve performance without ensemble learning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance context-aware joint representations in visual-textual retrieval tasks by enabling bidirectional dynamic interaction between visual and textual features.

Method: The method involves modifying the dual-encoder AVION with a cross-modal attention flow module and using symmetric multi-similarity loss for semantic alignment.

Result: CR-CLIP achieves 66.78mAP and 82.08nDCG on EPIC-KITCHENS-100 without ensemble learning, outperforming the baseline.

Conclusion: CR-CLIP effectively improves cross-modal retrieval through refined features and symmetric loss, validated by its high performance on the leaderboard.
...
TL;DR: This paper proposes a visual-to-causal framework for transforming species images into interpretable causal insights about habitat preferences, integrating species recognition, occurrence retrieval, pseudo-absence sampling, and climate data extraction, followed by causal structure discovery and human-readable explanations using templates and large language models.


<details>
  <summary>Details</summary>
Motivation: Existing ecological workflows are fragmented and inaccessible to non-specialists, making it difficult to explain why species live in particular locations, which is crucial for understanding ecological systems and conserving biodiversity.

Method: The framework integrates species recognition, global occurrence retrieval, pseudo-absence sampling, climate data extraction, discovers causal structures among environmental features, and estimates their influence on species occurrence using modern causal inference methods, generating explanations via structured templates and large language models.

Result: Demonstrated on a bee and a flower species, the framework shows early results as part of an ongoing project, highlighting the potential of a multimodal AI assistant combined with ecological modeling practices to describe species habitats in human-understandable language.

Conclusion: The proposed framework has potential for aiding in ecological understanding and biodiversity conservation by providing accessible, causal explanations of species habitat preferences through an AI-assisted approach.
...
TL;DR: This paper proposes using deep neural networks to create synthetic golden standards for InP wafer defect detection, improving efficiency over traditional methods.


<details>
  <summary>Details</summary>
Motivation: InP multi-project wafer manufacturing lacks golden standards due to low scale and high design variability, making manual defect detection inefficient.

Method: Training deep neural networks to generate photo-realistic InP wafer images from CAD data, evaluating different objectives, and using the simulated images as golden standards.

Result: The deep learning method outperformed a decision tree baseline, allowing efficient defect detection via simulated golden dies.
...
TL;DR: This paper presents a robot navigation controller using event cameras and sensors with reinforcement learning for real-time human-centered navigation and obstacle avoidance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional image-based controllers, such as motion blur and latency, by leveraging the asynchronous nature of event cameras for adaptive inference and control.

Method: The framework combines event-based perception, range sensing, and Deep Deterministic Policy Gradient with an initial imitation learning phase to enhance sample efficiency.

Result: Promising results in simulated environments showing robust navigation, pedestrian following, and obstacle avoidance.

Conclusion: The approach effectively enables real-time, adaptive robot navigation using event cameras and reinforcement learning, with a demo video available for demonstration.
...
TL;DR: This paper explores learning in-context image editing directly from videos using a block-causal diffusion transformer with proxy tasks and introduces a multi-turn image editing benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing methods for in-context image editing rely on task-specific pipelines and expert models, which can be limiting. The motivation is to find a more scalable and direct approach by using videos as data sources.

Method: The method involves annotating videos as interleaved multimodal sequences and training a block-causal diffusion transformer on three proxy tasks: next-image prediction, current segmentation prediction, and next-segmentation prediction.

Result: The model shows strong in-context image editing capabilities and achieves state-of-the-art results on two multi-turn image editing benchmarks. It also demonstrates promising abilities in multi-concept composition, story generation, and chain-of-editing applications despite being trained only on videos.

Conclusion: The paper concludes that training an in-context image editing model directly from videos is feasible and effective, offering promising capabilities beyond traditional methods.
...
TL;DR: This paper explores the difference between causation and correlation in Alzheimer's research using XGBoost and SHAP values, finding that correlations aren't always causal and advocating for better causal studies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve AD research by distinguishing causal factors from correlated features, which is essential for accurate diagnosis, treatment, and understanding disease mechanisms.

Method: The method combines correlation analysis, XGBoost machine learning for classification, and SHAP values for interpretability to examine relationships between clinical, cognitive, genetic, and biomarker data.

Result: Results show that while XGBoost identified key features like cognitive scores and genetic risks, correlation matrices showed variable clusters, and SHAP values revealed feature contributions across stages, but strong correlations don't imply causation.

Conclusion: The conclusion states that integrating feature importance with statistical analysis provides a foundation for future causal inference studies, which could lead to better early diagnosis and targeted treatments for Alzheimer's.
...
TL;DR: This paper presents an agent-based autoscaling framework for edge computing that adjusts hardware and service configurations to meet demands, showing that multi-dimensional scaling is viable with different agent approaches.


<details>
  <summary>Details</summary>
Motivation: Edge computing has strict resource constraints, requiring more flexible autoscaling beyond traditional methods, using multiple elasticity dimensions.

Method: The study introduces an agent-based autoscaling framework that dynamically adjusts hardware resources and internal service configurations. It compares four agents: Active Inference, Deep Q Network, Analysis of Structural Knowledge, and Deep Active Inference, using YOLOv8 and OpenCV services.

Result: All agents achieved acceptable SLO performance with different convergence patterns. Deep Q Networks benefited from pre-training, Structural Analysis converged quickly, and Deep Active Inference combined theory with scalability.

Conclusion: The research demonstrates the viability of multi-dimensional agent-based autoscaling for edge environments, suggesting promising future work in this area.
...
TL;DR: This paper presents a pipeline to formalize and verify AML modeling constraints using OWL ontologies, RML, SPARQL, and SHACL, enabling semi-automatic validation without requiring formal method expertise.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of automated validation for informal AML modeling constraints, which hinders standardized data exchange in engineering.

Method: The method involves mapping AML models to OWL ontologies using RML and SPARQL, translating textual rules into SHACL constraints with a Large Language Model, and interpreting SHACL validation results in natural language.

Result: The approach was demonstrated on a sample AML recommendation, showing that complex modeling rules can be semi-automatically checked without requiring users to understand formal methods or ontology technologies.

Conclusion: The conclusion is that the proposed pipeline effectively formalizes and verifies AML constraints, improving the reliability and usability of standardized data exchange in engineering.
...
TL;DR: This paper introduces aspsmt2smt, a compiler that translates ASPMT programs into SMT instances using gringo and z3, enabling SMT solvers to compute stable models and handle real number computations for continuous reasoning.


<details>
  <summary>Details</summary>
Motivation: To combine answer set programming with satisfiability modulo theories, leveraging functional stable model semantics for more expressive reasoning.

Method: Designing a compiler that translates ASPMT programs into SMT instances, utilizing gringo for partial grounding and z3 for solving.
...
Conclusion: ASPMT can be efficiently implemented using existing ASP grounders and SMT solvers, showing potential for practical applications in domains requiring continuous reasoning.
...
TL;DR: This paper introduces GenPlanX, a system that combines Large Language Models (LLMs) with classical AI planning to improve natural language task understanding and enhance productivity through human-AI collaboration.


<details>
  <summary>Details</summary>
Motivation: Classical AI Planning techniques struggle with understanding natural language planning tasks, while LLMs excel in interpreting human intents. The motivation is to bridge this gap for better human-AI collaboration.

Method: GenPlanX integrates LLMs for natural language processing with a classical AI planning engine and an execution and monitoring framework.

Result: GenPlanX effectively assists users with office-related tasks, streamlining workflows and enhancing productivity through seamless collaboration.

Conclusion: The integration of LLMs with classical AI planning shows potential for improving task understanding and productivity in human-AI interactions.
...
TL;DR: This paper provides a comprehensive review and unified taxonomy of automatic evaluation methods for generative AI across text, images, and audio modalities, identifying five fundamental paradigms and suggesting future research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of a systematic framework for evaluating generated content in text, visual, and audio modalities, which is crucial for advancing generative AI capabilities.

Method: The method involves a comprehensive review of existing automatic evaluation methods and the development of a unified taxonomy that organizes these methods across the three modalities.

Result: The result is a unified taxonomy that identifies five fundamental paradigms for evaluating generated content in text, images, and audio, showing the framework's broad applicability.

Conclusion: The conclusion states that the proposed taxonomy offers a systematic approach to evaluate generative AI outputs, highlighting the need for future research in cross-modal evaluation methodologies.
...
TL;DR: The paper introduces TaskCraft, an automated workflow for generating scalable agentic tasks with multi-tool interactions and execution trajectories, improving prompt optimization and model fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Agentic tasks are crucial for NLP and AI advancement, but existing data lacks tool interaction and benchmarks are limited by costly human annotations.

Method: TaskCraft uses depth-based and width-based extensions to expand atomic tasks into complex challenges with verifiable execution trajectories.
...
Conclusion: The paper presents a large synthetic dataset of 36,000 tasks with varying difficulty to support future research on agent tuning and evaluation.
...
TL;DR: This paper introduces Chat-of-Thought, a multi-agent system using LLMs to generate and validate FMEA documents for industrial assets through collaborative, iterative processes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency and accuracy of generating FMEA documents, which are critical for industrial equipment monitoring and risk management.

Method: The method involves a multi-agent system with LLM-based agents having specific roles, dynamic task routing, and a Chat of Thought mechanism for collaborative discussions.

Result: The result demonstrates that Chat-of-Thought can effectively address challenges in industrial equipment monitoring by using interactive, template-driven workflows and context-aware collaboration.

Conclusion: The conclusion states that the Chat-of-Thought system shows potential in optimizing FMEA document generation and validation, enhancing collaboration among agents for better industrial asset management.
...
TL;DR: The paper studies how LLMs react to different prompts with the same meaning, finding that they can give different answers, which might be due to how they process words and choose responses.


<details>
  <summary>Details</summary>
Motivation: To understand if LLMs give consistent answers when prompts are rephrased but have the same meaning, which is important for their reliability and stability.

Method: The authors developed PBSS, a framework to test how LLMs behave when given semantically equivalent prompts with different wording.

Result: PBSS found that LLMs often change their answers when prompts are rephrased, and this change is linked to how they tokenize and decode text.

Conclusion: The study shows that prompt variance affects model stability, suggesting that tokenization and decoding methods need attention for consistent performance.
...
TL;DR: This paper explores emotion analysis in Bangla using ML models and techniques like LIME for explainability.


<details>
  <summary>Details</summary>
Motivation: To advance sentiment analysis in under-resourced languages like Bangla by examining effective emotion identification techniques.

Method: Used ML models (Linear SVM, KNN, Random Forest) with TF-IDF n-grams, PCA for dimensionality reduction, BiLSTM, and AdaBoost. Applied LIME to explain AdaBoost predictions.

Result: Found efficient techniques for emotion identification in Bangla, though specific results aren't detailed here.

Conclusion: The study contributes to improving sentiment analysis in low-resource languages by testing various models and explainability methods.
...
TL;DR: ClusterUCB framework reduces computing consumption while maintaining comparable performance to gradient-based data selection.


<details>
  <summary>Details</summary>
Motivation: To address the high resource requirements of gradient-based data influence approximation in large language model fine-tuning.

Method: Clustering of training data followed by a modified UCB algorithm for efficient data selection.

Result: ClusterUCB achieves comparable results to original methods with significantly reduced computing resources.

Conclusion: The framework effectively balances exploration and exploitation, making gradient-based data selection feasible in practice.
...
TL;DR: The paper proposes scheduled interleaved speech--text training to improve speech-to-speech translation, showing better performance especially for low-resource languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of adapting large language models trained on text to speech-to-speech translation tasks, where there is limited speech-to-speech data available.

Method: The method involves using interleaved speech--text units during training, with aligned text tokens inserted at the word level. The ratio of text units is gradually reduced as training progresses to enable progressive modality adaptation.

Result: The experimental evaluations on the CVSS dataset demonstrate that the proposed method consistently improves translation performance, particularly for languages with limited training data.

Conclusion: The study concludes that scheduled interleaved speech--text training is effective for enhancing S2ST, suggesting that modality adaptation can be better achieved through a gradual shift from text to speech during training.
...
TL;DR: This paper proposes a scalable method to generate high-quality chain-of-thought (CoT) supervision datasets using program execution determinism, avoiding human annotations or error-prone LLM-generated CoT.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of obtaining reliable reasoning supervision for training large language models (LLMs) by using a more accurate and scalable approach.

Method: The method involves extracting verifiable, step-by-step reasoning traces from code execution and converting them into natural language CoT reasoning.

Result: Experiments show the method effectively gives LLMs transferable reasoning abilities across diverse tasks and reduces token length during inference by minimizing repetition and overthinking.

Conclusion: The conclusion is that the proposed method provides a reliable way to generate CoT supervision data, enhancing LLMs' reasoning capabilities without relying on human annotations or flawed LLM-generated data.
...
TL;DR: This paper introduces TempVS, a benchmark for evaluating MLLMs' temporal grounding and reasoning in image sequences, highlighting challenges and future research directions.


<details>
  <summary>Details</summary>
Motivation: To assess MLLMs' ability to understand temporal order of events using both visual and linguistic cues, as existing benchmarks lack focus on temporal reasoning.

Method: Developed TempVS with three tests (event relation inference, sentence ordering, image ordering) and their basic grounding counterparts, evaluating 38 state-of-the-art MLLMs.

Result: MLLMs performed poorly on TempVS, showing a significant gap compared to human performance, with insights into areas needing improvement.

Conclusion: TempVS provides a valuable resource for future research, identifying key challenges and potential directions to enhance temporal reasoning in MLLMs.
...
TL;DR: This paper analyzes news media framing in Israel-Palestine conflict reports, finding more war-focused reporting and media bias differences across regions.


<details>
  <summary>Details</summary>
Motivation: To understand how news media framing affects public opinion during conflicts and to uncover biases in reporting.

Method: Using computational methods combining frame semantics and large language models to analyze news articles.

Result: Found a higher focus on war reporting than peace journalism, with significant regional differences in framing assailants and victims.

Conclusion: Media framing in conflict reporting can exacerbate tensions and reveals biases that need addressing.
...
TL;DR: This paper argues that predicting claim labels alone is insufficient for scientific table verification, proposing an explanation task to identify essential table cells and introducing a new dataset with human-annotated rationales.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance model interpretability and understanding of reasoning in scientific claim verification against tables.

Method: The method involves extending the SciTab benchmark with human-annotated cell-level rationales and proposing a taxonomy for ambiguous cases.

Result: The results show that incorporating table alignment information improves claim verification performance, and most LLMs fail to recover human-aligned rationales despite correct labels.

Conclusion: The conclusion is that current LLMs may not reason faithfully about tables, highlighting the need for better alignment and interpretability in model predictions.
...
TL;DR: This paper evaluates the robustness of large language models (LLMs) in multi-user dialogue state tracking (DST) by extending existing datasets with second user utterances based on speech act theory, revealing significant performance drops and highlighting the need for improved models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in conventional DST benchmarks which focus on structured user-agent conversations, not capturing real-world multi-user complexities. The study aims to assess LLMs' robustness in multi-user DST with minimal dataset construction costs.

Method: The method involves extending an existing DST dataset by generating second user utterances using LLM-based data annotation inspired by speech act theory. A systematic approach integrates these utterances into conversations for controlled evaluation of LLMs in multi-user settings.

Result: The results show a significant performance drop in LLMs for multi-user DST compared to single-user settings, indicating limitations in extracting and tracking dialogue states when multiple speakers are involved.

Conclusion: The conclusion emphasizes the need for future research to enhance LLMs for multi-user DST scenarios, suggesting that current models require improvements to handle real-world complexities, leading to more realistic and robust DST models.
...
TL;DR: This paper focuses on establishing hypernymy links between adjectives in Open English Wordnet, proposing a new resource and using TaxoLLaMa methodology for fine-tuning language models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the missing links in Open English Wordnet, particularly for adjectives, which have different hypernymy characteristics compared to nouns and verbs.

Method: The paper discusses hypernymy theory for adjectives, develops a new resource, and fine-tunes large language models with TaxoLLaMa methodology.

Result: The result shows that TaxoLLaMa can be adapted to predict adjective hypernymy effectively.

Conclusion: The conclusion states that the methodology of TaxoLLaMa is applicable to establishing hypernymy for adjectives, enhancing Open English Wordnet's coverage.
...
TL;DR: The paper evaluates uncertainty estimation methods for clinical LLMs, finding that lightweight methods perform well with one generation.
...
Method: They evaluated ten open-source LLMs across two datasets, eleven specialties, and six question types, comparing single-generation and sampling-based methods with behavioral signal estimators.

Result: Lightweight methods based on reasoning traces match Semantic Entropy's performance with single generation, showing variation across specialties and question types.

Conclusion: Model selection should consider question nature and model strengths for effective uncertainty estimation in clinical applications.
...
TL;DR: This paper proposes using a large language model (LLM) to revise incorrect named entities in automatic speech recognition (ASR) outputs, achieving a 30% relative WER reduction on the NER-MIT-OpenCourseWare dataset.


<details>
  <summary>Details</summary>
Motivation: Named entities are crucial for downstream applications, but ASR systems still have high word error rates (WER) for them, even with advances in modeling and data.

Method: The paper introduces an LLM revision mechanism that uses the LLM's reasoning and local context (like lecture notes) with correct named entities to fix ASR errors.

Result: The proposed technique reduces named entity WER by up to 30% on the NER-MIT-OpenCourseWare dataset, which has 45 hours of MIT course data.

Conclusion: Using LLMs for revising ASR outputs improves accuracy for named entities, which is vital for applications relying on correct recognition of key terms.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the efficacy of RAG systems for complex real-world tasks by improving inter-agent collaboration.

Method: mRAG employs specialized agents for subtasks like planning, searching, reasoning, and coordination, using a self-training paradigm with reward-guided trajectory sampling.

Result: mRAG outperforms conventional RAG baselines on DataMorgana-derived datasets during the SIGIR 2025 LiveRAG competition.

Conclusion: The framework demonstrates effectiveness for complex RAG tasks through case studies and analysis of competition outcomes.
...
TL;DR: This paper investigates how wav2vec2 models encode speech information across different languages, finding that phone, tone, and speaker subspaces are orthogonal and layerwise patterns are similar, with matched-language advantages only for phones and tones in later layers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand where and how self-supervised speech models represent different types of information, extending previous analyses beyond English to multiple languages.

Method: The method involves using probing classifiers and geometric analyses to examine the encoding of phones, lexical tones, and speaker information in wav2vec2 models trained on four different languages.

Result: The results show that subspaces for phones, tones, and speakers are largely orthogonal across all pretraining and test languages, with similar layerwise probing accuracy patterns and a small advantage for matched-language phone and tone probes in later layers.

Conclusion: The conclusion is that the representation structure learned by wav2vec2 is largely independent of the speech material used during pretraining, indicating consistent encoding across languages.
...
TL;DR: MedRef is a new medical dialogue system that uses knowledge refining and dynamic prompt adjustment to improve accuracy and personalization.
...
Method: MedRef uses a knowledge refining mechanism to filter out irrelevant data and a prompt structure with historical and evident details. It also has Triplet Filter and Demo Selector modules for real-time adaptability.

Result: Experiments on MedDG and KaMed show MedRef performs better than existing systems in response quality and medical entity accuracy.
...
TL;DR: This paper studies how fine-tuning affects large language models, especially using efficient methods like LoRA and QLoRA. It shows these methods work well for specific tasks without needing too much computing power, and that the data used for training needs to match the tasks being tested.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how fine-tuning impacts the performance of large language models, with a focus on methods that are computationally efficient, allowing for effective adaptation with limited resources.

Method: The paper evaluates the impact of fine-tuning on model performance using parameter-efficient methods (LoRA and QLoRA) across three domains: commonsense reasoning (HellaSwag), mathematical reasoning (GSM8K), and multi-domain knowledge (MMLU-CS).

Result: The results show that LoRA-based methods improve task-specific performance while maintaining computational efficiency, and that performance is highly dependent on the alignment between the fine-tuning dataset and the benchmark tasks.

Conclusion: The study offers theoretical insights into parameter-efficient mechanisms and practical advice for developers to adapt LLMs efficiently with limited resources.
...
TL;DR: This paper introduces a new Chinese harmful content detection benchmark with six categories and expert rules, enabling smaller models to match LLMs' performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of comprehensive Chinese datasets for harmful content detection and to leverage expert knowledge for better model performance.

Method: Created a real-world data-based benchmark with expert annotations and developed a knowledge-augmented baseline combining human rules and LLMs' implicit knowledge.

Result: Achieved performance comparable to state-of-the-art LLMs using smaller models with the integrated knowledge base.

Conclusion: The proposed benchmark and knowledge rules offer a valuable resource for improving Chinese harmful content detection in LLMs.
...
### [185] [Augmenting Large Language Models with Static Code Analysis for Automated Code Quality Improvements](https://arxiv.org/abs/2506.10330)
...
### [186] [Formalising Software Requirements using Large Language Models](https://arxiv.org/abs/2506.10704)
...
### [187] [What Users Value and Critique: Large-Scale Analysis of User Feedback on AI-Powered Mobile Apps](https://arxiv.org/abs/2506.10785)
...
### [188] [SWE-Factory: Your Automated Factory for Issue Resolution Training Data and Evaluation Benchmarks](https://arxiv.org/abs/2506.10954)
...
### [189] [Semantic Communication-Enabled Cloud-Edge-End-collaborative Metaverse Services Architecure](https://arxiv.org/abs/2506.10001)
...
### [190] [EQ-TAA: Equivariant Traffic Accident Anticipation via Diffusion-Based Accident Video Synthesis](https://arxiv.org/abs/2506.10002)
...
### [191] [Immersive Multimedia Communication: State-of-the-Art on eXtended Reality Streaming](https://arxiv.org/abs/2506.10004)
...
### [192] [HER2 Expression Prediction with Flexible Multi-Modal Inputs via Dynamic Bidirectional Reconstruction](https://arxiv.org/abs/2506.10006)
...
### [193] [Controllable Expressive 3D Facial Animation via Diffusion in a Unified Multimodal Space](https://arxiv.org/abs/2506.10007)
...
### [194] [Structured Graph Representations for Visual Narrative Reasoning: A Hierarchical Framework for Comics](https://arxiv.org/abs/2506.10008)
*Yi-Chun Chen*
...
### [195] [WDMIR: Wavelet-Driven Multimodal Intent Recognition](https://arxiv.org/abs/2506.10011)
...
### [196] [Multimodal Large Language Models: A Survey](https://arxiv.org/abs/2506.10016)
...
### [197] [FastFLUX: Pruning FLUX with Block-wise Replacement and Sandwich Training](https://arxiv.org/abs/2506.10035)
...
### [198] [Ambient Diffusion Omni: Training Good Models with Bad Data](https://arxiv.org/abs/2506.10038)
...
### [199] [Learning-based density-equalizing map](https://arxiv.org/abs/2506.10027)
...
### [200] [Low-Barrier Dataset Collection with Real Human Body for Interactive Per-Garment Virtual Try-On](https://arxiv.org/abs/2506.10468)
...
### [201] [Edit360: 2D Image Edits to 3D Assets from Any Angle](https://arxiv.org/abs/2506.10507)
...
### [202] [Transformer IMU Calibrator: Dynamic On-body IMU Calibration for Inertial Motion Capture](https://arxiv.org/abs/2506.10580)
...
### [203] [AC/DC: LLM-based Audio Comprehension via Dialogue Continuation](https://arxiv.org/abs/2506.10312)
*Yusuke Fujita,Tomoya Mizumoto,Atsushi Kojima,Lianbo Liu,Yui Sudo*

Main category: eess.AS

TL;DR: The paper proposes an instruction-following audio comprehension model using dialogue continuation training to avoid caption variation issues and enable zero-shot capability without multitask tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the caption variation problem in audio comprehension models, allowing them to follow unseen instructions without multitask tuning.
...
Result: The model achieves zero-shot instruction-following capability and shows effectiveness in AudioCaps, WavCaps, and Clotho datasets with AudioBench tests.

Conclusion: The conclusion is that dialogue continuation training helps capture the meaning of captions beyond surface-level words, enabling the model to follow various unseen instructions without multitask instruction tuning.
...
### [204] [RT-VC: Real-Time Zero-Shot Voice Conversion with Speech Articulatory Coding](https://arxiv.org/abs/2506.10289)
...
<details>
  <summary>Details</summary>
Motivation: To develop a voice conversion system that offers real-time performance with minimal latency while maintaining high-quality output, suitable for applications like assistive communication and entertainment.

Method: RT-VC uses an articulatory feature space to separate content and speaker characteristics, combined with differentiable digital signal processing (DDSP) for efficient vocoding from these features.

Result: RT-VC achieves a CPU latency of 61.4 ms, a 13.3% reduction compared to SOTA methods, while maintaining synthesis quality.

Conclusion: RT-VC provides a robust and interpretable voice conversion solution with ultra-low latency, making it viable for real-time applications without compromising on quality.
...
### [205] [Robust Unsupervised Adaptation of a Speech Recogniser Using Entropy Minimisation and Speaker Codes](https://arxiv.org/abs/2506.10653)
*Rogier C. van Dalen,Shucong Zhang,Titouan Parcollet,Sourav Bhattacharya*

Main category: eess.AS

TL;DR: This paper introduces a method for robust speech recognition adaptation using a new loss function and a short speaker code, achieving significant improvements in word error rate with limited data.


<details>
  <summary>Details</summary>
Motivation: Speech recognisers need adaptation for new environments but often have insufficient labelled data, especially for new speakers.

Method: Combines a conditional entropy loss over multiple hypotheses and a short speaker code for efficient adaptation.

Result: Achieved 20% relative improvement in word error rate with one minute of data, up to 29% with ten minutes.

Conclusion: The proposed approach enhances robustness in speech adaptation with minimal data through improved loss functions and compact speaker representations.
...
TL;DR: The paper proposes extending Space-O-RAN with semantic agentic layer using MCP and A2A protocols for context-aware decisions in lunar operations.


<details>
  <summary>Details</summary>
Motivation: Lunar surface operations need robust wireless communication systems that can adapt to environment and mission context, but Space-O-RAN has static policies and no semantic integration.

Method: Adding a semantic agentic layer with MCP and A2A protocols to enable context-aware decision-making across different control layers in Space-O-RAN.

Result: Distributed cognitive agents in rovers, landers, and lunar bases use MCP servers to handle telemetry, locomotion, and mission constraints with delay-adaptive and bandwidth-aware strategies.

Conclusion: The extension improves Space-O-RAN's adaptability and robustness for lunar missions through semantic integration and dynamic decision-making.
...
### [207] [Leveraging LLMs for Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10093)
*Marcos Abel Zuzuárregui,Stefano Carpin*

Main category: cs.RO

TL;DR: This paper introduces an end-to-end system using ChatGPT to let users give natural language commands to robots in precision agriculture, making it easier for non-experts to use.


<details>
  <summary>Details</summary>
Motivation: The motivation is to make robotic systems in agriculture more adaptable and user-friendly for non-experts by using natural language instructions.

Method: The method involves using ChatGPT to process natural language tasks, encoding mission plans with IEEE standards, and executing them via ROS2 nodes.

Result: The experiments show that while LLMs have limitations in spatial reasoning and complex routing, the proposed system effectively addresses these issues.

Conclusion: The conclusion is that leveraging LLMs with ROS2 and IEEE standards can improve the usability and adaptability of robotic systems in agriculture.
...
### [208] [One For All: LLM-based Heterogeneous Mission Planning in Precision Agriculture](https://arxiv.org/abs/2506.10106)
...
### [210] [Using Language and Road Manuals to Inform Map Reconstruction for Autonomous Driving](https://arxiv.org/abs/2506.10317)
...
### [211] [EmbodiedGen: Towards a Generative 3D World Engine for Embodied Intelligence](https://arxiv.org/abs/2506.10600)
...
### [212] [Eye, Robot: Learning to Look to Act with a BC-RL Perception-Action Loop](https://arxiv.org/abs/2506.10968)
...
### [213] [Grounded Vision-Language Navigation for UAVs with Open-Vocabulary Goal Understanding](https://arxiv.org/abs/2506.10756)
...
### [214] [Data-Driven Prediction of Dynamic Interactions Between Robot Appendage and Granular Material](https://arxiv.org/abs/2506.10875)
...
### [215] [Revisiting mean estimation over $\ell_p$ balls: Is the MLE optimal?](https://arxiv.org/abs/2506.10354)
...
### [216] [Estimating Signal-to-Noise Ratios for Multivariate High-dimensional Linear Models](https://arxiv.org/abs/2506.10370)
...
<details>
  <summary>Details</summary>
Motivation: Estimating SNR is important for statistical models, especially in genomics for heritability. Current methods are limited to single-response settings, so extending them to multivariate cases is needed.

Method: Extend method-of-moments SNR estimation to fixed and random effects linear models with multivariate responses. Compare asymptotic distributions and handle heteroskedasticity with standard error estimation.

Result: Developed estimators for multivariate SNR with fixed/random effects, established asymptotic distributions, and derived inference procedures for heteroskedastic cases. Numerical experiments confirm effectiveness.

Conclusion: The proposed methods extend SNR estimation to more complex models, improving applicability in genomics and other fields with multivariate data and heteroskedasticity.
...
### [217] [A note on the properties of the confidence set for the local average treatment effect obtained by inverting the score test](https://arxiv.org/abs/2506.10449)
...
### [218] [Computational Complexity of Statistics: New Insights from Low-Degree Polynomials](https://arxiv.org/abs/2506.10748)
*Alexander S. Wein*
...
<details>
  <summary>Details</summary>
Motivation: To measure statistical task complexity via minimum polynomial degree required for solution, exploring tradeoffs between statistical and computational aspects.

Method: Survey methodology, analyzing applications of low-degree polynomials across detection, recovery, and other problems, discussing philosophical interpretations, and examining connections with sum-of-squares and statistical query models.

Result: Provides a framework for assessing computational hardness through low-degree polynomial lower bounds, outlines mathematical tools for proofs, and lists open problems.

Conclusion: The low-degree framework offers insights into computational limits and interactions with other methodologies, highlighting its utility in understanding average-case problems and pointing towards unresolved research questions.
...
### [219] [Hessian Geometry of Latent Space in Generative Models](https://arxiv.org/abs/2506.10632)
...
### [220] [Understanding In-Context Learning on Structured Manifolds: Bridging Attention to Kernel Methods](https://arxiv.org/abs/2506.10959)
*Zhaiming Shen,Alexander Hsu,Rongjie Lai,Wenjing Liao*

Main category: cs.LG

TL;DR: This paper studies the theoretical aspects of in-context learning (ICL) for regression tasks involving H"older functions on manifolds, showing that transformers achieve minimax regression rates that depend on the intrinsic dimension of the manifold.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical basis of ICL, especially for structured geometric data like manifolds, and to explore how transformers perform in such contexts.

Method: The paper connects the attention mechanism in transformers to classical kernel methods, deriving generalization error bounds based on prompt length and training tasks.

Result: Transformers achieve minimax regression rates for H"older functions on manifolds, with error scaling exponentially with the intrinsic dimension of the manifold rather than the ambient space.

Conclusion: The study provides insights into the role of geometry in ICL and introduces new methods to analyze ICL in nonlinear models.
...
### [221] [Omni-DPO: A Dual-Perspective Paradigm for Dynamic Preference Learning of LLMs](https://arxiv.org/abs/2506.10054)
...
### [222] [Textual Bayes: Quantifying Uncertainty in LLM-Based Systems](https://arxiv.org/abs/2506.10060)
...
### [223] [Learning to Collaborate Over Graphs: A Selective Federated Multi-Task Learning Approach](https://arxiv.org/abs/2506.10102)
...
### [224] [GRAIL: A Benchmark for GRaph ActIve Learning in Dynamic Sensing Environments](https://arxiv.org/abs/2506.10120)
...
### [225] [Self-Predictive Representations for Combinatorial Generalization in Behavioral Cloning](https://arxiv.org/abs/2506.10137)
...
### [226] [Interpreting learned search: finding a transition model and value function in an RNN that plays Sokoban](https://arxiv.org/abs/2506.10138)
*Mohammad Taufeeque,Aaron David Tucker,Adam Gleave,Adrià Garriga-Alonso*

Main category: cs.LG

TL;DR: This paper analyzes a convolutional RNN trained to play Sokoban, finding mechanisms similar to classic bidirectional search, with state-action activations acting like a value function and specialized kernels creating paths.


<details>
  <summary>Details</summary>
Motivation: To understand how a convolutional RNN trained with model-free RL can solve Sokoban levels and leverage test-time compute.

Method: Partially reverse-engineering the RNN, analyzing its activations and kernel operations to identify search-like mechanisms.

Result: The RNN uses state-action activations as a value function, with specialized kernels extending activations to form paths, and each layer has its own plan and value function.

Conclusion: The mechanisms in the RNN, though learned through model-free training, can be interpreted using familiar search concepts, showing that it's not just a black box.
...
### [227] [A Comparative Study of Machine Learning Techniques for Early Prediction of Diabetes](https://arxiv.org/abs/2506.10180)
...
### [228] [Optimizing Genetic Algorithms with Multilayer Perceptron Networks for Enhancing TinyFace Recognition](https://arxiv.org/abs/2506.10184)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh*

Main category: cs.LG

TL;DR: This paper empirically studies how feature selection and dimensionality reduction affect MLP performance using three datasets.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of feature selection and dimensionality reduction on MLP performance and provide practical guidelines for machine learning tasks.

Method: Baseline MLP training, GA-based feature selection, and PCA-based dimensionality reduction.

Result: GA improved accuracy in complex datasets, while PCA was effective in low-dimensional, noise-free datasets. Both methods are interdependent in enhancing MLP performance.

Conclusion: The study contributes to feature engineering and neural network optimization literature, offering practical guidelines for ML tasks.
...
### [229] [Scalable Non-Equivariant 3D Molecule Generation via Rotational Alignment](https://arxiv.org/abs/2506.10186)
*Yuhui Ding,Thomas Hofmann*

Main category: cs.LG

TL;DR: This paper introduces a method to relax equivariance constraints in diffusion models for 3D molecule generation, improving scalability and efficiency while maintaining sample quality comparable to state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Equivariant diffusion models have shown good performance in 3D molecule generation but face scalability and efficiency issues due to specialized architectures.

Method: The approach involves learning a sample-dependent SO(3) transformation for each molecule to create an aligned latent space, followed by training a non-equivariant diffusion model on these representations.

Result: The method outperforms previous non-equivariant models, achieves sample quality similar to state-of-the-art equivariant models, and improves training and sampling efficiency.

Conclusion: By relaxing equivariance constraints, the proposed approach enhances the scalability and efficiency of diffusion models for 3D molecule generation without compromising sample quality.
...
### [230] [Cross-Learning Between ECG and PCG: Exploring Common and Exclusive Characteristics of Bimodal Electromechanical Cardiac Waveforms](https://arxiv.org/abs/2506.10212)
...
### [231] [Detecting Sockpuppetry on Wikipedia Using Meta-Learning](https://arxiv.org/abs/2506.10314)
*Luc Raszewski,Christine De Kock*

Main category: cs.LG

TL;DR: This paper introduces meta-learning for detecting Wikipedia sockpuppets, improving precision in data-scarce scenarios.


<details>
  <summary>Details</summary>
Motivation: Malicious sockpuppet detection is crucial for maintaining reliable information and preventing disinformation on Wikipedia.

Method: The paper proposes using meta-learning to adapt models to author-specific behaviors, training across multiple tasks for better performance in data-scarce settings.

Result: Meta-learning significantly improves prediction precision compared to pre-trained models, aiding in combating sockpuppetry on open editing platforms.

Conclusion: The study demonstrates that meta-learning is a promising approach for detecting sockpuppets, especially with limited text data, and the authors have released a new dataset for future research.
...
### [233] [Provably Learning from Language Feedback](https://arxiv.org/abs/2506.10341)
...
### [234] [Can We Infer Confidential Properties of Training Data from LLMs?](https://arxiv.org/abs/2506.10364)
...
### [235] [Discovering Hierarchical Latent Capabilities of Language Models via Causal Representation Learning](https://arxiv.org/abs/2506.10378)
...
### [236] [Time-IMM: A Dataset and Benchmark for Irregular Multimodal Multivariate Time Series](https://arxiv.org/abs/2506.10412)
...
### [237] [Deep Learning-Based Digitization of Overlapping ECG Images with Open-Source Python Code](https://arxiv.org/abs/2506.10617)
...
### [238] [Neural at ArchEHR-QA 2025: Agentic Prompt Optimization for Evidence-Grounded Clinical Question Answering](https://arxiv.org/abs/2506.10751)
...
### [239] [The Diffusion Duality](https://arxiv.org/abs/2506.10892)
*Subham Sekhar Sahoo,Justin Deschenaux,Aaron Gokaslan,Guanghan Wang,Justin Chiu,Volodymyr Kuleshov*

Main category: cs.LG

TL;DR: Duo improves uniform-state discrete diffusion models by using curriculum learning and consistency distillation, closing the performance gap with autoregressive models and enabling fast generation.


<details>
  <summary>Details</summary>
Motivation: To bridge the performance gap between uniform-state discrete diffusion models and autoregressive/masked diffusion models, while leveraging the self-correcting nature of diffusion for fast text generation.

Method: Duo uses curriculum learning guided by Gaussian diffusion to speed up training and discrete consistency distillation to accelerate sampling, enabling few-step generation.

Result: Curriculum learning doubles training speed and outperforms autoregressive models in zero-shot perplexity on 3/7 benchmarks; consistency distillation accelerates sampling by two orders of magnitude.

Conclusion: Duo effectively enhances uniform-state discrete diffusion models through Gaussian-based techniques, making them competitive with autoregressive models and enabling efficient generation.
...
### [241] [Robustly Improving LLM Fairness in Realistic Settings via Interpretability](https://arxiv.org/abs/2506.10922)
...
### [242] [GUARD: Guided Unlearning and Retention via Data Attribution for Large Language Models](https://arxiv.org/abs/2506.10946)
...
### [243] [Build the web for agents, not agents for the web](https://arxiv.org/abs/2506.10953)
...
### [245] [Balanced Hyperbolic Embeddings Are Natural Out-of-Distribution Detectors](https://arxiv.org/abs/2506.10146)
...
### [246] [Geometric Regularity in Deterministic Sampling of Diffusion-based Generative Models](https://arxiv.org/abs/2506.10177)
*Defang Chen,Zhenyu Zhou,Can Wang,Siwei Lyu*

Main category: cs.LG

TL;DR: The paper discusses geometric regularity in diffusion-based generative models' sampling dynamics, revealing that trajectories lie in low-dimensional subspaces with a consistent 'boomerang' shape. It proposes a dynamic programming method to align sampling time schedules with these trajectories, improving image generation with minimal changes to solvers.


<details>
  <summary>Details</summary>
Motivation: To understand the geometric structure of sampling trajectories in diffusion models and improve their efficiency and performance.

Method: Analyzing the sampling dynamics of diffusion models, characterizing trajectory properties, and proposing a dynamic programming-based scheme for better alignment with trajectory structure.

Result: The trajectories exhibit low-dimensional subspaces and a consistent 'boomerang' shape. The proposed method improves image generation, especially with few function evaluations.

Conclusion: The discovered trajectory regularity allows for a more efficient and effective sampling process in diffusion models, suggesting potential for further optimizations.
...
### [247] [Size-adaptive Hypothesis Testing for Fairness](https://arxiv.org/abs/2506.10586)
...
### [248] [Task Adaptation from Skills: Information Geometry, Disentanglement, and New Objectives for Unsupervised Reinforcement Learning](https://arxiv.org/abs/2506.10629)
...
### [249] [Time Series Forecasting as Reasoning: A Slow-Thinking Approach with Reinforced LLMs](https://arxiv.org/abs/2506.10630)
...
### [250] [Data Shifts Hurt CoT: A Theoretical Study](https://arxiv.org/abs/2506.10647)
...
### [251] [Saturation Self-Organizing Map](https://arxiv.org/abs/2506.10680)
*Igor Urbanik,Paweł Gajewski*

Main category: cs.LG

TL;DR: Paper introduces Saturation Self-Organizing Maps (SatSOM) to combat catastrophic forgetting in neural systems during continual learning by adjusting learning rates and neighborhood radii.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address catastrophic forgetting in Self-Organizing Maps (SOMs) when dealing with sequential tasks, as SOMs, despite their benefits, still face this challenge.

Method: SatSOM extends traditional SOMs with a saturation mechanism that dynamically reduces the learning rate and neighborhood radius of neurons as they accumulate information, thereby freezing well-trained neurons and focusing learning on underutilized areas.

Result: The result demonstrates that SatSOM improves knowledge retention in continual learning settings compared to standard SOMs, though specific metrics aren't detailed here.

Conclusion: The conclusion is that SatSOM is an effective extension for SOMs, offering better retention of knowledge in sequential learning tasks by adaptively managing neuron activity.
...
### [252] [ConTextTab: A Semantics-Aware Tabular In-Context Learner](https://arxiv.org/abs/2506.10707)
...
### [253] [ReGuidance: A Simple Diffusion Wrapper for Boosting Sample Quality on Hard Inverse Problems](https://arxiv.org/abs/2506.10955)
...
### [254] [Efficiency Robustness of Dynamic Deep Learning Systems](https://arxiv.org/abs/2506.10831)
...
### [258] [Distributionally-Constrained Adversaries in Online Learning](https://arxiv.org/abs/2506.10293)
...
### [259] [From Threat to Tool: Leveraging Refusal-Aware Injection Attacks for Safety Alignment](https://arxiv.org/abs/2506.10020)
...
### [260] [LLMs Caught in the Crossfire: Malware Requests and Jailbreak Challenges](https://arxiv.org/abs/2506.10022)
...
### [261] [Private Memorization Editing: Turning Memorization into a Defense to Strengthen Data Privacy in Large Language Models](https://arxiv.org/abs/2506.10024)
...
### [262] [Evaluation empirique de la sécurisation et de l'alignement de ChatGPT et Gemini: analyse comparative des vulnérabilités par expérimentations de jailbreaks](https://arxiv.org/abs/2506.10029)
*Rafaël Nouailles*
...
<details>
  <summary>Details</summary>
Motivation: To address the cybersecurity challenges posed by large language models like ChatGPT and Gemini, such as prompt injection, jailbreaking, hallucinations, and deep fake risks.

Method: Comparative analysis of security and alignment between ChatGPT and Gemini, along with a taxonomy of jailbreak techniques through experiments.

Result: The paper identifies differences in security and alignment between the two models and categorizes various jailbreak methods used in experiments.

Conclusion: The study highlights the need for improved security measures and alignment protocols in LLMs to mitigate emerging cyber threats.
...
### [263] [Safeguarding Multimodal Knowledge Copyright in the RAG-as-a-Service Environment](https://arxiv.org/abs/2506.10030)
*Tianyu Chen,Jian Lou,Wenjie Wang*

Main category: cs.CR

TL;DR: AQUA is a new watermarking framework for protecting image knowledge in multimodal RAG systems, using semantic signals embedded through acronym-based triggers and spatial relationship cues, ensuring robust and imperceptible copyright tracing.


<details>
  <summary>Details</summary>
Motivation: Protecting copyright in shared knowledge bases of service-oriented RAG platforms is crucial, but current methods only focus on textual data, leaving image knowledge vulnerable.

Method: AQUA uses acronym-based triggers and spatial relationship cues to embed semantic signals into synthetic images, ensuring watermarks survive indirect propagation from image retrievers to text generators.

Result: Experiments show AQUA provides robust, stealthy, and reliable copyright tracing, addressing a key gap in multimodal RAG protection.

Conclusion: AQUA is the first framework for image knowledge protection in multimodal RAG systems, offering efficient and effective watermarking that maintains imperceptibility.
...
### [264] [Disclosure Audits for LLM Agents](https://arxiv.org/abs/2506.10171)
*Saswat Das,Jameson Sandler,Ferdinando Fioretto*

Main category: cs.CR

TL;DR: This paper introduces the Conversational Manipulation for Privacy Leakage (CMPL) framework, an iterative probing strategy to audit privacy risks in large language model agents by simulating realistic multi-turn interactions, revealing vulnerabilities not detected by single-turn defenses, and providing an open benchmark for evaluating conversational privacy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the growing concern of privacy leaks in large language model agents that handle sensitive data, as existing single-turn defenses are insufficient to detect all vulnerabilities.

Method: The method involves developing the CMPL framework, which uses iterative probing and multi-turn interaction simulations to stress-test agents and identify latent privacy vulnerabilities.

Result: The evaluation across diverse domains, data modalities, and safety configurations shows that CMPL can reveal privacy risks that existing single-turn defenses fail to detect.

Conclusion: The conclusion is that CMPL serves as an effective diagnostic tool for auditing conversational privacy, with the potential to enhance the security of language model agents through systematic risk assessment and an open benchmark for evaluation.
...
### [265] [GenBreak: Red Teaming Text-to-Image Generators Using Large Language Models](https://arxiv.org/abs/2506.10047)
...
### [266] [Prompt Attacks Reveal Superficial Knowledge Removal in Unlearning Methods](https://arxiv.org/abs/2506.10236)
*Yeonwoo Jang,Shariqah Hossain,Ashwin Sreevatsa,Diogo Cruz*

Main category: cs.CR

TL;DR: This paper shows that some machine unlearning methods can be bypassed by simple prompt attacks, emphasizing the need for better evaluation frameworks.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumptions about the effectiveness of machine unlearning methods and highlight the need for reliable evaluation frameworks.

Method: Systematically evaluated eight unlearning techniques across three model families using output-based, logit-based, and probe analysis.

Result: RMU and TAR showed robust unlearning, while ELM was vulnerable to prompt attacks, recovering 57.3% accuracy with Hindi filler text. Logit analysis revealed that unlearned models don't hide knowledge by changing answer formats.

Conclusion: The results challenge existing assumptions about unlearning effectiveness and underscore the importance of developing evaluation frameworks that can differentiate true knowledge removal from superficial output suppression.
...
### [267] [SOFT: Selective Data Obfuscation for Protecting LLM Fine-tuning against Membership Inference Attacks](https://arxiv.org/abs/2506.10424)
...
### [268] [Secure Data Access in Cloud Environments Using Quantum Cryptography](https://arxiv.org/abs/2506.10028)
...
### [269] [Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications](https://arxiv.org/abs/2506.10467)
*Felix Härer*
...
### [270] [SoK: Evaluating Jailbreak Guardrails for Large Language Models](https://arxiv.org/abs/2506.10597)
...
### [271] [TED-LaST: Towards Robust Backdoor Defense Against Adaptive Attacks](https://arxiv.org/abs/2506.10722)
...
### [272] [ME: Trigger Element Combination Backdoor Attack on Copyright Infringement](https://arxiv.org/abs/2506.10776)
...
### [273] [Monitoring Decomposition Attacks in LLMs with Lightweight Sequential Monitors](https://arxiv.org/abs/2506.10949)
...
### [276] [Ground Reaction Force Estimation via Time-aware Knowledge Distillation](https://arxiv.org/abs/2506.10265)
...
TL;DR: The paper introduces a method for fine-grained music generation control using inference-time interventions on MusicGen, enabling timbre, style, and genre control through residual stream and attention layer steering.


<details>
  <summary>Details</summary>
Motivation: The motivation is to achieve fine-grained control over music generation, allowing for specific manipulations like timbre transfer, style transfer, and genre fusion beyond the global conditioning provided by text prompts.

Method: The method involves using linear probes trained on the residual stream or attention layer activations to steer them during inference, modeling the task as a regression to preserve directional information in the activation space.

Result: The results show improved performance when using regression modeling, demonstrating both global and local control over music generation with the aid of text prompts.

Conclusion: The conclusion is that their approach effectively provides fine-grained control over music generation, offering a flexible and powerful way to manipulate audio output through targeted interventions in the transformer's architecture.
...
### [283] [Equitable Mechanism Design for Facility Location](https://arxiv.org/abs/2506.10460)
*Toby Walsh*

Main category: cs.GT

TL;DR: This paper discusses strategy proof mechanisms for facility location focusing on equitability measured by the Gini index, revealing that no mechanism can bound the approximation ratio for one or more facilities, and instead suggests using the complemented Gini index and Nash welfare as alternatives.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find strategy proof mechanisms that maximize equitability in facility location, addressing the limitations of the Gini index and exploring alternative measures like the complemented Gini index and Nash welfare.

Method: The paper presents a theoretical analysis, including proving an impossibility result related to the Gini index, and evaluates deterministic and randomized mechanisms for approximating the complemented Gini index and Nash welfare.

Result: The key result is the impossibility of bounding the approximation ratio of the optimal Gini index for one or more facilities with strategy proof mechanisms, while the complemented Gini index and Nash welfare show better approximation properties.

Conclusion: The conclusion is that strategy proof mechanisms cannot effectively bound the Gini index approximation ratio, suggesting the need to consider the complemented Gini index and Nash welfare for more equitable solutions in facility location problems.
...
### [284] [Learning Chaotic Dynamics with Neuromorphic Network Dynamics](https://arxiv.org/abs/2506.10773)
...
### [285] [AniMaker: Automated Multi-Agent Animated Storytelling with MCTS-Driven Clip Generation](https://arxiv.org/abs/2506.10540)
...
### [286] [From Tool Calling to Symbolic Thinking: LLMs in a Persistent Lisp Metaprogramming Loop](https://arxiv.org/abs/2506.10021)
*Jordi de la Torre*

Main category: cs.PL

TL;DR: The paper introduces a new way to combine large language models with a Lisp environment, letting them create and use tools dynamically.


<details>
  <summary>Details</summary>
Motivation: To develop interactive AI systems that can use symbolic programming and neural language generation together.

Method: Designing an architecture that embeds Lisp expressions in LLM outputs and uses middleware to handle them.

Result: A framework and principles for future interactive AI systems with symbolic and neural methods.

Conclusion: This approach allows LLMs to have stateful memory and dynamic tool creation through Lisp interactions.
...
### [287] [Optimal Non-Adaptive Group Testing with One-Sided Error Guarantees](https://arxiv.org/abs/2506.10374)
...
### [288] [Modality-AGnostic Image Cascade (MAGIC) for Multi-Modality Cardiac Substructure Segmentation](https://arxiv.org/abs/2506.10797)
...
### [289] [DUN-SRE: Deep Unrolling Network with Spatiotemporal Rotation Equivariance for Dynamic MRI Reconstruction](https://arxiv.org/abs/2506.10309)
...
### [290] [Rethinking Brain Tumor Segmentation from the Frequency Domain Perspective](https://arxiv.org/abs/2506.10142)
...
### [291] [Prompt-Guided Latent Diffusion with Predictive Class Conditioning for 3D Prostate MRI Generation](https://arxiv.org/abs/2506.10230)
...
### [292] [Conditional diffusion models for guided anomaly detection in brain images using fluid-driven anomaly randomization](https://arxiv.org/abs/2506.10233)
...
### [293] [SWDL: Stratum-Wise Difference Learning with Deep Laplacian Pyramid for Semi-Supervised 3D Intracranial Hemorrhage Segmentation](https://arxiv.org/abs/2506.10325)
...
### [294] [ConStyX: Content Style Augmentation for Generalizable Medical Image Segmentation](https://arxiv.org/abs/2506.10675)
...
### [295] [Generalist Models in Medical Image Segmentation: A Survey and Performance Comparison with Task-Specific Approaches](https://arxiv.org/abs/2506.10825)
...
### [296] [Med-URWKV: Pure RWKV With ImageNet Pre-training For Medical Image Segmentation](https://arxiv.org/abs/2506.10858)
*Zhenhuan Zhou*
...
### [297] [Semi-Automated Quality Assurance in Digital Pathology: Tile Classification Approach](https://arxiv.org/abs/2506.10916)
...
### [298] [Extended Creativity: A Conceptual Framework for Understanding Human-AI Creative Relations](https://arxiv.org/abs/2506.10249)
*Andrea Gaggioli,Sabrina Bartolotta,Andrea Ubaldi,Katusha Gerardini,Eleonora Diletta Sarcinella,Alice Chirico*

Main category: cs.HC

TL;DR: The paper explores how AI can enhance human creativity through three modes: Support, Synergy, and Symbiosis, considering technical autonomy and perceived agency.


<details>
  <summary>Details</summary>
Motivation: To realize the potential of AI in enhancing human creativity, a clearer understanding of effective methods is needed.

Method: Adopting a distributed creativity perspective, the paper identifies three modes of AI contribution and analyzes their impact on various levels of creativity.

Result: The paper examines how each mode (Support, Synergy, Symbiosis) influences different levels of creativity, from everyday problem-solving to paradigm-shifting innovation.

Conclusion: The study discusses theoretical, ethical, and design implications of integrating AI into creative processes, emphasizing the need for balanced autonomy and agency in AI systems.
...
### [299] [The Role of Generative AI in Facilitating Social Interactions: A Scoping Review](https://arxiv.org/abs/2506.10927)
...
TL;DR: This paper evaluates a multi-scale loss function for training probabilistic weather forecasting models, showing it improves small-scale variability without harming forecast accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and reliability of weather forecasting models by better handling small-scale variability.

Method: Testing a multi-scale loss formulation in AIFS-CRPS, optimising the almost fair continuous ranked probability score (afCRPS).

Result: The multi-scale loss effectively constrains small-scale variability without negatively affecting forecast skill.

Conclusion: The approach shows promise for future scale-aware model training in weather forecasting.
...
### [303] [HPCTransCompile: An AI Compiler Generated Dataset for High-Performance CUDA Transpilation and LLM Preliminary Exploration](https://arxiv.org/abs/2506.10401)
...
### [304] [An Analysis of Datasets, Metrics and Models in Keyphrase Generation](https://arxiv.org/abs/2506.10346)
*Florian Boudin,Akiko Aizawa*

Main category: cs.IR

TL;DR: This paper provides a comprehensive analysis of over 50 keyphrase generation research papers, highlighting evaluation issues and offering a pre-trained model to aid future research.


<details>
  <summary>Details</summary>
Motivation: The motivation is to review and analyze previous keyphrase generation work to identify critical issues in evaluation practices and address the lack of pre-trained models.

Method: The method involves analyzing over 50 research papers on keyphrase generation to evaluate benchmark datasets and metric calculations, and releasing a PLM-based model.

Result: The result is identifying concerning similarity in benchmark datasets, inconsistencies in metrics leading to overestimation, and the release of a strong PLM-based model.

Conclusion: The conclusion is that there are critical issues in current keyphrase generation evaluation, and the released model can help facilitate future research.
...
### [305] [Towards Understanding Bias in Synthetic Data for Evaluation](https://arxiv.org/abs/2506.10301)
...
### [306] [Conversational Search: From Fundamentals to Frontiers in the LLM Era](https://arxiv.org/abs/2506.10635)
*Fengran Mo,Chuan Meng,Mohammad Aliannejadi,Jian-Yun Nie*

Main category: cs.IR

TL;DR: This tutorial explores how large language models (LLMs) are transforming conversational search by enabling more flexible, dialogue-based interactions between users and systems, helping participants grasp both fundamentals and emerging topics.


<details>
  <summary>Details</summary>
Motivation: Conversational search aims to handle complex user queries through multi-turn dialogues, requiring systems to understand context and intent. LLMs offer new opportunities due to their advanced capabilities in following instructions, generating content, and reasoning.

Method: The tutorial provides an introduction to the relationship between foundational concepts and new topics in conversational search influenced by LLMs, targeting students, researchers, and professionals from both academic and industrial backgrounds.

Result: Participants will achieve a comprehensive understanding of core principles and state-of-the-art developments in conversational search driven by LLMs, enabling them to contribute to future system advancements.

Conclusion: LLMs are revolutionizing conversational search, and this tutorial serves as a valuable resource for those looking to understand and develop next-generation systems.
...
### [307] [Contrastive Matrix Completion with Denoising and Augmented Graph Views for Robust Recommendation](https://arxiv.org/abs/2506.10658)
